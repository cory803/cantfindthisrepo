...
import com.chaos.model.input.impl.EnterAmountOfPosOfferPrice;
...
public class PlayerOwnedShopContainer extends ItemContainer {
	...
	public void sellItem(Player player, int slot, int amountToSell, long price) {
		...
		if (price == 0) {
			...
			player.setInputHandling(new EnterAmountOfPosOfferPrice(slot, amountToSell)); // Enter
																							// the
		...
		for (int i = amountToSell; i > 0; i--) {
			...
			if (!itemToSell.getDefinition().isStackable()) {
				if (inventorySpace) {
					...
					// player.getInventory().add(new Item(995, itemValue),
					// false);
					PlayerLogs.other(player, "Player owned shop sold item: ID: " + itemToSell.getId() + " AMOUNT: "
							+ amountToSell + " PRICE: " + price + "");
	...
	public PlayerOwnedShopContainer switchItem(ItemContainer to, Item item, int slot, boolean sort, boolean refresh) {
		...
		if (!player.getUsername().equalsIgnoreCase(o.getOwner())) {
				...
				if (!item.getDefinition().isStackable()) {
					if (playerCurrencyAmount >= value && hasInventorySpace(player, item, 995, (int) value)) {
						if (usePouch) {
							...
							PlayerLogs.other(player, "Player owned shop removed coins from inventory: " + value
			...
			if (owner != null) {
				PlayerLogs.other(owner, "Player owned shop recieved coins to money pouch: " + formatAmount(total) + "");
				...
				PlayerLogs.other(player, "Player owned shop added coins for global extraction: " + formatAmount(total)
						+ " for " + o.getOwner() + "");
