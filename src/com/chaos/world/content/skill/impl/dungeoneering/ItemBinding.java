...
public class ItemBinding {
	...
	public static int getRandomBindableItem() {
		...
		if (ItemDefinition.forId(BINDABLE_ITEMS[index][0]).getName().toLowerCase().contains("body")
				...
				|| ItemDefinition.forId(BINDABLE_ITEMS[index][0]).getName().toLowerCase().contains("skirt")) {
	...
	public static void bindItem(Player p, int item) {
		if (Dungeoneering.doingDungeoneering(p)) {
			...
			if (amountBound >= 4) {
				...
				return;
			} else if (amountBound == 3 && p.getSkillManager().getCurrentLevel(Skill.DUNGEONEERING) < 120) {
				p.getPacketSender()
						.sendMessage("You need a Dungeoneering level of at least 120 to have 4 bound items.");
				return;
			} else if (amountBound == 2 && p.getSkillManager().getCurrentLevel(Skill.DUNGEONEERING) < 85) {
				p.getPacketSender().sendMessage("You need a Dungeoneering level of at least 85 to have 3 bound items.");
				return;
			} else if (amountBound == 1 && p.getSkillManager().getCurrentLevel(Skill.DUNGEONEERING) < 50) {
				p.getPacketSender().sendMessage("You need a Dungeoneering level of at least 50 to have 2 bound items.");
				return;
			}
