...
public class House extends RegionInstance {
	...
	public void process() {
		...
		Room r = house.getOwner().getHouseRooms()[player
				.inConstructionDungeon() ? 4 : player.getPosition().getZ() % 4][myTiles[0] - 1][myTiles[1] - 1];
		if (r == null)
			return;
	...
	/*
	 * public void playerKilled(Player p) { int[] myTiles =
	 * Construction.getMyChunk(p); if(p.combatRingType > 0) {
	 * p.getPacketSender().sendInteractionOption("null", 3, true);
	 * p.combatRingType = 0; p.moveTo(new
	 * Position(ConstructionData.BASE_X+(myTiles[0]*8)+1,
	 * ConstructionData.BASE_Y+(myTiles[1]*8)+1, p.getPosition().getZ())); }
	 * else { PlayerFurniture portal = Construction.findNearestPortal(p); int
	 * toX = ConstructionData.BASE_X+((portal.getRoomX()+1)*8); int toY =
	 * ConstructionData.BASE_Y+((portal.getRoomY()+1)*8); p.moveTo(new
	 * Position(toX+2, toY+2, 0)); } }
	 */
