...
public class Construction {
	...
	public static void moveHouse(Player p, HouseLocation location) {
		...
		if (p.getHouseLocation() == location) {
			...
			if (p.getInventory().getAmount(995) >= location.getCost()) {
				...
				p.getPacketSender()
						.sendMessage("Your house has succesfully been moved to " + location.toString() + ".");
				p.getPacketSender().sendInterfaceRemoval();
			} else {
	...
	public static void switchTheme(Player p, HouseTheme theme) {
		...
		if (p.getHouseTheme() == theme) {
			...
			if (p.getInventory().getAmount(995) >= theme.getCost()) {
				...
				p.getPacketSender()
						.sendMessage("Your house has succesfully been redecorated as " + theme.toString() + ".");
				for (int x = 0; x < 13; x++)
					for (int y = 0; y < 13; y++)
			...
			public void execute() {
				...
				case 1:
					if (me.inConstructionDungeon())
						me.getPacketSender()
								.constructMapRegion(((House) houseOwner.getRegionInstance()).getSecondaryPalette());
					else
				...
				case 4:
					// if (me.inConstructionDungeon())
					// me.getPacketSender().constructMapRegion(((House)
					// houseOwner.getRegionInstance()).getSecondaryPalette());
					// else
					// me.getPacketSender().constructMapRegion(((House)
					// houseOwner.getRegionInstance()).getPalette());
					break;
				case 5:
					me.getPacketSender().sendInterfaceRemoval();
					...
					if (me.getConstructionCoords() != null) {
						me.moveTo(new Position(me.getConstructionCoords()[0], me.getConstructionCoords()[1],
								me.getConstructionCoords()[2]));
	...
	public static HouseFurniture findNearestPortal(Player p) {
		...
		for (HouseFurniture hf : portals) {
			if (new Position(hf.getRoomX(), hf.getRoomY())
					.getDistance(p.getPosition().setZ(0)) < new Position(closest.getRoomX(), closest.getRoomY())
							.getDistance(p.getPosition().setZ(0))) {
				closest = hf;
			}
		}
	...
	public static void doFurniturePlace(HotSpots s, Furniture f, ArrayList<HotSpots> hsses, int[] myTiles, int actualX,
		...
		if (remove) {
			p.getPacketSender().sendObject_cons(actualX, actualY, -1, 0, 0, height);
			return;
		}
		...
		if (s.getHotSpotId() == 92) {
			...
			if (s.getObjectId() == 15326 || s.getObjectId() == 15327) {
				p.getPacketSender().sendObject_cons(actualX, actualY, s.getObjectId() == 15327
						? (placeBack ? 15327 : f.getFurnitureId() + 1) : (placeBack ? 15326 : f.getFurnitureId()),
						s.getRotation(roomRot), 0, height);
				offsetX += ConstructionData.getXOffsetForObjectId(f.getFurnitureId(),
	...
	public static boolean buildActions(Player p, Furniture f, HotSpots hs) {
		...
		/*
		 * Check to make sure 5 seconds have passed before we can build
		 * something else to prevent spamming
		 */
	...
	public static boolean handleFirstObjectClick(int obX, int obY, int objectId, Player p) {
		...
		// Stairs
		if (objectId == ConstructionData.Furniture.OAK_STAIRCASE.getFurnitureId()
				|| objectId == ConstructionData.Furniture.OAK_STAIRCASE_1.getFurnitureId()
				|| objectId == ConstructionData.Furniture.TEAK_STAIRCASE.getFurnitureId()
				|| objectId == ConstructionData.Furniture.TEAK_STAIRCASE_1.getFurnitureId()
				|| objectId == ConstructionData.Furniture.MARBLE_STAIRCASE.getFurnitureId()
				|| objectId == ConstructionData.Furniture.MARBLE_STAIRCASE_1.getFurnitureId()) {
			int myTiles[] = getMyChunk(p);
			Room room = p.getRegionInstance().getOwner().getHouseRooms()[1][myTiles[0] - 1][myTiles[1] - 1];
			if (room == null) {
		...
		if (objectId == ConstructionData.Furniture.DUNGEON_ENTRANCE.getFurnitureId()) {
			 ...
			 * { p.setInConstructionDungeon(true); p.setConstructionCoords(new
			 * int[] { p.getPosition().getX(), p.getPosition().getY(), 4 });
			 * p.moveTo(new Position(ConstructionData.MIDDLE_X,
			 * ConstructionData.MIDDLE_Y, 4)); if (p.inConstructionDungeon()) {
			 * p.getPacketSender().constructMapRegion(((House)
			 * p.getRegionInstance()).getSecondaryPalette());
			 * placeAllFurniture(p, 4);
			 * p.getPacketSender().constructMapRegion(((House)
			 ...
			 * Position(p.getConstructionCoords()[0],
			 * p.getConstructionCoords()[1], p.getConstructionCoords()[2]));
			 * p.setConstructionCoords(null); return true; }
			 */
		}
		return false;
	}
	...
	public static void createRoom(int roomType, Player p, int toHeight) {
		...
		if (rd == null)
			return;
		if (rd == RoomData.MENAGERY || rd == RoomData.QUEST_HALL_DOWN || rd == RoomData.QUEST_ROOM
				|| rd == RoomData.SKILL_ROOM) {
		...
		if (rd.getLevelToBuild() > p.getSkillManager().getCurrentLevel(Skill.CONSTRUCTION)) {
			p.getPacketSender().sendMessage(
					"You need a Construction level of at least " + rd.getLevelToBuild() + " to build this room.");
			return;
		}
		...
		house.getOwner().getHouseRooms()[p.inConstructionDungeon() ? 4 : toHeight][(myTiles[0] - 1)
				+ xOff][(myTiles[1] - 1) + yOff] = new Room(rotation, roomType, toHeight);
		...
		p.moveTo(
				new Position(p.getConstructionCoords()[0], p.getConstructionCoords()[1], p.getConstructionCoords()[2]));
		p.setConstructionCoords(null);
		p.getPacketSender().sendInterfaceRemoval();
	...
	public static void deleteRoom(Player p, int toHeight) {
		...
		if (p.getPosition().getZ() == 0) {
			...
			p.getRegionInstance().getOwner().getHouseRooms()[p.inConstructionDungeon() ? 4
					: toHeight][chunkX][chunkY] = new Room(0, roomType, toHeight);
		} else {
			if (p.inConstructionDungeon()) {
