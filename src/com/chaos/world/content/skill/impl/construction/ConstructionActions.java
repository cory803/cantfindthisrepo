...
public class ConstructionActions {
	...
	public static boolean handleFirstObjectClick(Player p, int objectId) {
		...
		case 13627:
		case 13634:
			if (TeleportHandler.checkReqs(p, ConstructionConstants.YANILLE, true)) {
				p.getRegionInstance().remove(p);
				TeleportHandler.teleportPlayer(p, ConstructionConstants.YANILLE, TeleportType.NORMAL, true);
			}
			break;
	...
	public static boolean handleItemOnObject(Player p, int objectId, int item) {
		...
		case 13197:
			if (Prayer.isBone(item))
				BonesOnAltar.openInterface(p, item, objectId == 13197);
			break;
