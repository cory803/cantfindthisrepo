...
import com.chaos.world.content.skill.impl.construction.dialogs.RedirectPortals;
import com.chaos.world.content.skill.impl.construction.dialogs.WithdrawSupplies;
...
/**
 * THIS IS FUCKING BAD I KNOW *cries*
 * 
 * @author senne
 *
 */
public class ConstructionActions {
	...
	public static boolean handleFirstObjectClick(Player p, int objectId) {
		...
		switch (objectId) {
		case 15477:
			p.getDialog().sendDialog(new MainPortal(p));
			break;
		case 13405:
			p.getRegionInstance().remove(p);
			TeleportHandler.teleportPlayer(p, new Position(3086, 3502, 0), TeleportType.NORMAL);
			break;
		...
		case 13197:
			...
			break;
		case 13640:
			p.getDialog().sendDialog(new RedirectPortals(p));
			break;
	...
	public static boolean handleItemOnObject(Player p, int objectId, int item) {
		...
		case 13197:
			if (Prayer.isBone(item))
				BonesOnAltar.openInterface(p, item, false);
			break;
	...
	private static boolean handleLarder(Player p, int objectId) {
		if (objectId == Furniture.WOODEN_LARDER.getFurnitureId()) {
			p.getDialog().sendDialog(new WithdrawSupplies(p, 11));
			return true;
		} else if (objectId == Furniture.OAK_LARDER.getFurnitureId()) {
			p.getDialog().sendDialog(new WithdrawSupplies(p, 12));
			return true;
		} else if (objectId == Furniture.TEAK_LARDER.getFurnitureId()) {
			p.getDialog().sendDialog(new WithdrawSupplies(p, 13));
			return true;
		}
	...
	private static boolean handleShelves(Player p, int objectId) {
		...
		//TODO: Code shelves to get supplies
