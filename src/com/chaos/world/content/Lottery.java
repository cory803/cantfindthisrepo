...
public class Lottery {
	...
	/**
	 * Adds a user to the lottery by writing their username to the file aswell
	 * as adding them to the list of users who have entered already.
	 * 
	 * @param user
	 *            The username to add to the lists.
	 */
	...
	/**
	 * Reads the lottery list and adds every user from the .txt files to the
	 * lists.
	 */
	public static void init() {
		try {
			...
			while (true) {
				...
				if (line.length() > 0) {
					if (!CONTESTERS.contains(line)) // user might have gotten on
													// list twice somehow..
													// don't
													// give them extra chance of
													// winning
						CONTESTERS.add(line);
				}
			}
	...
	/**
	 * Handles the lottery for a player on login Checks if a user won the
	 * lottery without being rewarded.
	 * 
	 * @param p
	 *            The player to handle login for.
	 */
