...
public final class MovementQueue {
    ...
    /**
     * Adds the first step to the queue, attempting to connect the server and client position by
     * looking at the previous queue.
     *
     ...
     * @param y Y to walk to Can the step walk through objects?
     */
    ...
    public boolean canWalk(int deltaX, int deltaY) {
        ...
        if (character.getPosition().getZ() == -1 && to.getZ() == -1 && character.isNpc()
                ...
                || character.getLocation() == Location.RECIPE_FOR_DISASTER)
            return true;
   ...
   * if(SoulWars.checkBarricade(position.getX() + x, position.getY()+ y, position.getZ())) {
   * ((Player)character).getPacketSender().sendMessage( "The path is blocked by a Barricade.");
   * reset(true); return true; } } } return false; }
   */
    ...
    public void handleRegionChange() {
        final int diffX =
                character.getPosition().getX() - character.getLastKnownRegion().getRegionX() * 8;
        final int diffY =
                character.getPosition().getY() - character.getLastKnownRegion().getRegionY() * 8;
                ...
                public void execute() {
                    ...
                    if (!Location.ignoreFollowDistance(character)) {
                        boolean summNpc = followCharacter.isPlayer() && character.isNpc()
                                && ((NPC) character).isSummoningNpc();
                        if (!character.getPosition().isWithinDistance(followCharacter.getPosition(), summNpc ? 10 : 7)) {
                            character.setEntityInteraction(null);
                            this.stop();
                            if (summNpc)
     ...
     */
    public static final int FIRST_MOVEMENT_X = 0, FIRST_MOVEMENT_Y = 1, SECOND_MOVEMENT_X = 2,
            SECOND_MOVEMENT_Y = 3, MOVEMENT_SPEED = 4, MOVEMENT_REVERSE_SPEED = 5, MOVEMENT_DIRECTION = 6;
