...
/**
 * A static utility class that contains methods for changing the appearance animation for a player
 * whenever a new weapon is equipped or an existing item is unequipped.
 * 
 * @author lare96
 */
public final class WeaponAnimations {
  ...
  /**
   * Executes an animation for the argued player based on the animation of the argued item.
   * 
  ...
  public static CharacterAnimations getUpdateAnimation(Item item) {
    ...
    if (weaponName.contains("halberd") || weaponName.contains("guthan")) {
      ...
      playerWalkIndex = 1146;
        ...
        || weaponName.contains("staff") || weaponName.contains("spear") || item.getId() == 21005
        || item.getId() == 21010) {
  ...
  public static int getAttackAnimation(Player c) {
    ...
    if (weaponName.contains("2h sword") || weaponName.contains("godsword")
      ...
      /*
       * if(prop.contains("stab")) return 11981; else if(prop.contains("slash")) return 11980;
       */
      return 11979;
    }
  ...
  public static int getBlockAnimation(Player c) {
    int weaponId = c.getEquipment().getItems()[Equipment.WEAPON_SLOT].getId();
    String shield = ItemDefinition.forId(c.getEquipment().getItems()[Equipment.SHIELD_SLOT].getId())
        .getName().toLowerCase();
