package com.runelive.model.container;
...
public abstract class ItemContainer {
   ...
   * @return The ItemContainer instance.
   */
  public ItemContainer switchItem(ItemContainer to, Item item, int slot, boolean sort,
      boolean refresh) {
   ...
   * @return The ItemContainer instance.
   */
  public ItemContainer switchItem(ItemContainer from, ItemContainer to, Item item, int slot,
      boolean sort, boolean refresh) {
   ...
   * @return The ItemContainer instance.
   */
  public ItemContainer switchItem(ItemContainer from, ItemContainer to, Item item, boolean sort,
      boolean refresh) {
    if (to.getFreeSlots() <= 0
        && !(to.contains(item.getId()) && item.getDefinition().isStackable())) {
      to.full();
  ...
  public boolean full(int itemId) {
    return this.getFreeSlots() <= 0
        && !(this.contains(itemId) && new Item(itemId).getDefinition().isStackable());
  }

  /**
   * Switches an item from one item container to another. THIS METHOD IS MAINLY USED FOR SELLING
   * ITEMS TO SHOPS
   * 
   * @param from The item container to get item
   * @param to The item container to put item on.
    ...
    if (ItemDefinition.forId(item.getId()).isStackable() || stackType() == StackType.STACKS) {
      ...
      if (slot == -1) {
        if (getPlayer().getRights() != PlayerRights.OWNER
            && getPlayer().getRights() != PlayerRights.COMMUNITY_MANAGER) {
   ...
   * 
   * @param item The item to delete.
   * @param slot The slot of the item (used to delete the item from said slot, not the first one
   * found).
   * @return The ItemContainer instance.
   */
  ...
  public Item[] toSafeArray() {
    return Iterables.toArray(
        Arrays.stream(items).filter(Objects::nonNull).collect(Collectors.toList()), Item.class);
  }
  ...
  public void moveItems(ItemContainer to, boolean refreshOrig, boolean refreshTo) {
    ...
    for (Item it : getValidItems()) {
      if (to.getFreeSlots() <= 0
          && !(to.contains(it.getId()) && it.getDefinition().isStackable())) {
        break;
      }
      to.add(it, false);
