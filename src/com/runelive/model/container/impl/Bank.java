...
import com.runelive.model.input.impl.ItemSearch;
...
public class Bank extends ItemContainer {
  ...
  public Bank open() {
    ...
    if (getPlayer().getBankPinAttributes().hasBankPin()
        && !getPlayer().getBankPinAttributes().hasEnteredBankPin()
    ...
    getPlayer().getPacketSender().sendConfig(115, getPlayer().withdrawAsNote() ? 1 : 0)
        ...
        .sendConfig(117,
            (getPlayer().getBankSearchingAttribtues().isSearchingBank()
                && getPlayer().getBankSearchingAttribtues().getSearchedBank() != null) ? 1 : 0)
  ...
  public Bank switchItem(ItemContainer to, Item item, int slot, boolean sort, boolean refresh) {
    if (!getPlayer().isBanking() || getPlayer().getInterfaceId() != 5292
        || to instanceof Inventory
            && !(getPlayer().getBank(getPlayer().getCurrentBankTab()).contains(item.getId())
                || getPlayer().getBankSearchingAttribtues().getSearchedBank() != null && getPlayer()
                    .getBankSearchingAttribtues().getSearchedBank().contains(item.getId()))) {
      getPlayer().getPacketSender().sendClientRightClickRemoval();
      return this;
    }
    ...
    if (to.getFreeSlots() <= 0
        ...
        && !(getPlayer().withdrawAsNote() && def != null && def.isNoted()
            && to.contains(def.getId()))) {
      to.full();
      return this;
    }
  ...
  public Bank refreshItems() {
    ...
    getPlayer().getPacketSender().sendItemContainer(getPlayer().getInventory(),
  ...
  public static void sendTabs(Player player) {
    ...
    /*
     * boolean moveRest = false; for(int i = 1; i <= 7; i++) { if(isEmpty(player.getBank(i)) ||
     * moveRest) { int j = i+2 > 8 ? 8 : i+2; player.setBank(i, player.getBank(j));
     * player.setBank(j, new Bank(player)); moveRest = true; } }
     */
  ...
  public static void depositItems(Player p, ItemContainer from, boolean ignoreReqs) {
    ...
    for (Item it : from.getValidItems()) {
      ...
      Item toBank =
          new Item(ItemDefinition.forId(it.getId()).isNoted() ? (it.getId() - 1) : it.getId(),
              it.getAmount());
   ...
   */
  public static final int[][] BANK_TAB_INTERFACES =
      {{5, 0}, {13, 1}, {26, 2}, {39, 3}, {52, 4}, {65, 5}, {78, 6}, {91, 7}, {104, 8}};
  ...
  public static class BankSearchAttributes {
    ...
    public static void stopSearch(Player player, boolean openBank) {
      player.getPacketSender().sendClientRightClickRemoval();
      player.getBankSearchingAttribtues().setSearchedBank(null).setSearchingBank(false)
          .setSearchSyntax(null);
      player.setCurrentBankTab(0).setNoteWithdrawal(false);
      player.getPacketSender().sendString(27002, Integer.toString(0)).sendString(27000, "1")
          .sendConfig(117, 0).sendString(5383, "        The Bank of RuneLive");
      if (openBank)
