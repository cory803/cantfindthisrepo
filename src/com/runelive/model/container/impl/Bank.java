...
public class Bank extends ItemContainer {
    ...
    public Bank switchItem(ItemContainer to, Item item, int slot, boolean sort, boolean refresh) {
        ...
        if (getPlayer().getBankSearchingAttributes().isSearchingBank() && getPlayer().getBankSearchingAttributes().getSearchedBank() != null) {
            int tab = Bank.getTabForItem(getPlayer(), item.getId());
            if (!getPlayer().getBank(tab).contains(item.getId()) || !getPlayer().getBankSearchingAttributes().getSearchedBank().contains(item.getId())) {
                return this;
            }
    ...
    public static void depositItems(Player p, ItemContainer from, boolean ignoreReqs) {
        ...
        for (Item it : from.getValidItems()) {
            ...
            if (p.getBankSearchingAttributes().isSearchingBank() && p.getBankSearchingAttributes().getSearchedBank() != null) {
    ...
    public static class BankSearchAttributes {
        ...
        public static void stopSearch(Player player, boolean openBank) {
            player.getPacketSender().sendClientRightClickRemoval();
            player.getBankSearchingAttributes().setSearchedBank(null).setSearchingBank(false)
