...
/**
 * This file represents a player's relation with other world entities, this manages adding and
 * removing friends who we can chat with and also adding and removing ignored players who will not
 * be able to message us or see us online.
 *
...
public class PlayerRelations {
   ...
   * 
   * @param online If <code>true</code>, the players who have this player added, will be sent the
   * notification this player has logged in.
   * @return The PlayerRelations instance.
   */
  public PlayerRelations updateLists(boolean online, int sendmessage) {
    ...
    /*
     * for (Player players : World.getPlayers()) { if(players == null) continue; boolean
     * temporaryOnlineStatus = online; if
     * (players.getRelations().friendList.contains(player.getLongUsername())) { if
     * (status.equals(PrivateChatStatus.FRIENDS_ONLY) &&
     * !friendList.contains(players.getLongUsername()) || status.equals(PrivateChatStatus.OFF) ||
     * ignoreList.contains(players.getLongUsername())) { temporaryOnlineStatus = false; }
     * players.getPacketSender().sendFriend(player.getLongUsername(), temporaryOnlineStatus ? 1 :
     * 0); } boolean tempOn = true; if
     * (player.getRelations().friendList.contains(players.getLongUsername())) { if
     ...
     * players.getRelations().getIgnoreList().contains(player.getLongUsername())) { tempOn = false;
  ...
  public void sendStatus() {
    int privateChat =
        status == PrivateChatStatus.OFF ? 2 : status == PrivateChatStatus.FRIENDS_ONLY ? 1 : 0;
  ...
  public void addFriend(Long username) {
    ...
    if (ignoreList.contains(username)) {
      player.getPacketSender()
          .sendMessage("Please remove " + name + " from your ignore list first.");
      return;
    }
  ...
  public void addIgnore(Long username) {
    ...
    if (friendList.contains(username)) {
      player.getPacketSender()
          .sendMessage("Please remove " + name + " from your friend list first.");
      return;
    }
    if (ignoreList.contains(username)) {
  ...
  public void message(Player friend, byte[] message, int size) {
    ...
    if (this.player.getSkillManager().getTotalLevel() < tlNeeded && !friend.getRights().isStaff()) {
      this.player.getPacketSender()
          .sendMessage("You cannot use the private message until you have over " + (tlNeeded + 1)
              + " total level.");
    ...
    if (Misc.blockedWord(Misc.textUnpack(message, size).replaceAll(";", "."))) {
      // Logs.write_data(this.player.getUsername()+ ".txt", "advertisers", "Player was caught saying
      // in private message: "+Misc.textUnpack(message, size).replaceAll(";", ".")+"");
      return;
    }
    friend.getPacketSender().sendPrivateMessage(player.getLongUsername(), player.getRights(),
        message, size, player);
    // Logs.write_data(friend.getUsername()+ ".txt", "private_messages", "Recieved from
    // "+this.player.getUsername()+": " + Misc.textUnpack(message, size).replaceAll(";", "."));
    // Logs.write_data(this.player.getUsername()+ ".txt", "private_messages", "Sent to
    // "+friend.getUsername()+": " + Misc.textUnpack(message, size).replaceAll(";", "."));
  }

  /**
   * Represents a player's friends list status, whether others will be able to see them online or
   * not.
   */
