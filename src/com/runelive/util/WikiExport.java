  ...
  public static void exportDropTable(int npcId) {
	...
	for(int i = 0; i < NPC_DROP_RARE.size(); i++) {
		...
		for(int i2 = 0; i2 < ItemDropAnnouncer.TO_ANNOUNCE.length; i2++) {
			if(ItemDropAnnouncer.TO_ANNOUNCE[i2] != dropItem.getId() && i2 == ItemDropAnnouncer.TO_ANNOUNCE.length) {
				dumpData("{{DropsLine|Name="+ItemDefinition.forId(dropItem.getId()).getName()+"|Quantity="+dropItem.getAmount()+"|Rarity=uncommon}}", ""+npcId+"");
			}
		}
	} 
	...
	for(int i = 0; i < NPC_DROP_RARE.size(); i++) {
		...
		for(int i2 = 0; i2 < ItemDropAnnouncer.TO_ANNOUNCE.length; i2++) {
			if(ItemDropAnnouncer.TO_ANNOUNCE[i2] == dropItem.getId()) {
				dumpData("{{DropsLine|Name="+ItemDefinition.forId(dropItem.getId()).getName()+"|Quantity="+dropItem.getAmount()+"|Rarity=rare}}", ""+npcId+"");
			}
		}
	} 
  }
