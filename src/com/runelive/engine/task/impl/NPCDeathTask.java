...
/**
 * Represents an npc's death task, which handles everything an npc does before and after their death
 * animation (including it), such as dropping their drop table items.
 * 
 * @author relex lawl
 */
...
public class NPCDeathTask extends Task {
  ...
  public void stop() {
    ...
    if (npc.getDefinition().getRespawnTime() > 0 && npc.getLocation() != Location.BOSS_SYSTEM
        ...
        && npc.getLocation() != Location.WILDKEY_ZONE && npc.getLocation() != Location.ZULRAH_PIT && npc.getLocation() != Location.SHREKS_HUT) {
