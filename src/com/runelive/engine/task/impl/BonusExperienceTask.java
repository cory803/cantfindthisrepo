...
public class BonusExperienceTask extends Task {
  ...
  public void execute() {
    ...
    if (newMinutes < 0) {
      player.getPacketSender()
          .sendMessage("<img=4> <col=330099>Your bonus experience has run out.");
      player.setMinutesBonusExp(-1, false);
      stop();
    } else if (msg == 2) {
      player.getPacketSender().sendMessage("<img=4> <col=330099>You have "
          + player.getMinutesBonusExp() + " minutes of bonus experience left.");
      msg = 0;
    }
    msg++;
  }
  ...
  public static void addBonusXp(final Player p, int minutes) {
    ...
    p.getPacketSender().sendMessage("<img=4> <col=330099>You have " + p.getMinutesBonusExp()
