package com.runelive.net.security;
...
public class ConnectionHandler {
    ...
    if (!isLocal(host)) {
      if (CONNECTIONS.get(host) != null) {
        if (CONNECTIONS.get(host) >= GameSettings.CONNECTION_AMOUNT) {
          System.out
              .println("Connection limit reached : " + player.getUsername() + ". Host: " + host);
   ...
   */
  private static final Map<String, Integer> CONNECTIONS =
   ...
   * The concurrent map of registered connections.
   */
  private static final Map<String, Integer> STARTERS =
      Collections.synchronizedMap(new HashMap<String, Integer>());
    ...
    GameServer.getLoader().getEngine().submit(() -> {
      try {
        BufferedWriter writer =
            new BufferedWriter(new FileWriter(BLACKLISTED_SERIAL_NUMBERS_DIR, true));
   ...
   * @return {@code true} if the host is connecting locally, {@code false} otherwise.
   */
  public static boolean isLocal(String host) {
    return host == null || host.equals("null") || host.equals("127.0.0.1")
