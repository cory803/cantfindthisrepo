...
/**
 * This packet listener is called when a player drops an item they have placed in their inventory.
 * 
 * @author relex lawl
 */
...
public class DropItemPacketListener implements PacketListener {
    ...
    if (item.getId() == 21077) {
      if (player.getInventory().contains(21080) || player.getInventory().getFreeSlots() == 1) {
        ...
        player.getPacketSender()
            .sendMessage("You have removed the charges from your Toxic staff of the dead.");
      } else {
        player.getPacketSender()
            .sendMessage("You must have atleast 1 inventory space in order to uncharge your item.");
      }
      return;
    }
	*/
    if (item != null && item.getId() != -1 && item.getAmount() >= 1) {
      if (item.tradeable() && !ItemBinding.isBoundItem(item.getId())) {
        ...
        if (item.getId() == 4045) {
          ...
          player.dealDamage(
              new Hit((player.getConstitution() - 1) == 0 ? 1 : player.getConstitution() - 1,
  ...
  public static void destroyItemInterface(Player player, Item item) {// Destroy item created by
                                                                     // Remco
    player.setUntradeableDropItem(item);
    String[][] info = {// The info the dialogue gives
        {"Are you sure you want to discard this item?", "14174"}, {"Yes.", "14175"},
