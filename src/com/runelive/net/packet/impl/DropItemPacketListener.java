...
public class DropItemPacketListener implements PacketListener {
	...
	public void handleMessage(Player player, Packet packet) {
		...
		/*
		 * if (item.getId() == 21077) { if
		 * (player.getInventory().contains(21080) ||
		 * player.getInventory().getFreeSlots() == 1) {
		 * player.getInventory().delete(new Item(21077, 1));
		 * player.getInventory().add(new Item(21079, 1));
		 * player.getInventory().add(new Item(21080,
		 * player.getToxicStaffCharges())); player.setToxicStaffCharges(0);
		 * player.getPacketSender() .sendMessage(
		 * "You have removed the charges from your Toxic staff of the dead."); }
		 * else { player.getPacketSender() .sendMessage(
		 * "You must have atleast 1 inventory space in order to uncharge your item."
		 * ); } return; }
		 */
