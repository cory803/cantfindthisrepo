...
public class HeightCheckPacketListener implements PacketListener {
  ...
  public void handleMessage(Player player, Packet packet) {
    ...
    if (player.getPosition().getZ() >= 0 && player.getPosition().getZ() < 4) { // Only check for
                                                                               // normal height
                                                                               // levels, not
                                                                               // minigames etc

      if (plane != player.getPosition().getZ()) { // Player might have used a third-party-software
                                                  // to change their height level
