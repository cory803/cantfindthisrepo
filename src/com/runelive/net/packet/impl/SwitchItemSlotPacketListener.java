  ...
  public void handleMessage(Player player, Packet packet) {
    ...
    if (player.isBanking() && player.getInterfaceId() == 5292) {
      ...
      for (int i = 0; i < Bank.BANK_TAB_INTERFACES.length; i++) {
        ...
        Item item =
            new Item(player.getBank(player.getCurrentBankTab()).getItems()[fromSlot].getId(),
                player.getBank(player.getCurrentBankTab()).getItems()[fromSlot].getAmount());
        if (!player.getBank(player.getCurrentBankTab()).contains(item.getId()) || player
        ...
        if (toBankTab) {
          ...
          player.getBank(Bank.getTabForItem(player, item.getId()))
              .switchItem(player.getBank(bankTab), item, slot, true, false);
