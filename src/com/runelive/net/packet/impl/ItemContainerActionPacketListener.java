...
import com.runelive.model.input.impl.EnterAmountToBuyFromPos;
import com.runelive.model.input.impl.EnterAmountToBuyFromShop;
import com.runelive.model.input.impl.EnterAmountToPriceCheck;
import com.runelive.model.input.impl.EnterAmountToRemoveFromBank;
import com.runelive.model.input.impl.EnterAmountToRemoveFromBob;
import com.runelive.model.input.impl.EnterAmountToRemoveFromPriceCheck;
import com.runelive.model.input.impl.EnterAmountToRemoveFromStake;
import com.runelive.model.input.impl.EnterAmountToRemoveFromTrade;
import com.runelive.model.input.impl.EnterAmountToSellToPos;
import com.runelive.model.input.impl.EnterAmountToSellToShop;
import com.runelive.model.input.impl.EnterAmountToStake;
import com.runelive.model.input.impl.EnterAmountToStore;
  ...
  private static void firstAction(Player player, Packet packet) {
    ...
    switch (interfaceId) {
      ...
      case Equipment.INVENTORY_INTERFACE_ID:
        ...
        boolean stackItem =
            item.getDefinition().isStackable() && player.getInventory().getAmount(item.getId()) > 0;
      ...
      case Bank.INTERFACE_ID:
        if (!player.isBanking() || player.getInterfaceId() != 5292)
          break;
        player.getBank(player.getCurrentBankTab()).switchItem(player.getInventory(), item, slot,
            true, true);
      ...
      case Bank.INVENTORY_INTERFACE_ID:
        ...
        player.getInventory().switchItem(player.getBank(player.getCurrentBankTab()), item, slot,
            false, true);
        break;
      ...
      case BeastOfBurden.INTERFACE_ID:
        if (player.getInterfaceId() == BeastOfBurden.INTERFACE_ID
          ...
          player.getInventory().switchItem(player.getSummoning().getBeastOfBurden(), item, slot,
              false, true);
        }
        break;
      case PriceChecker.INTERFACE_PC_ID:
        if (player.getInterfaceId() == PriceChecker.INTERFACE_ID
    ...
    switch (interfaceId) {
      ...
      case Bank.INTERFACE_ID:
        ...
        player.getBank(player.getCurrentBankTab()).switchItem(player.getInventory(),
            new Item(id, 5), slot, true, true);
        player.getBank(player.getCurrentBankTab()).open();
        break;
      case Bank.INVENTORY_INTERFACE_ID:
        item = player.getInventory().forSlot(slot).copy().setAmount(5).copy();
        if (!player.isBanking() || item.getId() != id
            || !player.getInventory().contains(item.getId()) || player.getInterfaceId() != 5292)
          return;
        player.setCurrentBankTab(Bank.getTabForItem(player, item.getId()));
        player.getInventory().switchItem(player.getBank(player.getCurrentBankTab()), item, slot,
            false, true);
      ...
      case Shop.ITEM_CHILD_ID:
        if (player.getShop() != null) {
          ...
          player.getShop().setPlayer(player).switchItem(player.getInventory(), item, slot, false,
              true);
        } else if (player.getPlayerOwnedShop() != null) {
          item = player.getPlayerOwnedShop().forSlot(slot).copy().setAmount(1).copy();
          player.getPlayerOwnedShop().setPlayer(player).switchItem(player.getInventory(), item,
              slot, false, true);
        }
        break;
      case Shop.INVENTORY_INTERFACE_ID:
      ...
      case BeastOfBurden.INTERFACE_ID:
        if (player.getInterfaceId() == BeastOfBurden.INTERFACE_ID
          ...
          player.getInventory().switchItem(player.getSummoning().getBeastOfBurden(),
              new Item(id, 5), slot, false, true);
        }
        break;
      case PriceChecker.INTERFACE_PC_ID:
        if (player.getInterfaceId() == PriceChecker.INTERFACE_ID
            && player.getPriceChecker().isOpen()) {
          player.getInventory().switchItem(player.getPriceChecker(), new Item(id, 5), slot, false,
              true);
        }
        break;
      case 1119: // smithing interface row 1
    ...
    switch (interfaceId) {
        ...
        switch (id) {
          ...
          case 11283:
            ...
            if (charges >= 1) {
              ...
              player.getPacketSender()
                  .sendMessage("Your shield doesn't have enough power yet. It has "
      ...
      case BeastOfBurden.INTERFACE_ID:
        if (player.getInterfaceId() == BeastOfBurden.INTERFACE_ID
          ...
          player.getInventory().switchItem(player.getSummoning().getBeastOfBurden(), storeItem,
              slot, false, true);
        }
        break;
      case PriceChecker.INTERFACE_PC_ID:
        if (player.getInterfaceId() == PriceChecker.INTERFACE_ID
            && player.getPriceChecker().isOpen()) {
          player.getInventory().switchItem(player.getPriceChecker(), new Item(id, 10), slot, false,
              true);
        }
        break;
      case 1119: // smithing interface row 1
    ...
    switch (interfaceId) {
      ...
      case Bank.INTERFACE_ID:
        if (!player.isBanking() || player.getBank(Bank.getTabForItem(player, id)).getAmount(id) <= 0
          ...
          return;
        player.getBank(player.getCurrentBankTab()).switchItem(player.getInventory(),
            new Item(id, player.getBank(Bank.getTabForItem(player, id)).getAmount(id)), slot, true,
            true);
        player.getBank(player.getCurrentBankTab()).open();
        break;
      case Bank.INVENTORY_INTERFACE_ID:
        Item item = player.getInventory().forSlot(slot).copy()
            .setAmount(player.getInventory().getAmount(id));
        if (!player.isBanking() || item.getId() != id
            || !player.getInventory().contains(item.getId()) || player.getInterfaceId() != 5292)
          return;
        player.setCurrentBankTab(Bank.getTabForItem(player, item.getId()));
      ...
      case Bank.INTERFACE_ID:
        if (player.isBanking()) {
          if (interfaceId == 11) {
            ...
            player.getBank(player.getCurrentBankTab()).switchItem(player.getInventory(),
                new Item(id, player.getBank(Bank.getTabForItem(player, id)).getAmount(id) - 1),
                slot, true, true);
            player.getBank(player.getCurrentBankTab()).open();
          }
        }
