...
import com.runelive.world.content.skill.impl.construction.Palette;
...
/**
 * This class manages making the packets that will be sent (when called upon) onto the associated
 * player's client.
 * 
...
public class PacketSender {
  ...
  /**
   * Sends the map region a player is located in and also sets the player's first step position of
   * said region as their {@code lastKnownRegion}.
   * 
   * @return The PacketSender instance.
   */
  ...
  /**
   * Sends the state in which the player has their chat options, such as public, private, friends
   * only.
   * 
   ...
   * <p>
   * If the state is -1 then the screen fades from black to transparent. When the state is +1 the
   * screen fades from transparent to black. If the state is 0 all drawing is stopped.
   */
  ...
  /*
   * public PacketSender sendBlinkingHint(String title, String information, int x, int y, int speed,
   * int pause, int type, final int time) { player.getSession().queueMessage(new PacketBuilder(179,
   * PacketType.SHORT).putString(title).putString(information).putShort(x).putShort(y).put(speed).
   * put(pause).put(type)); if(type > 0) { TaskManager.submit(new Task(time, player, false) {
   * @Override public void execute() { player.getPacketSender().sendBlinkingHint("", "", 0, 0, 0, 0,
   * -1, 0); stop(); } }); } return this; }
   */
  ...
  public PacketSender sendInterfaceRemoval() {
    ...
    if (player.isPlayerOwnedShopping()) {
      sendClientRightClickRemoval().sendItemsOnInterface(PlayerOwnedShopContainer.INTERFACE_ID,
          new Item[] {new Item(-1)});
     ...
     * sendClientRightClickRemoval().sendItemsOnInterface(Shop.INTERFACE_ID, new Item[]{new
     * Item(-1)});
     * player.getMinigameAttributes().getFishingTrawlerAttributes().setViewingInterface(false).
     * getRewards().clear(); } if(player.getAdvancedSkills().getSummoning().isStoring()) {
  ...
  /*
   * public PacketSender sendConstructionInterfaceItems(ArrayList<Furniture> items) { PacketBuilder
   * builder = new PacketBuilder(53, PacketType.SHORT); builder.writeShort(38274);
   * builder.writeShort(items.size()); for (int i = 0; i < items.size(); i++) {
   * builder.writeByte(1); builder.writeLEShortA(items.get(i).getItemId() + 1); }
   ...
   * @param tilePosition The position on the square (middle = 2; west = 3; east = 4; south = 5;
   * north = 6)
  ...
  public PacketSender sendGlobalGraphic(Graphic graphic, Position position) {
    ...
    for (Player p : player.getLocalPlayers()) {
      if (p.getPosition().distanceToPoint(player.getPosition().getX(),
  ...
  public PacketSender sendInterfaceSpriteChange(int childId, int firstSprite, int secondSprite) {
    // player.write(new PacketBuilder(140).writeShort(childId).writeByte((firstSprite << 0) +
  ...
  public PacketSender sendProjectile(Position position, Position offset, int angle, int speed,
      int gfxMoving, int startHeight, int endHeight, int lockon, int time) {
    sendPosition(position);
  ...
  public void sendAllProjectile(Position position, Position offset, int angle, int speed,
      int gfxMoving, int startHeight, int endHeight, int lockon, int time) {
    for (Player all : player.getLocalPlayers()) {
      ...
      if (all.getPosition().isViewableFrom(position)) {
        all.getPacketSender().sendProjectile(position, offset, angle, speed, gfxMoving, startHeight,
  ...
  public void sendObject_cons(int objectX, int objectY, int objectId, int face, int objectType,
      int height) {
    ...
    if (objectId == -1 || objectId == 0 || objectId == 6951) {
      CustomObjects.spawnObject(player,
          new GameObject(objectId, new Position(objectX, objectY, height)));
    }
  }

  /*
   * public PacketSender constructMapRegion(Palette palette) { PacketBuilder bldr = new
   * PacketBuilder(241, PacketType.SHORT); if(palette != null) { bldr.putString("palette"); //Inits
   * map construction sequence bldr.putString(""+(player.getPosition().getRegionY() + 6)+"");
   * bldr.putString(""+(player.getPosition().getRegionX() + 6)+""); for (int z = 0; z < 4; z++) {
   * for (int x = 0; x < 13; x++) { for (int y = 0; y < 13; y++) { PaletteTile tile =
   * palette.getTile(x, y, z); boolean b = false; if (x < 2 || x > 10 || y < 2 || y > 10) b = true;
   * int toWrite = !b && tile != null ? 5 : 0; bldr.putString(""+toWrite+""); if(toWrite == 5) { int
   * val = tile.getX() << 14 | tile.getY() << 3 | tile.getZ() << 24 | tile.getRotation() << 1;
   * bldr.putString(""+val+""); } } } } } else { bldr.putString("null"); //Resets map construction
   * sequence } player.getSession().queueMessage(bldr); return this; }
   */
      ...
      for (int z = 0; z < 4; z++) {
        for (int x = 0; x < 13; x++) {
          for (int y = 0; y < 13; y++) {
            ...
            if (toWrite == 5) {
              int val = tile.getX() << 14 | tile.getY() << 3
                      | tile.getZ() << 24
                      | tile.getRotation() << 1;
              bldr.putString("" + val + "");
            }
  ...
  public PacketSender sendConstructionInterfaceItems(
  ...
  public PacketSender sendObjectsRemoval(int chunkX, int chunkY, int height) {
    player.getSession().queueMessage(
            new PacketBuilder(153).put(chunkX).put(chunkY).put(height));
    return this;
  }
}
