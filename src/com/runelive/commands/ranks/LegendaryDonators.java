	...
	public static void initiate_command(final Player player, String[] command, String wholeCommand) {
		...
		if(wholeCommand.toLowerCase().startsWith("yell")) {
			...
			if(!player.getYellTag().equals("invalid_yell_tag_set")) {
				World.sendYell("<img=10> <col=0>[<col=3E0069>"+player.getYellTag()+"<col=0>] "+player.getUsername()+": "+yellmessage, player);	
				player.getYellTimer().reset();
				return;
			}
			World.sendYell("<img=10> <col=0>[<col=3E0069>Legendary<col=0>] "+player.getUsername()+": "+yellmessage, player);	
			player.getYellTimer().reset();
		}
