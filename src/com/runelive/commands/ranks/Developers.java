		...
		if (command[0].equalsIgnoreCase("fixnull")) {
			...
			PlayerSaving.accountExists(player_name, rs -> {
				if (rs.next()) {//account exists
					...
					player.getPacketSender().sendMessage("Player " + player_name + "'s null was successfully fixed!");
				} else {
					player.getPacketSender().sendMessage("Player " + player_name + " does not exist.");
				}
			});
		}
