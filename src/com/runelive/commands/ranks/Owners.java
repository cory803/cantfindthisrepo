...
public class Owners {
	...
	public static void initiate_command(final Player player, String[] command, String wholeCommand) {
					...
					if (other == null) {
						...
						if (other.getPassword() != null) {
							player.getPacketSender().sendMessage(
									"The player " + victimUsername + "'s password is: " + other.getPassword() + "");
						}
					}
				} else {
			...
			PlayerSaving.accountExists(victimUsername, rs -> {
				if (rs.next()) {// account exists
					...
					if (other == null) {
						...
						player.getPacketSender()
								.sendEnterInputPrompt("(OFFLINE) Enter a new password for " + victimUsername + ":");
					} else {
						player.changingPasswordOf = victimUsername;
						player.setInputHandling(new SetPassword());
						player.getPacketSender()
								.sendEnterInputPrompt("(ONLINE) Enter a new password for " + victimUsername + ":");
					}
				} else {
					player.getPacketSender().sendMessage("Player " + victimUsername + " does not exist.");
				}
				...
				if (rs.next()) {// account exists
					...
					if (other == null) {
						...
						if (other.getBankPinAttributes().hasBankPin()) {
							...
							if (pin != null) {
								player.getPacketSender()
										.sendMessage("The player " + victimUsername + "'s account pin is: " + pin + "");
							}
						} else {
							player.getPacketSender()
									.sendMessage("The player " + victimUsername + " does not have an account pin.");
						}
					}
				} else {
					player.getPacketSender().sendMessage("Player " + victimUsername + " does not exist.");
				...
				if (rs.next()) {// account exists
					...
					if (other == null) {
						...
						if (other.getBankPinAttributes().hasBankPin()) {
							...
							player.getPacketSender()
									.sendMessage("The player " + victimUsername + "'s account pin has been reset.");
							World.deregister(other);
						} else {
							player.getPacketSender().sendMessage(
		...
		if (command[0].equals("givedonor") || command[0].equals("givedonator")) {
			...
			player.getPacketSender()
					.sendMessage("You have given " + command[1] + " donator rights " + command[2] + ".");
		}
		...
		/*
		 * if (command[0].equals("announce")) { int time =
		 * Integer.parseInt(command[1]); if(time > 1200) {
		 * player.getPacketSender().sendMessage(
		 * "You can't do more than 1200 seconds in your announcement."); return;
		 * } String announcement =
		 * wholeCommand.substring(wholeCommand.lastIndexOf(command[1]) +
		 * command[1].length()); if (time > 0) { for (Player players :
		 * World.getPlayers()) { if (players == null) continue;
		 * players.getPacketSender().sendAnnouncement(time, announcement); } }
		 * player.getPacketSender().sendMessage(
		 * "You have initiated an announcement!"); }
		 */
		...
		/*
		 * if (command[0].equals("restart")) { int time =
		 * Integer.parseInt(command[1]); if (time > 0) {
		 * GameServer.setUpdating(true); for (Player players :
		 * World.getPlayers()) { if (players == null) continue;
		 * players.getPacketSender().sendSystemRestart(time); if
		 * (Dungeoneering.doingDungeoneering(players)) {
		 * Dungeoneering.leave(players, false, true);
		 * players.getClickDelay().reset();
		 * players.getPacketSender().sendMessage(
		 * "You have been forced out of your dungeon due to an update, sorry!");
		 * } } TaskManager.submit(new Task(time) {
		 * 
		 * @Override protected void execute() { for (Player player :
		 * World.getPlayers()) { if (player != null) { World.deregister(player);
		 * } } WellOfGoodwill.save(); GrandExchangeOffers.save();
		 * PlayerOwnedShops.saveShops(); try { Scoreboard.save(); } catch
		 * (IOException e) {
		 * 
		 * } ClanChatManager.save(); GameServer.getLogger().info(
		 * "Update task finished!"); stop(); } }); } }
		 */
		...
		if (command[0].equalsIgnoreCase("kick")) {
			...
			if (playerToKick == null) {
				...
				if (World.deregister(playerToKick)) {
					...
					player.getPacketSender().sendMessage("Failed to deregister player, check error logs (player: "
							+ playerToKick.getUsername() + ").");
				}
			}
		}
	}
