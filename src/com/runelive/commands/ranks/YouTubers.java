	...
	public static void initiate_command(final Player player, String[] command, String wholeCommand) {
		if (command[0].equals("bank")) {
			if (player.getLocation() == Location.WILDERNESS) {
				player.getPacketSender().sendMessage("You can't do this inside the wilderness!");
				return;
			}
			...
			player.getPacketSender().sendMessage("You have opened your bank!");
		}
		if (player.isJailed()) {
		...
		if (wholeCommand.toLowerCase().startsWith("yell")) {
			...
			if (!player.getYellTag().equals("invalid_yell_tag_set")) {
				World.sendYell("<img=11> <col=0>[<col=ffff00><shad=0>" + player.getYellTag() + "<col=0></shad>] "
						+ player.getUsername() + ": " + yellmessage, player);
				player.getYellTimer().reset();
				return;
			}
			World.sendYell("<img=5> <col=0>[<col=ff0000><shad=620000>YouTuber</shad><col=0>] " + player.getUsername()
					+ ": " + yellmessage, player);
		}
