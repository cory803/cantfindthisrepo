  ...
  public boolean execute(Player player, String key, String input) throws Exception {
    ...
    if (!player.getRights().isStaff()) {
      if (input.contains("<img=") || input.contains("<col=") || input.contains("<shad=")) {
        player.getPacketSender()
            .sendMessage("You are not aloud to put these symbols in your yell message.");
        return false;
      }
    }
    ...
    switch (player.getRights()) {
      case PLAYER:
        ...
        if (!World.isGlobalYell()) {
          player.getPacketSender()
              .sendMessage("An admin has temporarily disabled the global yell channel.");
          return false;
        }
        switch (player.getDonorRights()) {
          ...
          case 5: // Uber Donator
            if (!player.getYellTag().equals("invalid_yell_tag_set")) {
              title =
                  "<img=11> <col=0>[<col=ffff00><shad=0>" + player.getYellTag() + "<col=0></shad>]";
            } else {
    ...
    if (timer > 0) {
      if (!player.getYellTimer().elapsed(timer * 1000)) {
        player.getPacketSender().sendMessage("Do not flood the yell channel. You must wait another "
            + Misc.getTimeLeft(0, timer, TimeUnit.SECONDS) + " seconds before yelling again.");
        return false;
      }
