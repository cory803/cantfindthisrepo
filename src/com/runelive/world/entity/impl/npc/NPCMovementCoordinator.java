...
public class NPCMovementCoordinator {
	...
	public void sequence() {
		...
		if (coordinateState == CoordinateState.AWAY) {
			if (npc.getPosition().getDistance(npc.getDefaultPosition()) < 2) {
				npc.getMovementCoordinator().setCoordinateState(CoordinateState.HOME);
			}
		}
		...
		case AWAY:
			...
			if (npc.getCombatBuilder().isAttacking()) {
				if (npc.getDefinition().doesRetreat()) {
					...
					PathFinder.findPath(npc, npc.getDefaultPosition().getX(), npc.getDefaultPosition().getY(), true, 1,
							1);
				} else {
					return;
				}
