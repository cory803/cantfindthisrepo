...
import com.runelive.net.packet.PacketBuilder.AccessType;
import com.runelive.net.packet.ValueType;
...
public class PlayerUpdating {
    ...
    public static void update(final Player player) {
        ...
        for (Iterator<Player> playerIterator = player.getLocalPlayers().iterator(); playerIterator
                .hasNext(); ) {
     ...
     * @return The PlayerUpdating instance.
     */
    private static void appendUpdates(Player player, PacketBuilder builder, Player target,
                                      boolean updateAppearance, boolean noChat) {
        if (!target.getUpdateFlag().isUpdateRequired() && !updateAppearance)
            return;
    /*
     * if (player.getCachedUpdateBlock() != null && !player.equals(target) && !updateAppearance &&
     * !noChat) { builder.putBytes(player.getCachedUpdateBlock()); return; }
     */
        // synchronized (target) {
    ...
    private static void updateChat(PacketBuilder builder, Player target) {
        ...
        builder.putShort(((message.getColour() & 0xff) << 8) | (message.getEffects() & 0xff),
                ByteOrder.LITTLE);
        builder.put(rankId);
    ...
    private static void updateForcedMovement(Player player, PacketBuilder builder, Player target) {
        ...
        builder.put((position.getLocalX(myPosition)
                + target.getForceMovement()[MovementQueue.FIRST_MOVEMENT_X]), ValueType.C);
        builder.put((position.getLocalY(myPosition)
                + target.getForceMovement()[MovementQueue.FIRST_MOVEMENT_Y]), ValueType.S);
        builder.put((position.getLocalX(myPosition)
                + target.getForceMovement()[MovementQueue.SECOND_MOVEMENT_X]), ValueType.S);
        builder.put((position.getLocalX(myPosition)
                + target.getForceMovement()[MovementQueue.SECOND_MOVEMENT_Y]), ValueType.C);
    ...
    private static void updateGraphics(PacketBuilder builder, Player target) {
        ...
        builder.putInt(((target.getGraphic().getHeight().ordinal() * 50) << 16)
                + (target.getGraphic().getDelay() & 0xffff));
    }
    ...
    /**
     * This update block is used to update a player's appearance, this includes their equipment,
     * clothing, combat level, gender, head icons, user name and animations.
     *
     * @param builder The packet builder to write information on.
    ...
    private static void updateAppearance(Player player, PacketBuilder out, Player target) {
        ...
        properties
                .put(target.getLocation() == Location.WILDERNESS ? appearance.getBountyHunterSkull() : -1);
        ...
        if (player.getNpcTransformationId() <= 0) {
            ...
            if (appearance.getLook()[Appearance.BEARD] <= 0
                    || appearance.getGender().equals(Gender.FEMALE)) {
                properties.put(0);
            } else {
    ...
    /**
     * Resets the associated player's flags that will need to be removed upon completion of a single
     * update.
     *
     * @return The PlayerUpdating instance.
     */
