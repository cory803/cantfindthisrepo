...
/**
 * Represents a player's npc updating task, which loops through all local npcs and updates their
 * masks according to their current attributes.
 * 
 * @author Relex lawl
 */
...
public class NPCUpdating {
  ...
  public static void updateFacing(Player player, NPC npc) {
    if (npc.getMovementCoordinator().getCoordinator().isCoordinate()
        || npc.getCombatBuilder().isBeingAttacked() || npc.getMovementQueue().isMoving())
      return;
  ...
  private static void appendUpdates(NPC npc, PacketBuilder block) {
    ...
    if (flag.flagged(Flag.ENTITY_INTERACTION)) {
      ...
      block.putShort(
          entity == null ? -1 : entity.getIndex() + (entity instanceof Player ? 32768 : 0));
    }
    if (flag.flagged(Flag.FORCED_CHAT) && npc.getForcedChat().length() > 0) {
  ...
  private static void updateGraphics(PacketBuilder builder, NPC npc) {
    ...
    builder.putInt(((npc.getGraphic().getHeight().ordinal() * 50) << 16)
        + (npc.getGraphic().getDelay() & 0xffff));
  }
