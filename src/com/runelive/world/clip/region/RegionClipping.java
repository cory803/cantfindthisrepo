...
public final class RegionClipping {
	...
	private static Map<Integer, RegionClipping> regions;
	...
	public RegionClipping(int id, int groundFile, int objectFile) {
		this.id = id;
	...
	public static void loadRegions(Archive archive) throws Exception {
		...
		for (int i = 0; i < size; i++) {
			regions.put(regionIds[i], new RegionClipping(regionIds[i], mapGroundFileIds[i], mapObjectsFileIds[i]));
		}
	}
	...
	public static boolean objectExists(GameObject object) {
		...
		if (well || mageBankLever || lawAltar || trees || wellOfGoodwill || chaosTunnels || lunar || barrows || rfd
				|| lumbridgeCastle || barbCourseRopeswing || catherbyAquariums || freeForAllPortal || warriorsGuild
				|| fightPit || godwars || barrows || waterRcAltar || crystalChest || shreksDoor)
			return true;
