...
public class MoneyPouch {
  ...
  public static boolean depositMoney(Player plr, int amount) {
    ...
    if (validateAmount(plr, amount)) {
      ...
      plr.getPacketSender().sendMessage("You do not seem to have "
          + currencyFormatter.format(amount) + " coins in your inventory.");
      return false;
    }
  }
  ...
  public static boolean withdrawMoney(Player plr, long amount) {
    ...
    if (plr.getLocation() == Location.DUNGEONEERING) {
      plr.getPacketSender().sendMessage(
          "You cannot withdraw from your pouch in dungeoneering. You will lose this money.");
      return false;
    }
    ...
    if ((plr.getInventory().getAmount(995) + amount) < Integer.MAX_VALUE) {
      ...
      plr.getPacketSender().sendString(8135, "" + plr.getMoneyInPouch());
      plr.getPacketSender().sendMessage(
          "You've withdrawn " + currencyFormatter.format(amount) + " coins from your pouch.");
      if (allowWithdraw)
      ...
      plr.getPacketSender().sendString(8135, "" + plr.getMoneyInPouch());
      plr.getPacketSender().sendMessage(
          "You could only withdraw " + currencyFormatter.format(canWithdraw) + " coins.");
      if (allowWithdraw)
        plr.getPacketSender().sendItemContainer(plr.getInventory(), 3322);
      return true;
    }
  ...
  public static void toBank(Player player) {
    ...
    if (totalAmount > Integer.MAX_VALUE || totalAmount < 0) {
      int canWithdraw =
          (Integer.MAX_VALUE - player.getBank(Bank.getTabForItem(player, 995)).getAmount(995));
      if (canWithdraw <= 0) {
