...
public class RunecraftingPouches {
  ...
  public static void fill(Player p, RunecraftingPouch pouch) {
    if (p.getInterfaceId() > 0) {
      p.getPacketSender()
              .sendMessage(
                      "Please close the interface you have open before doing this.");
      return;
    }
    ...
    if (rEss == 0 && pEss == 0) {
      p.getPacketSender().sendMessage(
              "You do not have any essence in your inventory.");
      return;
    }
    ...
    if (p.getStoredRuneEssence() >= pouch.maxRuneEss)
      p.getPacketSender().sendMessage(
              "Your pouch can not hold any more Rune essence.");
    if (p.getStoredPureEssence() >= pouch.maxPureEss)
      p.getPacketSender().sendMessage(
              "Your pouch can not hold any more Pure essence.");
    while (rEss > 0 && p.getStoredRuneEssence() < pouch.maxRuneEss
            && p.getInventory().contains(RUNE_ESS)) {
      p.getInventory().delete(RUNE_ESS, 1);
      p.setStoredRuneEssence(p.getStoredRuneEssence() + 1);
      stored++;
    }
    while (pEss > 0 && p.getStoredPureEssence() < pouch.maxPureEss
            && p.getInventory().contains(PURE_ESS)) {
    ...
    if (stored > 0)
      p.getPacketSender().sendMessage(
              "You fill your pouch with " + stored + " essence..");
  }
  ...
  public static void empty(Player p, RunecraftingPouch pouch) {
    ...
    while (p.getStoredRuneEssence() > 0
            && p.getInventory().getFreeSlots() > 0) {
      p.getInventory().add(RUNE_ESS, 1);
      p.setStoredRuneEssence(p.getStoredRuneEssence() - 1);
    }
    while (p.getStoredPureEssence() > 0
            && p.getInventory().getFreeSlots() > 0) {
  ...
  public static void check(Player p, RunecraftingPouch pouch) {
    p.getPacketSender().sendMessage(
            "Your pouch currently contains " + p.getStoredRuneEssence()
                    + "/" + pouch.maxRuneEss + " Rune essence and "
                    + p.getStoredPureEssence() + "/" + pouch.maxPureEss
                    + " Pure essence.");
  }
}
