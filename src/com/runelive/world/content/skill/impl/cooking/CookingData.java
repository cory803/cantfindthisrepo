...
public enum CookingData {
  ...
  CookingData(int rawItem, int cookedItem, int burntItem, int levelReq, int xp, int stopBurn,
      String name) {
  ...
  public static final int[] cookingRanges = {2732, 114, 2728};
  ...
  public static boolean canCook(Player player, int id) {
    ...
    if (player.getSkillManager().getMaxLevel(Skill.COOKING) < fish.getLevelReq()) {
      player.getPacketSender().sendMessage(
          "You need a Cooking level of atleast " + fish.getLevelReq() + " to cook this.");
      return false;
    }
