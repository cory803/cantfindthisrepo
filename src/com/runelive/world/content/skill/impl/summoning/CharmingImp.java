...
public class CharmingImp {
  ...
  public static void changeConfig(Player player, int index, int config) {
    /*
     * player.getSummoning().setCharmImpConfig(index, config); player.getPacketSender()
     * .sendInterfaceRemoval() .sendMessage( "<img=5> <col=996633>Your configuration for " +
  ...
  private static void sendMessage(Player player, int config, int itemId, int amount) {
    ...
    switch (config) {
      case 0:
        // player.getPacketSender().sendMessage("Your charming imp loots your charms and grants you
        // some experience too!");
        // player.getPacketSender().sendMessage("Your Charming imp has found <col=ff0000>" + amount+
  ...
  public static void sendConfig(Player player) {
    for (int i = 0; i < 4; i++) {
      ...
      switch (state) {
        case 0:
          player.getPacketSender()
              .sendMessage("<img=5> <col=996633>Your Charming imp is placing all "
                  + ItemDefinition.forId(charm).getName() + "s it finds in your inventory.");
          break;
        case 1:
          player.getPacketSender()
              .sendMessage("<img=5> <col=996633>Your Charming imp is turning all "
