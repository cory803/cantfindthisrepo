...
public class Farming {
  ...
  public void sequence() {
    ...
    for (int i = 0; i < patches.length; i++) {
      if (i >= FarmingPatches.values().length)
        break;
      if ((patches[i] != null)
          && (!inhabited(FarmingPatches.values()[i].x, FarmingPatches.values()[i].y))) {
  ...
  public void doConfig() {
    for (int i = 0; i < FarmingPatches.values().length; i++) {
      int value = getConfigFor(FarmingPatches.values()[i].config),
          config = FarmingPatches.values()[i].config;
        ...
        for (Plants plant : Plants.values()) {
          if (plant.seed == seed) {
            if (player.getSkillManager().getCurrentLevel(Skill.FARMING) >= plant.level) {
              ...
              if (player.getInventory().contains(patch.planter)) {
  ...
  public void save() {
    ...
    try {
      BufferedWriter writer =
          new BufferedWriter(new FileWriter(DIR + "" + player.getUsername() + ".txt"));
      for (int i = 0; i < patches.length; i++) {
        if (i >= FarmingPatches.values().length)
          break;
        if (patches[i] != null) {
  ...
  public void load() {
    ...
    try {
      BufferedReader r =
          new BufferedReader(new FileReader(DIR + "" + player.getUsername() + ".txt"));
      int stage = -1, patch = -1, plant = -1, watered = -1, minute = -1, hour = -1, day = -1,
          year = -1, harvested = -1;
      while (true) {
