  ...
  public static void openSelection(final Player player, int log) {
    ...
    if (log == 1511) {
      ...
      player.getPacketSender().sendString(8889,
          "" + ItemDefinition.forId(shortBow.getBowID()).getName() + "");
      player.getPacketSender().sendString(8893,
          "" + ItemDefinition.forId(longBow.getBowID()).getName() + "");
  ...
  public static void fletchBow(final Player player, final int product, final int amountToMake) {
    ...
    player.setCurrentTask(new Task(2, player, true) {
      ...
      public void execute() {
        ...
        if (bow != null
            && player.getSkillManager().getCurrentLevel(Skill.FLETCHING) < bow.getLevelReq()) {
    ...
    for (final StringingData g : StringingData.values()) {
      if (log == g.unStrung()) {
        if (player.getSkillManager().getCurrentLevel(Skill.FLETCHING) < g.getLevel()) {
          player.getPacketSender().sendMessage(
              "You need a Fletching level of at least " + g.getLevel() + " to make this.");
          return;
        }
      ...
      if (player.getSkillManager().getCurrentLevel(Skill.FLETCHING) >= arr.getLevelReq()) {
        if (player.getInventory().getAmount(arr.getItem1()) >= 15
          ...
          player.getPacketSender()
              .sendMessage("You must have at least 15 of each supply to make arrows.");
        }
      } else {
        player.getPacketSender().sendMessage(
