...
public class TrapExecution {
  ...
  public static int successFormula(Trap trap, NPC npc) {
    ...
    if (Hunter.hasLarupia(trap.getOwner()))
      chance = chance + 10;
    chance =
        chance + (int) (trap.getOwner().getSkillManager().getCurrentLevel(Skill.HUNTER) / 1.5) + 10;
   ...
   * 
   * @param trap is the given trap we are managing
    ...
    if (trap.getTicks() <= 0) {
      ...
      if (trap.getOwner() != null)
        trap.getOwner().getPacketSender()
            .sendMessage("You left your trap for too long, and it collapsed.");
    }
    return true;
  }
}
