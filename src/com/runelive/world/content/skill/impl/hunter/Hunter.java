...
import com.runelive.engine.task.impl.HunterTrapsTask;
...
public class Hunter {
    ...
    if (client.getTrapsLaid() > 0) {
      for (final Trap trap : traps) {
        ...
        if (trap.getOwner() != null && trap.getOwner().getUsername().equals(client.getUsername())
            && !Locations.goodDistance(trap.getGameObject().getPosition(), client.getPosition(),
                50)) {
          deregister(trap);
          client.getPacketSender()
  ...
  public static void layTrap(Player client, Trap trap) {
    ...
    if (trap instanceof BoxTrap) {
      ...
      if (client.getSkillManager().getCurrentLevel(Skill.HUNTER) < 60) {
        client.getPacketSender()
            .sendMessage("You need a Hunter level of at least 60 to lay this trap.");
        return;
      }
    }
          ...
          if (theTrap.getOwner() == client) {
            if (theTrap instanceof SnareTrap) {
              ...
              if (theTrap.getGameObject().getId() == 19180) {
                ...
                client.getInventory().add(9978, 1);
                client.getPacketSender()
                    .sendMessage("You've succesfully caught a Copper Longtail.");
                ...
                client.getInventory().add(9978, 1);
                client.getPacketSender()
                    .sendMessage("You've succesfully caught a Cerulean Twitch.");
                ...
                client.getInventory().add(9978, 1);
                client.getPacketSender()
                    .sendMessage("You've succesfully caught a Tropical Wagtail.");
  ...
  public static void catchNPC(Trap trap, NPC npc) {
    if (trap.getTrapState().equals(TrapState.CAUGHT))
      return;
    if (trap.getOwner() != null) {
      if (trap.getOwner().getSkillManager()
          .getCurrentLevel(Skill.HUNTER) < requiredLevel(npc.getId())) {
        ...
        trap.getOwner().getPacketSender().sendMessage(
            "You need atleast " + requiredLevel(npc.getId()) + " Hunter to catch this animal");
        return;
      }
      deregister(trap);
      if (trap instanceof SnareTrap)
