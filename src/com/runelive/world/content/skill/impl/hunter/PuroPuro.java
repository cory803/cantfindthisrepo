...
public class PuroPuro {
  ...
  public static final int[][] implings = {
      ...
      {7903, 3348, 9623}, {7903, 3372, 9639}, {7903, 3350, 9655}
  ...
  public static void catchImpling(Player player, final NPC imp) {
    ...
    if (player.getInterfaceId() > 0 || player == null || imp == null || implingData == null
        || !imp.isRegistered() || !player.getClickDelay().elapsed(1000))
      return;
    if (player.getSkillManager().getCurrentLevel(Skill.HUNTER) < implingData.levelReq) {
      player.getPacketSender().sendMessage("You need a Hunter level of at least "
          + implingData.levelReq + " to catch this impling.");
      return;
    }
  ...
  public static void lootJar(final Player player, Item jar, JarData jarData) {
    ...
    if (player.getInventory().getFreeSlots() < 2) {
      player.getPacketSender()
          .sendMessage("You need at least 2 free inventory space to loot this.");
      return;
    }
    ...
    player.getPacketSender().sendMessage("You loot the " + jar.getDefinition().getName()
        + " and find " + s + " " + rewardName + ".");
    player.getClickDelay().reset();
  }
