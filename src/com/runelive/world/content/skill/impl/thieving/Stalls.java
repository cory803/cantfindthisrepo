...
public class Stalls {
	...
	public static void stealFromStall(Player player, int lvlreq, int xp, int reward, String message) {
		...
		int amount = 1;
		if (Skillcape_Data.THIEVING.isWearingCape(player)
				|| Skillcape_Data.MASTER_THIEVING.isWearingCape(player) && Misc.inclusiveRandom(0, 10) == 0) {
	...
	public static void stealFromStall2(Player player, int lvlreq, int xp, int reward, int goldAmount, String message) {
		if (player.getInventory().getFreeSlots() < 1) {
			player.getPacketSender().sendMessage("You need some more inventory space to do this.");
			return;
		}
		...
		if (reward == 15009)
			...
			Achievements.doProgress(player, AchievementData.STEAL_5000_SCIMITARS, amount);
		}
	}
