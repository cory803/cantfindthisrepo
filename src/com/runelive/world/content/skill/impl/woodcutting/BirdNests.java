...
public class BirdNests {
  ...
  /**
   * Generates the random drop and creates a ground item where the player is standing
   */
  ...
  public static void dropNest(Player p) {
    ...
    if (Misc.getRandom((Skillcape_Data.WOODCUTTING.isWearingCape(p) || Skillcape_Data.MASTER_WOODCUTTING.isWearingCape(p) ? 40 : 60)) == 1) {
      Item nest = null;
      ...
      if (nest != null) {
        nest.setAmount(1);
        GroundItemManager.spawnGroundItem(p, new GroundItem(nest, p
                .getPosition().copy(), p.getUsername(), false, 80,
                true, 80));
