...
public class Woodcutting {
  ...
  public static boolean infernoAdze(Player player) {
    ...
    if (player.getEquipment().get(Equipment.WEAPON_SLOT).getId() == 13661) {
      contains = true;
    } else if (player.getInventory().contains(13661)) {
      contains = true;
    }
    if (player.getSkillManager().getCurrentLevel(Skill.WOODCUTTING) < 80) {
      contains = false;
    }
  ...
  public static void treeRespawn(final Player player, final GameObject oldTree) {
    ...
    player.getPacketSender().sendClientRightClickRemoval();
    player.getSkillManager().stopSkilling();
    CustomObjects.globalObjectRespawnTask(new GameObject(1343, oldTree.getPosition().copy(), 10, 0),
        oldTree, 20 + Misc.getRandom(10));
  }
