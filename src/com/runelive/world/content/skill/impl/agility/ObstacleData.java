...
public enum ObstacleData {
      ...
      TaskManager.submit(new Task(1, player, false) {
        ...
        public void stop() {
          ...
          player.getPacketSender()
              .sendMessage("You manage to safely make your way across the log.");
        }
      });
    }
  },
  ...
  NETS_2(2286, false) {
    ...
    public void cross(final Player player) {
      ...
      TaskManager.submit(new Task(2, player, false) {
        ...
        public void execute() {
          player.moveTo(
              new Position(player.getPosition().getX(), player.getPosition().getY() + 2, 0));
        ...
        public void stop() {
          ...
          if (Agility.passedAllObstacles(player)) {
            DialogueManager.start(player,
                DialogueManager.getDialogues().get(57 + Misc.getRandom(2)));
  ...
  RAMP(1948, false) {
    ...
    public void cross(final Player player) {
      ...
      final boolean oneStep =
          player.getPosition().getX() == 2537 || player.getPosition().getX() == 2542;
  ...
  GATE_1(2309, true) {
    ...
    public void cross(final Player player) {
      ...
      player.getUpdateFlag().flag(Flag.APPEARANCE);
      player.getPacketSender()
          .sendMessage("You enter the gate and begin walking across the narrow path..");
      TaskManager.submit(new Task(1, player, true) {
        int tick = 0;
  ...
  ROPE_SWING_2(2283, true) {
    ...
    public void cross(final Player player) {
      if (player.getPosition().getY() > 3953) {
        player.getPacketSender()
            .sendMessage("You must be positioned infront of the Ropeswing to do that.");
        player.setCrossingObstacle(false);
        return;
      }
      player.getPacketSender().sendMessage("You attempt to swing on the ropeswing..");
      ...
      TaskManager.submit(new Task(1, player, true) {
        ...
        public void execute() {
          ...
          if (tick == 4 || tick == 7 || tick == 10 || tick == 13 || tick == 16) {
            player
                .moveTo(new Position(player.getPosition().getX() - 1, player.getPosition().getY()));
          } else if (tick >= 17) {
  ...
  BALANCE_LEDGE_2(2297, true) {
    ...
    public void cross(final Player player) {
      ...
      TaskManager.submit(new Task(1, player, true) {
        ...
        public void stop() {
          setEventRunning(false);
          player.setCrossedObstacle(5, fail ? false : true).setCrossingObstacle(false)
              .setSkillAnimation(-1);
