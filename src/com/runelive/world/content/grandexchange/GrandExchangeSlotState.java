...
public enum GrandExchangeSlotState {
  ...
  PENDING_PURCHASE {
    ...
    public void update(Player p, int slot, int geData, int percent, int item) {
      ...
      p.getPacketSender().sendGrandExchangeUpdate("slotselected item #" + item + "# slotbuy <"
          + slot + "> [" + geData + "] slotpercent {" + percent + "}");
    }
  },
  FINISHED_PURCHASE {
    @Override
  ...
  PENDING_SALE {
    ...
    public void update(Player p, int slot, int geData, int percent, int item) {
      ...
      p.getPacketSender().sendGrandExchangeUpdate("slotselected item #" + item + "# slotsell <"
          + slot + "> [" + geData + "] slotpercent {" + percent + "}");
    }
  },
  FINISHED_SALE {
