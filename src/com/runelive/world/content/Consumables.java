...
import com.runelive.engine.task.impl.PoisonImmunityTask;
...
/**
 * Consumables are items that players can use to restore stats/points. Examples of Consumable items:
 * Food, Potions
 * 
...
public class Consumables {
  ...
  public static void handleHealAction(Player player) {
    ...
    player.getPacketSender()
        .sendMessage("You do not have any items that can heal you in your inventory.");
  }
  ...
  private enum FoodType {
     ...
     */
    KEBAB(new Item(1971), 40), CHEESE(new Item(1985), 45), CAKE(new Item(1891),
        50), SECOND_CAKE_SLICE(new Item(1893), 50), THIRD_CAKE_SLICE(new Item(1895), 50), BANDAGES(
            new Item(14640), 120), JANGERBERRIES(new Item(247), 20), WORM_CRUNCHIES(new Item(2205),
                70), EDIBLE_SEAWEED(new Item(403), 40), ANCHOVIES(new Item(319), 10), SHRIMPS(
                    new Item(315),
                    30), SARDINE(new Item(325), 40), COD(new Item(339), 70), TROUT(new Item(333),
                        70), PIKE(new Item(351), 80), SALMON(new Item(329), 90), TUNA(new Item(361),
                            100), LOBSTER(new Item(379), 120), BASS(new Item(365), 130), SWORDFISH(
                                new Item(373),
                                140), MEAT_PIZZA(new Item(2293), 145), MONKFISH(new Item(7946),
                                    160), SHARK(new Item(385), 200), SEA_TURTLE(new Item(397),
                                        210), MANTA_RAY(new Item(391), 220), CAVEFISH(
                                            new Item(15266), 230), ROCKTAIL(new Item(15272), 230),
    /*
    ...
    POTATO(new Item(1942), 10), BAKED_POTATO(new Item(6701), 40), POTATO_WITH_BUTTER(new Item(6703),
        140), CHILLI_POTATO(new Item(7054), 140), EGG_POTATO(new Item(7056),
            160), POTATO_WITH_CHEESE(new Item(6705),
                160), MUSHROOM_POTATO(new Item(7058), 200), TUNA_POTATO(new Item(7060), 220),

    /*
     * Fruit food types which a player can get by picking from certain trees or hand-making them
     * (such as pineapple chunks/rings).
     */
    SPINACH_ROLL(new Item(1969), 20), BANANA(new Item(1963), 20), BANANA_(new Item(18199),
        20), CABBAGE(new Item(1965), 20), ORANGE(new Item(2108), 20), PINEAPPLE_CHUNKS(
            new Item(2116), 20), PINEAPPLE_RINGS(new Item(2118), 20), PEACH(new Item(6883), 80),

    /*
     * Dungeoneering food types, which you can get in the Dungeoneering skill dungeons.
     */
    HEIM_CRAB(new Item(18159), 20), RED_EYE(new Item(18161), 50), DUSK_EEL(new Item(18163),
        70), GIANT_FLATFISH(new Item(18165), 100), SHORT_FINNED_EEL(new Item(18167),
            120), WEB_SNIPPER(new Item(18169), 150), BOULDABASS(new Item(18171),
                170), SALVE_EEL(new Item(18173), 200), BLUE_CRAB(new Item(18175), 220),
  ...
  public static void handlePotion(final Player player, final int itemId, final int slot) {
    ...
    if (Dueling.checkRule(player, DuelRule.NO_FOOD) && healingPotion(itemId)) {
      player.getPacketSender().sendMessage(
          "Since food has been disabled in this duel, health-healing potions won't work.");
      return;
    }
    ...
    // player.getPacketSender().sendMessage("Since food has been disabled in this clan battle,
    ...
    if (player.getPotionTimer().elapsed(900)) {
      ...
      switch (itemId) {
        ...
        case 2446:
          ...
          player.getPacketSender()
              .sendMessage("You're now immune to any kind of poison for another 86 seconds.");
          break;
        case 175:
          player.performAnimation(new Animation(829));
        ...
        case 14385:
          ...
          if (!player.getSpecialRestoreTimer().elapsed(30000)) {
            player.getPacketSender()
                .sendMessage("This potion can only be used once every 30 seconds.");
            return;
          }
          player.performAnimation(new Animation(829));
        ...
        case 14325:
          ...
          player.getSkillManager().setCurrentLevel(Skill.forId(4),
              player.getSkillManager().getCurrentLevel(Skill.forId(4))
                  + getExtremePotionBoost(player, 4));
          break;
        case 14323:
          if (player.getLocation() != null && player.getLocation() == Location.WILDERNESS) {
            player.getPacketSender().sendMessage("You cannot use this potion in the Wilderness.");
        ...
        case 14337:
          ...
          player.getSkillManager().setCurrentLevel(Skill.forId(6),
              player.getSkillManager().getCurrentLevel(Skill.forId(6))
                  + getExtremePotionBoost(player, 6));
          break;
        case 14335:
          if (player.getLocation() != null && player.getLocation() == Location.WILDERNESS) {
            player.getPacketSender().sendMessage("You cannot use this potion in the Wilderness.");
        ...
        case 14349:
          ...
          player.getSkillManager().setCurrentLevel(Skill.forId(1),
              player.getSkillManager().getCurrentLevel(Skill.forId(1))
                  + getExtremePotionBoost(player, 1));
          break;
        case 14347:
          if (player.getLocation() != null && player.getLocation() == Location.WILDERNESS) {
            player.getPacketSender().sendMessage("You cannot use this potion in the Wilderness.");
        ...
        case 14361:
          ...
          player.getSkillManager().setCurrentLevel(Skill.forId(2),
              player.getSkillManager().getCurrentLevel(Skill.forId(2))
                  + getExtremePotionBoost(player, 2));
          break;
        case 14359:
          if (player.getLocation() != null && player.getLocation() == Location.WILDERNESS) {
            player.getPacketSender().sendMessage("You cannot use this potion in the Wilderness.");
        ...
        case 14373:
          ...
          player.getSkillManager().setCurrentLevel(Skill.forId(0),
              player.getSkillManager().getCurrentLevel(Skill.forId(0))
                  + getExtremePotionBoost(player, 0));
          break;
        case 14371:
          if (player.getLocation() != null && player.getLocation() == Location.WILDERNESS) {
            player.getPacketSender().sendMessage("You cannot use this potion in the Wilderness.");
        ...
        case 14427: // Sara Brew Flask 6
          ...
          for (int tD : decrease) {
            player.getSkillManager().setCurrentLevel(Skill.forId(tD),
                player.getSkillManager().getCurrentLevel(Skill.forId(tD))
                    - getBrewStat(player, tD, .10));
          ...
          if (player.getSkillManager().getCurrentLevel(Skill
              .forId(3)) > (player.getSkillManager().getMaxLevel(Skill.forId(3)) * amount + 1)) {
            player.getSkillManager().setCurrentLevel(Skill.forId(3),
        ...
        case 2450:
          ...
          player.getSkillManager().setCurrentLevel(Skill.forId(5),
              player.getSkillManager().getCurrentLevel(Skill.forId(5))
                  + getBrewStat(player, 5, .10));
          if (player.getSkillManager().getCurrentLevel(
        ...
        case 21630:
          if (player.getPrayerRenewalPotionTimer() > 0) {
            player.getPacketSender()
                .sendMessage("You already have the effect of a Prayer Renewal potion.");
            return;
          }
          player.performAnimation(new Animation(829));
  ...
  public static void drinkStatPotion(final Player player, int potion, int replacePotion, int slot,
      int skill, boolean super_pot) {
    if (slot >= 0) {
      player.performAnimation(new Animation(829));
  ...
  public static boolean drinkOverload(final Player player, int slot, int replacePotion) {
    ...
    if (player.getSkillManager().getCurrentLevel(Skill.CONSTITUTION) < 500) {
      player.getPacketSender()
          .sendMessage("You need to have at least 500 Hitpoints to drink this potion.");
      return false;
    }
    player.performAnimation(new Animation(829));
    player.getInventory().getItems()[slot] = new Item(replacePotion, 1);
  ...
  public static void overloadIncrease(final Player player, Skill skill, double l) {
    player.getSkillManager().setCurrentLevel(skill, (int) (SkillManager
        .getLevelForExperience(player.getSkillManager().getExperience(skill))
  ...
  public static int getExtremePotionBoost(Player player, int skill) {
    ...
    increaseBy = (int) (SkillManager
        .getLevelForExperience(player.getSkillManager().getExperience(Skill.forId(skill))) * .25)
        + 1;
    if (player.getSkillManager().getCurrentLevel(Skill.forId(skill)) + increaseBy > SkillManager
        .getLevelForExperience(player.getSkillManager().getExperience(Skill.forId(skill)))
        + increaseBy + 1) {
      return SkillManager
          .getLevelForExperience(player.getSkillManager().getExperience(Skill.forId(skill)))
  ...
  public static int getPotionBoost(Player player, int skill) {
    int increaseBy = 0;
    increaseBy = (int) (SkillManager
        .getLevelForExperience(player.getSkillManager().getExperience(Skill.forId(skill))) * .18)
        + 1;
    if (player.getSkillManager().getCurrentLevel(Skill.forId(skill)) + increaseBy > SkillManager
        .getLevelForExperience(player.getSkillManager().getExperience(Skill.forId(skill)))
        + increaseBy + 1) {
