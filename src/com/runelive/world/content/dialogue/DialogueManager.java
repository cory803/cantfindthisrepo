...
public class DialogueManager {
  ...
  public static void start(Player player, Dialogue dialogue) {
    ...
    if (player.isBanking() || player.isShopping()
        || player.getInterfaceId() > 0 && player.getInterfaceId() != 50)
  ...
  /**
   * Configures the dialogue's type and shows the dialogue interface and sets its child id's.
   * 
  ...
  private static void showDialogue(Player player, Dialogue dialogue) {
    ...
    switch (dialogue.type()) {
      case NPC_STATEMENT:
        ...
        player.getPacketSender().sendInterfaceAnimation(headChildId,
            dialogue.animation().getAnimation());
      ...
      case ITEM_STATEMENT:
        ...
        headChildId = startDialogueChildId - 2;
        player.getPacketSender().sendInterfaceModel(headChildId,
            Integer.valueOf(dialogue.item()[0]), Integer.valueOf(dialogue.item()[1]));
  ...
  /**
   * This array contains the child id where the dialogue statement starts for npc and item
   * dialogues.
   */
  private static final int[] NPC_DIALOGUE_ID = {4885, 4890, 4896, 4903};

  /**
   * This array contains the child id where the dialogue statement starts for player dialogues.
   */
  private static final int[] PLAYER_DIALOGUE_ID = {971, 976, 982, 989};

  /**
   * This array contains the child id where the dialogue statement starts for option dialogues.
   */
