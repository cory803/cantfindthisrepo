...
public class PlayerPunishment {
  ...
  public static boolean isPlayerBanned(String username) {
	...
	GameServer.getCharacterPool().executeQuery("SELECT * FROM `punishments` WHERE LOWER (`username`) = LOWER('" + username + "') LIMIT 1", new SQLCallback() {
		...
		private SQLCallback init(boolean var){
			continuation = var;
			return this;
		}
	}.init(continuation));
