...
public class Lottery {
   ...
   */
  private static final File LAST_WINNER_FILE_LOCATION =
      new File("./data/saves/lottery/lotterywin.txt");
   ...
   */
  private static final int PRICE_TO_ENTER = 10_000_000;
  ...
  public static final int getPot() {
    ...
    return (CONTESTERS.size() * (PRICE_TO_ENTER - 2_500_000));
  }
  ...
  public static void enterLottery(Player p) {
    ...
    if (p.getInventory().getAmount(995) < PRICE_TO_ENTER && !usePouch
        || p.getRights() == PlayerRights.OWNER) {
      p.getPacketSender().sendInterfaceRemoval().sendMessage("")
          .sendMessage(
              "You do not have enough money in your inventory to enter this week's lottery.")
    ...
    p.getAchievementAttributes()
        .setCoinsGambled(p.getAchievementAttributes().getCoinsGambled() + PRICE_TO_ENTER);
  ...
  /**
   * Adds a user to the lottery by writing their username to the file aswell as adding them to the
   * list of users who have entered already.
   * 
  ...
  public static void init() {
    try {
      ...
      while (true) {
        ...
        if (line.length() > 0) {
          if (!CONTESTERS.contains(line)) // user might have gotten on list twice somehow.. don't
                                          // give them extra chance of winning
            CONTESTERS.add(line);
        }
      }
      r.close();
  ...
  public static void restartLottery() {
    ...
    try {
      ...
      if (winner != null) {
        ...
        writer.close();
        World.sendMessage(
            "<col=D9D919><shad=0>This week's lottery winner is " + winner + "! Congratulations!");
      } else
        World.sendMessage(
  ...
  public static void rewardPlayer(Player player, boolean ignore) throws IOException {
    if ((!LAST_WINNER_REWARDED || ignore) && LAST_WINNER.equalsIgnoreCase(player.getUsername())) {
      ...
      player.getPacketSender()
          .sendMessage("You've won the lottery for this week! Congratulations!");
  ...
  /**
   * Handles the lottery for a player on login Checks if a user won the lottery without being
   * rewarded.
   * 
