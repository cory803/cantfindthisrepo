package com.runelive.world.content.clan;
...
public class ClanChatManager {
  ...
  /**
   * Loads all the files located in the directory {@link #FILE_DIRECTORY} and populates the
   * {@link #clans} array.
   */
  public static void init() {
    try {
      ...
      if (!directory.exists()) {
        Logger.getLogger(ClanChatManager.class.getName())
      ...
      System.out.println("Loaded " + amount + " clan chat channel" + (amount != 1 ? "s" : "")
          + " in " + (System.currentTimeMillis() - startup) + "ms");
  ...
  public static ClanChat create(Player player) {
    ...
    if (index == -1) { // Too many clans
      player.getPacketSender()
          .sendMessage("An error occured! Please contact an administrator and report this.");
      return null;
    }
        ...
        if (rank != null) {
          if (rank == ClanChatRank.OWNER || rank == ClanChatRank.STAFF) {
            member.getPacketSender().sendClanChatListOptionsVisible(2); // Kick/demote/promote
                                                                        // options
          } else if (clan.getRankRequirement()[ClanChat.RANK_REQUIRED_TO_KICK] != null && rank
              .ordinal() >= clan.getRankRequirement()[ClanChat.RANK_REQUIRED_TO_KICK].ordinal()) {
  ...
  public static void sendMessage(Player player, String message) {
    ...
    if (clan.getRankRequirement()[ClanChat.RANK_REQUIRED_TO_TALK] != null) {
      if (rank == null
        ...
        player.getPacketSender()
            .sendMessage("You do not have the required rank to speak in this channel.");
        return;
      }
    }
    ...
    for (Player memberPlayer : clan.getMembers()) {
      if (memberPlayer != null) {
        ...
        memberPlayer.getPacketSender()
            .sendMessage(":clan:" + bracketColor + "[" + clanNameColor + clan.getName()
                + bracketColor + "]" + nameColor + "" + rankImg + ""
  ...
  private static void join(Player player, ClanChat clan) {
    ...
    if (clan.isBanned(player.getUsername())) {
      player.getPacketSender().sendMessage(
          "You're currently banned from using this channel. Bans expire every 20 minutes.");
      return;
    }
    ...
    if (clan.getRankRequirement()[ClanChat.RANK_REQUIRED_TO_ENTER] != null) {
      if (rank == null || clan.getRankRequirement()[ClanChat.RANK_REQUIRED_TO_ENTER]
    ...
    player.getPacketSender().sendString(29450,
        "Owner: " + NameUtils.capitalizeWords(clan.getOwnerName()));
  ...
  public static void setName(Player player, String newName) {
    ...
    if (clan.getLastAction().elapsed(5000) || player.getRights().isStaff()) {
      ...
      player.getPacketSender()
          .sendMessage("You need to wait a few seconds between every clanchat action.");
    }
  }
  ...
  public static void kick(Player player, Player target) {
    ...
    if (rank == null || rank != ClanChatRank.STAFF
      ...
      player.getPacketSender()
          .sendMessage("You do not have the required rank to kick this player.");
      return;
    }
    for (Player member : clan.getMembers()) {
      if (member != null && member.equals(target)) {
        ...
        sendMessage(player.getCurrentClanChat(),
            "<col=16777215>[<col=255>" + clan.getName() + "<col=16777215>]<col=3300CC> "
                + member.getUsername() + " has been kicked from the channel by "
                + player.getUsername() + ".");
        break;
      }
    }
  ...
  public static void handleMemberOption(Player player, int index, int menuId) {
    if ((player.getCurrentClanChat() == null
        || !player.getCurrentClanChat().getOwnerName().equals(player.getUsername()))
        && menuId != 1) {
    ...
    switch (menuId) {
      ...
      case 3:
        ...
        if (player.getCurrentClanChat().getLastAction().elapsed(5000)
          ...
          sendMessage(player.getCurrentClanChat(),
              "<col=16777215>[<col=255>" + player.getCurrentClanChat().getName()
                  ...
                  + " has been given the rank: " + Misc.formatText(rank.name().toLowerCase())
                  + ".");
  ...
  public static boolean dropShareLoot(Player player, NPC npc, Item itemDropped) {
    /*
     * ClanChat clan = player.getFields().getClanChat(); if (clan != null) { boolean received =
     * false; List<Player> players = getPlayersWithinPosition(clan, npc.getPosition()); String green
     * = "<col=" + ClanChatMessageColor.GREEN.getRGB()[player.getFields().rgbIndex] + ">"; if
     * (clan.isItemSharing() && itemDropped.getId() != 995) { Player rewarded = players.size() > 0 ?
     * players.get(MathUtils.random(players.size() - 1)) : null; if (rewarded != null) {
     * rewarded.getPacketSender().sendMessage(green + "You have received " + itemDropped.getAmount()
     * + "x " + itemDropped.getDefinition().getName() + "."); received = true; } } if
     * (clan.isCoinSharing() && itemDropped.getId() == 995) { for (Item drop : npc.getDrops()) { if
     * ((drop.getDefinition().getValue() * drop.getAmount()) < 50000) { GroundItem groundItem = new
     * GroundItem(drop, npc.getPosition().copy()); GameServer.getWorld().register(groundItem,
     * player); continue; } int amount = (int) (ItemDefinition.forId(drop.getId()).getValue() /
     * players.size()); Item split = new Item(995, amount); for (Player member : players) {
     * GroundItem groundItem = new GroundItem(split.copy(), npc.getPosition().copy());
     * GameServer.getWorld().register(groundItem, member);
     * member.getPacketSender().sendMessage(green + "You have received " + amount + "x " +
     * split.getDefinition().getName() + " as part of a split drop."); } } } else
     * if(!clan.isItemSharing() && !clan.isCoinSharing() || !received) return false; } else return
     * false;
     */
    return false;
  }
  ...
  public static void toggleLootShare(Player player) {
    ...
    if (!player.getRights().isStaff()) {
      ...
      if (!clan.getOwner().equals(player)) {
        player.getPacketSender()
            .sendMessage("Only the owner of the channel has the power to do this.");
        return;
      }
    }
    if (clan.getLastAction().elapsed(5000) || player.getRights().isStaff()) {
      ...
      sendMessage(clan,
          "<col=16777215>[<col=255>" + clan.getName() + "<col=16777215>] <col=3300CC>"
              + player.getUsername() + " has " + (clan.getLootShare() ? "enabled" : "disabled")
              + " Lootshare.");
      for (Player member : clan.getMembers()) {
        if (member != null) {
      ...
      switch (id) {
        ...
        case -17536:
          ...
          clan.setRankRequirements(ClanChat.RANK_REQUIRED_TO_ENTER,
              id == -18281 ? null : ClanChatRank.forId(l));
          player.getPacketSender()
              .sendMessage("You have changed your clanchat channel's settings.");
          if (clan.getRankRequirement()[ClanChat.RANK_REQUIRED_TO_ENTER] != null) {
            for (Player member : clan.getMembers()) {
              if (member == null)
                continue;
              ClanChatRank rank = clan.getRank(member);
              if (rank == null || clan.getRankRequirement()[ClanChat.RANK_REQUIRED_TO_ENTER]
                  .ordinal() > rank.ordinal() && rank != ClanChatRank.STAFF) {
        ...
        case -17526:
          ...
          clan.setRankRequirements(ClanChat.RANK_REQUIRED_TO_TALK,
              id == -18278 ? null : ClanChatRank.forId(l));
          player.getPacketSender()
              .sendMessage("You have changed your clanchat channel's settings.");
          clanChatSetupInterface(player, false);
          writeFile(clan);
        ...
        case -17515:
          ...
          clan.setRankRequirements(ClanChat.RANK_REQUIRED_TO_KICK,
              id == -18275 ? null : ClanChatRank.forId(l));
          player.getPacketSender()
              .sendMessage("You have changed your clanchat channel's settings.");
          clanChatSetupInterface(player, false);
          updateList(clan);
  ...
  public static void clanChatSetupInterface(Player player, boolean check) {
    ...
    if (channel.getRankRequirement()[ClanChat.RANK_REQUIRED_TO_ENTER] == null) {
      ...
      player.getPacketSender().sendString(47815,
          Misc.formatText(
              channel.getRankRequirement()[ClanChat.RANK_REQUIRED_TO_ENTER].name().toLowerCase())
              + "+");
    }
    ...
    if (channel.getRankRequirement()[ClanChat.RANK_REQUIRED_TO_TALK] == null) {
      ...
      player.getPacketSender().sendString(47816,
          Misc.formatText(
              channel.getRankRequirement()[ClanChat.RANK_REQUIRED_TO_TALK].name().toLowerCase())
              + "+");
    }
    ...
    if (channel.getRankRequirement()[ClanChat.RANK_REQUIRED_TO_KICK] == null) {
      ...
      player.getPacketSender().sendString(47817,
          Misc.formatText(
              channel.getRankRequirement()[ClanChat.RANK_REQUIRED_TO_KICK].name().toLowerCase())
              + "+");
    }
