...
/**
 * @author: @Gabbe Warning: This crap is so messy and ugly. Will redo it once I get some time over.
 * Should be dupe-free.
 */
...
public class Trading {
  ...
  public void requestTrade(Player player2) {
    if (player == null || player2 == null || player.getConstitution() <= 0
        || player2.getConstitution() <= 0 || player.isTeleporting() || player2.isTeleporting())
      return;
    ...
    if (player2.getGameMode() == GameMode.HARDCORE_IRONMAN) {
      player.getPacketSender()
          .sendMessage("That player is an Ironman player and can therefore not stake.");
      return;
    }
     ...
     * "You must have played for at least 15 minutes in order to trade someone."); return; }
     */
    ...
    if (System.currentTimeMillis() - lastTradeSent < 5000 && !inTrade()) {
      player.getPacketSender()
          .sendMessage("You're sending trade requests too frequently. Please slow down.");
      return;
    }
    ...
    if (player2.busy() || player2.getInterfaceId() > 0 || player2.getTrading().inTrade()
        || player2.isBanking() || player2
            .isShopping()/*
    ...
    if (player.getInterfaceId() > 0 || inTrade() || player.isBanking() || player
        .isShopping()/* || player.getDueling().inDuelScreen || FightPit.inFightPits(player) */) {
    ...
    if (!inTrade() && player2.getTrading().tradeRequested()
      ...
      player.getPacketSender()
          .sendMessage("You've sent a trade request to " + player2.getUsername() + ".");
  ...
  public void openTrade() {
    ...
    if (player == null || player2 == null || getTradeWith() == player.getIndex()
        || player.isBanking())
      return;
    setTrading(true);
  ...
  public void sendText(Player player2) {
    ...
    player2.getPacketSender().sendString(3417,
        "Trading with: " + Misc.formatPlayerName(player.getUsername()) + "");
    player.getPacketSender().sendString(3451,
        "" + Misc.formatPlayerName(player2.getUsername()) + "");
    player.getPacketSender().sendString(3417,
        "Trading with: " + Misc.formatPlayerName(player2.getUsername()) + "");
  ...
  public void tradeItem(int itemId, int amount, int slot) {
    ...
    if (!player.getInventory().contains(itemId))
      return;
    if (slot >= player.getInventory().capacity()
    ...
    if (player.getLocation() != Location.DUNGEONEERING) {
      if (itemToTrade.getDefinition().getName().contains("Primal")) {
    	if(player.isCanWearDungItems()) {
    		player.getPacketSender().sendMessage("You cannot put primal items in a trade... Only you can wear these.");
    		return;
    	}
  ...
  public void acceptTrade(int stage) {
    ...
    if (!twoTraders(player, player2)) {
      player.getPacketSender()
          .sendMessage("An error has occured. Please try re-trading the player.");
      return;
    }
    if (stage == 2) {
      ...
      tradeConfirmed = true;
      if (inTrade() && player2.getTrading().tradeConfirmed && player2.getTrading().goodTrade
          && goodTrade) {
        confirmScreen();
  ...
  public void confirmScreen() {
    ...
    for (Item item : offeredItems) {
      if (item.getAmount() >= 1000 && item.getAmount() < 1000000) {
        SendAmount =
            "@cya@" + (item.getAmount() / 1000) + "K @whi@(" + Misc.format(item.getAmount()) + ")";
    ...
    try {
      for (Item item : player2.getTrading().offeredItems) {
		...
		if(futuristic_value > Integer.MAX_VALUE && item.getDefinition().isStackable() && item.getId() != 995) {
			...
			if(item.getId() == 18016) {
				...
				player.getPacketSender().sendMessage("You did not have enough inventory space, "+Misc.formatAmount((long) item_amount * 25)+" has been added to your pouch.");
			} else {
				player.getBank(0).add(item.getId(), item_amount);
				player.getPacketSender().sendMessage("You did not have enough inventory space, "+Misc.formatAmount((long) item_amount)+" shards has been added to your bank.");
			}
		} else {
			player.getInventory().add(item);
		}
      ...
      for (Item item : player.getTrading().offeredItems) {
        PlayerLogs.log(player.getUsername(),
            "Gave item in trade to " + player2.getUsername() + ". Id: "
                + item.getDefinition().getName() + "(" + item.getId() + "), amount: "
                + item.getAmount());
      }
      for (Item item : player2.getTrading().offeredItems) {
        PlayerLogs.log(player.getUsername(),
            "Receiving item from trade from " + player2.getUsername() + ". Id: "
                + item.getDefinition().getName() + "(" + item.getId() + "), amount: "
                + item.getAmount());
      }
  ...
  public boolean getCanOffer() {
    return canOffer && player.getInterfaceId() == 3323 && !player.isBanking()
  ...
  public static boolean twoTraders(Player p1, Player p2) {
    ...
    for (Player player : World.getPlayers()) {
      if (player == null)
        continue;
      if (player.getTrading().inTradeWith == p1.getIndex()
          || player.getTrading().inTradeWith == p2.getIndex()) {
        count++;
      }
    }
    return count == 2;
