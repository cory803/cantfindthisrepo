...
public class Scoreboards {
  ...
  public static void init() {
    TOP_PKERS = new Scoreboard("RuneLive's Top Pkers", "top-pkers.txt") {
      @Override
    ...
    TOP_KILLSTREAKS = new Scoreboard("RuneLive's Top Killstreaks", "top-killstreaks.txt") {
      @Override
      public void resort(ArrayList<ScoreboardEntry> entries) {
    ...
    TOP_ACHIEVER = new Scoreboard("RuneLive's Top Achiever", "top-achievers.txt") {
      @Override
      public void resort(ArrayList<ScoreboardEntry> entries) {
        Scoreboards.resort(entries, "integer");
      }
    ...
    TOP_TOTAL_EXP = new Scoreboard("RuneLive's Top Total Exp", "top-exp.txt") {
      @Override
      public void resort(ArrayList<ScoreboardEntry> entries) {
    ...
    for (Player p : World.getPlayers()) {
      ...
      if (scoreboard == Scoreboards.TOP_PKERS) {
        ...
        entries.add(new ScoreboardEntry(p.getUsername(),
            new String[] {String.valueOf(kdr), String.valueOf(kc), String.valueOf(dc)}));
      } else if (scoreboard == Scoreboards.TOP_KILLSTREAKS) {
        entries.add(new ScoreboardEntry(p.getUsername(),
            new String[] {String.valueOf(p.getPlayerKillingAttributes().getPlayerKillStreak())}));
  ...
  public static void open(Player player, Scoreboard scoreboard) {
    ...
    for (int i = 0; i <= 50; stringId++, i++) {
      ...
      if (entry != null) {
        if (scoreboard == Scoreboards.TOP_KILLSTREAKS) {
          line = "@whi@Rank @or1@" + (i + 1) + "@whi@ - " + entry.player.replaceAll("_", " ")
              + " - Killstreak: @or1@" + Integer.parseInt(entry.value[0]);
        } else if (scoreboard == Scoreboards.TOP_PKERS) {
          line = "@whi@Rank @or1@" + (i + 1) + "@whi@ - " + entry.player.replaceAll("_", " ")
              + " - KDR: @or1@" + Double.valueOf(entry.value[0]) + " @whi@Kills: @or1@"
              + Integer.parseInt(entry.value[1]) + "@whi@ Deaths:@or1@ "
              + Integer.parseInt(entry.value[2]) + "";
        } else if (scoreboard == Scoreboards.TOP_TOTAL_EXP) {
          line = "@whi@Rank @or1@" + (i + 1) + "@whi@ - " + entry.player.replaceAll("_", " ")
              + " - Total Exp: @or1@" + Misc.insertCommasToNumber(entry.value[0]);
        } else if (scoreboard == Scoreboards.TOP_ACHIEVER) {
          line = "@whi@Rank @or1@" + (i + 1) + "@whi@ - " + entry.player.replaceAll("_", " ")
              + " - Achievement Points: @or1@" + Misc.insertCommasToNumber(entry.value[0]);
        }
      }
    ...
    player.getPacketSender().sendInterface(6308)
        .sendString(6400, "Scoreboard - " + scoreboard.title + "").sendString(6399, "")
        .sendString(6401, "Close");
  }
