...
/**
 * Holds constants that hold data for all of the special attacks that can be used.
 * 
 * @author lare96
 */
public enum CombatSpecial {

  DRAGON_DAGGER(new int[] {1215, 1231, 5680, 5698}, 25, 1.16, 1.20, CombatType.MELEE,
  ...
  ABYSSAL_WHIP(new int[] {21047, 21372, 4151, 15441, 15442, 15443, 15444, 21000, 21001, 21002,
      21003, 21004, 21005, 21006, 21007}, 50, 1, 1, CombatType.MELEE, WeaponInterface.WHIP) {
    @Override
    public CombatContainer container(Player player, Character target) {
      ...
      return new CombatContainer(player, target, 1, CombatType.MELEE, true) {
        ...
        public void onHit(int damage, boolean accurate) {
          if (target.isPlayer() && accurate) {
            ...
            // player.getPacketSender().sendMessage("Your hammer forces some of your opponent's
            // defences to break.");
          }
        }
      };
        ...
        public void onHit(int dmg, boolean accurate) {
          if (accurate) {
            ...
            if (player.getSkillManager().getCurrentLevel(Skill.CONSTITUTION) < player
              ...
              int level = player.getSkillManager().getCurrentLevel(Skill.CONSTITUTION)
                  + damageHeal > player.getSkillManager().getMaxLevel(Skill.CONSTITUTION)
                      ? player.getSkillManager().getMaxLevel(Skill.CONSTITUTION)
                      : player.getSkillManager().getCurrentLevel(Skill.CONSTITUTION) + damageHeal;
  ...
  STAFF_OF_LIGHT(new int[] {14004, 14005, 14006, 14007, 15486}, 100, 1, 1, CombatType.MELEE,
    ...
    public void onActivation(Player player, Character target) {
      ...
      player.getPacketSender()
          .sendMessage("You are shielded by the spirits of the Staff of light!");
      player.getCombatBuilder().cooldown(true);
    }
  ...
  TOXIC_STAFF_OF_DEAD(new int[] {21077, 21079}, 100, 1, 1, CombatType.MELEE,
      WeaponInterface.LONGSWORD) {
    @Override
    public void onActivation(Player player, Character target) {
      ...
      TaskManager.submit(new StaffOfLightSpecialAttackTask(player));
      player.getPacketSender()
          .sendMessage("You are shielded by the gods of the Toxic staff of the dead!");
      player.getCombatBuilder().cooldown(true);
    }
        ...
        public void onHit(int damage, boolean accurate) {
          if (target.isPlayer()) {
            ...
            ((Player) target).getPacketSender()
                .sendMessage("Your overhead prayers have been disabled!");
  ...
  DRAGON_2H_SWORD(new int[] {7158}, 60, 1, 1, CombatType.MELEE, WeaponInterface.TWO_HANDED_SWORD) {
    ...
    public CombatContainer container(Player player, Character target) {
      ...
      return new CombatContainer(player, target, 1, CombatType.MELEE, false) {
        ...
        public void onHit(int damage, boolean accurate) {
          /*
           * if (Location.inMulti(player)) { List<GameCharacter> localEntities; if
           * (target.isPlayer()) { localEntities = Optional.of(player.getLocalPlayers()); } else if
           * (target.isNpc()) { localEntities = Optional.of(player.getLocalNpcs()); } for
           * (GameCharacter e : localEntities.get()) { if (e == null) { continue; } if
           * (e.getPosition().isWithinDistance( target.getPosition(), 1) && !e.equals(target) &&
           * !e.equals(player) && e.getConstitution() > 0 && !e.isDead()) { Hit hit =
           * CombatFactory.getHit(player, target, CombatType.MELEE); e.dealDamage(hit);
  ...
  DRAGON_HALBERD(new int[] {3204}, 30, 0.8, 0.75, CombatType.MELEE, WeaponInterface.HALBERD) {
    @Override
    public CombatContainer container(Player player, Character target) {
  ...
  ARMADYL_GODSWORD(new int[] {11694}, 50, 1.43, 1.63, CombatType.MELEE,
      WeaponInterface.TWO_HANDED_SWORD) {
    @Override
    public CombatContainer container(Player player, Character target) {
        ...
        public void onHit(int damage, boolean accurate) {
          if (target != null && target.isPlayer() && accurate) {
            ...
            ((Player) target).getPacketSender().sendMessage(
                "Your " + Misc.formatText(Skill.forId(skillDrain).toString().toLowerCase())
  ...
  /**
   * Create a new {@link CombatSpecial}. the weapon ID's that perform this special when activated.
   * 
   ...
   */
  private CombatSpecial(int[] identifiers, int drainAmount, double strengthBonus,
      double accuracyBonus, CombatType combatType, WeaponInterface weaponType) {
   ...
   * @param target the target when activating the special attack bar, will be <code>null</code> if
   * the player is not in combat while activating the special bar.
   */
  public void onActivation(Player player, Character target) {
  ...
  public static void drain(Player player, int amount) {
    player
        .decrementSpecialPercentage(player.getEquipment().contains(19669) ? amount *= .9 : amount);
  ...
  /**
   * Updates the special bar with the amount of special energy the argued {@link Player} has.
   * 
  ...
  public static void updateBar(Player player) {
    ...
    for (int i = 0; i < 10; i++) {
      player.getPacketSender().sendInterfaceComponentMoval(specialAmount >= specialCheck ? 500 : 0,
          0, --specialBar);
      specialCheck--;
    }
  ...
  public static void activate(Player player) {
    ...
    if (player.isSpecialActivated()) {
      ...
      boolean instantSpecial = spec == CombatSpecial.GRANITE_MAUL
          || spec == CombatSpecial.DRAGON_BATTLEAXE || spec == CombatSpecial.STAFF_OF_LIGHT
          || spec == CombatSpecial.TOXIC_STAFF_OF_DEAD || spec == CombatSpecial.STAFF_OF_DEAD;
      if (spec != CombatSpecial.STAFF_OF_LIGHT && spec != CombatSpecial.TOXIC_STAFF_OF_DEAD
        ...
        player.getPacketSender()
            .sendMessage("You are already being protected by the Staff of Light!");
        return;
      } else if (spec == CombatSpecial.TOXIC_STAFF_OF_DEAD && player.hasStaffOfLightEffect()) {
        player.getPacketSender()
            .sendMessage("You are already being protected by the Toxic staff of the dead!");
        return;
      } else if (spec == CombatSpecial.STAFF_OF_DEAD && player.hasStaffOfLightEffect()) {
        player.getPacketSender()
            .sendMessage("You are already being protected by the Staff of the dead!");
        return;
      }
