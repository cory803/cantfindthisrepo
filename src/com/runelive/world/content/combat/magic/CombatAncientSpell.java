...
/**
 * A {@link CombatSpell} implementation that is primarily used for spells that are a part of the
 * ancients spellbook.
 * 
 * @author lare96
 */
public abstract class CombatAncientSpell extends CombatSpell {
    ...
    for (Iterator<? extends Character> $it = it; $it.hasNext();) {
      ...
      if (next.isNpc()) {
        ...
        if ((p.getLocation() != Location.WILDERNESS
            && p.getLocation() != Location.FREE_FOR_ALL_ARENA) || !Location.inMulti(p)) {
          continue;
        }
      }

      if (next.getPosition().isWithinDistance(castOn.getPosition(), spellRadius())
          && !next.equals(cast) && !next.equals(castOn) && next.getConstitution() > 0
          && next.getConstitution() > 0) {
        if (next.isPlayer()) {
          Player p2 = (Player) next;
          Player p3 = (Player) cast;
          int combatDifference = CombatFactory.combatLevelDifference(
              p3.getSkillManager().getCombatLevel(), p2.getSkillManager().getCombatLevel());
          if (combatDifference > p3.getWildernessLevel()
              || combatDifference > p2.getWildernessLevel()) {
  ...
  /**
   * The radius of this spell, only comes in effect when the victim is hit in a multicombat area.
   * 
   * @return how far from the target this spell can hit when targeting multiple entities.
   */
