...
/**
 * A parent class represented by any generic spell able to be cast by an {@link Entity}.
 * 
 * @author lare96
 */
public abstract class Spell {
  ...
  /**
   * Determines if this spell is able to be cast by the argued {@link Player}. We do not include
   * {@link Npc}s here since no checks need to be made for them when they cast a spell.
   * 
   * @param player the player casting the spell.
   * @return <code>true</code> if the spell can be cast by the player, <code>false</code> otherwise.
   */
  public boolean canCast(Player player, boolean delete) {
    ...
    if (itemsRequired(player).isPresent()) {
      ...
      if (!player.getInventory().containsAll(items)) {
        ...
        player.getPacketSender()
            .sendMessage("You do not have the required items to cast this spell.");
    ...
    if (equipmentRequired(player).isPresent()) {
      if (!player.getEquipment().containsAll(equipmentRequired(player).get())) {
        player.getPacketSender()
            .sendMessage("You do not have the required equipment to cast this spell.");
        resetPlayerSpell(player);
        player.getCombatBuilder().reset(true);
        return false;
      }
   ...
   * 
   * @return the items required to cast this spell, or <code>null</code> if there are no items
   * required.
   */
   ...
   * 
   * @return the equipment required to cast this spell, or <code>null</code> if there is no
   * equipment required.
   */
