...
import com.runelive.world.content.combat.range.CombatRangedAmmo.RangedWeaponType;
...
/**
 * The default combat strategy assigned to an {@link Character} during a ranged based combat
 * session.
 * 
 * @author lare96
 */
public class DefaultRangedCombatStrategy implements CombatStrategy {
  ...
  public CombatContainer attack(Character entity, Character victim) {
    ...
    if (!player.isSpecialActivated()) {
      ...
      player.performGraphic(
          new Graphic(ammo.getStartGfxId(), ammo.getStartGfxId() == 2138 ? GraphicHeight.LOW
              : ammo.getStartHeight() >= 43 ? GraphicHeight.HIGH : GraphicHeight.MIDDLE));
  ...
  public static void fireProjectile(Character e, Character victim, final AmmunitionData ammo,
      boolean dBow) {
    TaskManager.submit(new Task(1, e.getCombatBuilder(), false) { // TODO
      ...
      protected void execute() {
        new Projectile(e, victim, ammo.getProjectileId(), ammo.getProjectileDelay() + 16,
            ammo.getProjectileSpeed(), ammo.getStartHeight(), ammo.getEndHeight(), 0)
                .sendProjectile();
        if (dBow) {
  ...
  /**
   * Starts the performAnimation for the argued {@link Player} in the current combat hook.
   * 
   * @param player the player to start the performAnimation for.
   */
  private void startAnimation(Player player) {
    if (player.getEquipment().get(Equipment.WEAPON_SLOT).getDefinition().getName()
        .startsWith("Karils")) {
  ...
  /**
   * Checks the ammo to make sure the argued {@link Player} has the right type and amount before
   * attacking.
   * 
   * @param player the player's ammo to check.
   * @return <code>true</code> if the player has the right ammo, <code>false</code> otherwise.
   */
  private boolean checkAmmo(Player player) {
    ...
    if (!properEquipment) {
      String ammoName = ammunition.getDefinition().getName(),
          weaponName =
              player.getEquipment().getItems()[Equipment.WEAPON_SLOT].getDefinition().getName(),
  ...
  public static void decrementAmmo(Player player, Position pos) {
    ...
    int slot = player.getWeapon() == WeaponInterface.SHORTBOW
        || player.getWeapon() == WeaponInterface.LONGBOW
        || player.getWeapon() == WeaponInterface.CROSSBOW ? Equipment.AMMUNITION_SLOT
    ...
    final boolean avas =
        player.getEquipment().get(Equipment.CAPE_SLOT).getId() == 10499 || compCapes;
    if (avas) { // Avas
      if (Misc.getRandom(8) == 1 || Misc.getRandom(8) == 2 || Misc.getRandom(8) == 3
          || Misc.getRandom(8) == 4 || Misc.getRandom(8) == 5 || Misc.getRandom(8) == 6
          || Misc.getRandom(8) == 7)
        return;
    }
    if (ardy) { // ardy
      if (Misc.getRandom(7) == 1 || Misc.getRandom(7) == 2 || Misc.getRandom(7) == 3
          || Misc.getRandom(7) == 4 || Misc.getRandom(7) == 5 || Misc.getRandom(7) == 0
          || Misc.getRandom(7) == 6)
        return;
    }
  ...
  private static final int getModifiedDamage(Player player, Character target,
      CombatContainer container) {
    ...
    switch (ammo) {
      ...
      case 9242:
        ...
        int priceDamage =
            (int) (player.getSkillManager().getCurrentLevel(Skill.CONSTITUTION) * 0.08);
        if (priceDamage < 0) {
          return damage;
        }
        int dmg2 = (int) ((int) ((int) target.getConstitution() * 0.065) > 1000
            ? 650 + Misc.getRandom(50) : ((int) target.getConstitution() * 0.065));
        if (dmg2 <= 0) {
          return damage;
        }
