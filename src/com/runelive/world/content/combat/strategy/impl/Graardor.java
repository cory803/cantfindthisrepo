...
public class Graardor implements CombatStrategy {
  ...
  public boolean customContainerAttack(Character entity, Character victim) {
    ...
    CombatType style = Misc.getRandom(4) <= 1
        && Locations.goodDistance(graardor.getPosition(), victim.getPosition(), 1)
    ...
    if (style == CombatType.MELEE) {
      ...
      for (Player t : Misc.getCombinedPlayerList(target)) {
        if (t == null || t.getLocation() != Location.GODWARS_DUNGEON || t.isTeleporting())
          continue;
        if (t.getPosition().distanceToPoint(graardor.getPosition().getX(),
            graardor.getPosition().getY()) > 20)
          continue;
