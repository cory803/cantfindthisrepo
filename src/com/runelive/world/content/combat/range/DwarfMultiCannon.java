...
public class DwarfMultiCannon {
  ...
  CANNON_BASE = 7, CANNON_STAND = 8, CANNON_BARRELS = 9, CANNON = 6,
      ...
      CANNONBALL = 2, CANNON_BASE_ID = 6, CANNON_STAND_ID = 8, CANNON_BARRELS_ID = 10,
  ...
  public static boolean canSetupCannon(Player c) {
    if (c.getSkillManager().getCurrentLevel(Skill.RANGED) < 75) {
      c.getPacketSender()
          .sendMessage("You need a Ranged level of atleast 75 to setup the dwarf-cannon.");
      return false;
    }
    if (!c.getInventory().contains(CANNON_BASE_ID) || !c.getInventory().contains(CANNON_STAND_ID)
      ...
      c.getPacketSender()
          .sendMessage("You don't have the required items to setup the dwarf-cannon.");
      return false;
    }
    if (c.getCannon() != null) {
    ...
    if (!c.getMovementQueue().canWalk(3, 3) || CustomObjects.objectExists(c.getPosition().copy())
      ...
      c.getPacketSender()
          .sendMessage("The dwarf-cannon cannot be setup here. Try moving around a bit.");
      return false;
    }
  ...
  public static void startFiringCannon(Player c, DwarfCannon cannon) {
    ...
    if (cannon.getCannonballs() <= 15) {
      int playerCannonballs =
          c.getInventory().getAmount(2) > 30 ? 30 : c.getInventory().getAmount(2);
      int cannonballsToAdd = playerCannonballs - cannon.getCannonballs();
      if (playerCannonballs < 1) {
        c.getPacketSender().sendMessage(
            "You do not have any cannonballs in your inventory to fire the cannon with.");
        return;
      }
  ...
  public static void attack(Player player) { // TODO: Bad hardcoding of setting damage, switch to
                                             // combathittask
    ...
    if (n == null)
      return;
    Hit dmg = new Hit(Misc.getRandom(266) - Misc.getRandom(n.getDefinition().getDefenceRange()),
        Hitmask.RED, CombatIcon.CANNON);
    ...
    Achievements.doProgress(player, AchievementData.DEAL_MEDIUM_DAMAGE_USING_RANGED,
        dmg.getDamage());
