...
import com.runelive.world.content.ItemDegrading.DegradingItem;
...
/**
 * A static factory class containing all miscellaneous methods related to, and used for combat.
 *
 * @author lare96
 * @author Scu11
 * @author Graham
...
public final class CombatFactory {
    ...
    /**
     * The default constructor, will throw an {@link UnsupportedOperationException} if instantiated.
     */
    ...
    public static boolean arrowsEquipped(Player player) {
        ...
        return !(!item.getDefinition().getName().endsWith("arrow")
                && !item.getDefinition().getName().endsWith("arrowp")
    ...
    /**
     * Attempts to poison the argued {@link Character} with the argued {@link PoisonType}. This method
     * will have no effect if the entity is already poisoned.
     *
    ...
    /**
     * Attempts to venom the argued {@link Character} with the argued {@link PoisonType}. This method
     * will have no effect if the entity is already poisoned.
     *
     * @param entity the entity that will be poisoned, if not already. the venom type that this entity
     *               is being inflicted with.
     */
    ...
    /**
     * Attempts to venom the argued {@link Character} with the argued {@link PoisonType}. This method
     * will have no effect if the entity is already venomed.
     *
     * @param entity the entity that will be venomed, if not already. the venom type that this entity
     *               is being inflicted with.
     */
    ...
    /**
     * Attempts to put the skull icon on the argued player, including the effect where the player
     * loses all item upon death. This method will have no effect if the argued player is already
     * skulled.
     *
    ...
    /**
     * Calculates the combat level difference for wilderness player vs. player combat.
     *
    ...
    public static Hit getHit(Character entity, Character victim, CombatType type) {
        switch (type) {
            ...
            case RANGED:
                return new Hit(Misc.inclusiveRandom(1, CombatFactory.calculateMaxRangedHit(entity, victim)),
                        Hitmask.RED, CombatIcon.RANGED);
            case MAGIC:
                return new Hit(Misc.inclusiveRandom(1, DesolaceFormulas.getMagicMaxhit(entity)),
                        Hitmask.RED, CombatIcon.MAGIC);
            case DRAGON_FIRE:
                return new Hit(
                        Misc.inclusiveRandom(0, CombatFactory.calculateMaxDragonFireHit(entity, victim)),
                        Hitmask.RED, CombatIcon.MAGIC);
            default:
    ...
    /**
     * A flag that determines if the entity's attack will be successful based on the argued attacker's
     * and victim's stats.
     *
        ...
        if (attacker.isPlayer() && victim.isPlayer()) {
            ...
            switch (type) {
                case MAGIC:
                    ...
                    return Misc
                            .getRandom(DesolaceFormulas.getMagicDefence(p2)) < Misc.getRandom((mageAttk / 2))
                            + Misc.getRandom((int) (mageAttk / 1.9));
                case MELEE:
                    int def = 1 + DesolaceFormulas.getMeleeDefence(p2);
                    return Misc.getRandom(def) < Misc.getRandom(1 + DesolaceFormulas.getMeleeAttack(p1))
                            + (def / 4.5);
                case RANGED:
            ...
            switch (type) {
        /*
         * case MAGIC: case KORASI: int mageAttk = DesolaceFormulas.getMagicAttack(p1); return
         * Misc.getRandom(n.getDefinition().getDefenceMage()) < Misc.getRandom((mageAttk / 2)) +
         * Misc.getRandom((int) (mageAttk/2.1));
         */
                case MELEE:
                    int def = 1 + n.getDefinition().getDefenceMelee();
                    return Misc.getRandom(def) < Misc.getRandom(5 + DesolaceFormulas.getMeleeAttack(p1))
                            + (def / 4);
                case RANGED:
    ...
    /**
     * Calculates the maximum melee hit for the argued {@link Character} without taking the victim
     * into consideration.
     *
    ...
    public static int calculateMaxMeleeHit(Character entity, Character victim) {
        ...
        double baseDamage = 1.3 + (effectiveStrengthDamage / 10)
                ...
                + ((effectiveStrengthDamage
                * player.getBonusManager().getOtherBonus()[BonusManager.BONUS_STRENGTH]) / 640);
    ...
    /**
     * Calculates the maximum ranged hit for the argued {@link Character} without taking the victim
     * into consideration.
     *
     * @param entity the entity to calculate the maximum hit for.
     * @param victim the victim being attacked.
    ...
    public static int calculateMaxRangedHit(Character entity, Character victim) {
        ...
        int effectiveRangeDamage =
                (int) ((rangeLevel * prayerMultiplier * otherBonusMultiplier) + combatStyleBonus);
    ...
    /**
     * A series of checks performed before the entity attacks the victim. the builder to perform the
     * checks with.
     *
    ...
    public static boolean checkHook(Character entity, Character victim) {
        ...
        if (victim.isNpc() && entity.isPlayer()) {
            ...
            if (Nex.nexMob(npc.getId()) || npc.getId() == 6260 || npc.getId() == 6261
                    ...
                    || npc.getId() == 6203 || npc.getId() == 6208 || npc.getId() == 6204
                    || npc.getId() == 6206 || npc.getId() == 6247 || npc.getId() == 6248
            ...
            if (npc.getLocation() != Location.DUNGEONEERING && npc.getDefinition()
                    .getSlayerLevel() > ((Player) entity).getSkillManager().getCurrentLevel(Skill.SLAYER)) {
            ...
            if (npc.getId() == 13465 || npc.getId() == 13469 || npc.getId() == 13474
                    || npc.getId() == 13478 || npc.getId() == 13479) {
                if (entity.getLocation() != Location.WILDERNESS
                        && entity.getLocation() != Location.EZONE_DONOR) {
                    ((Player) entity).getPacketSender().sendMessage("You cannot reach that.");
                    entity.getCombatBuilder().reset(true);
                    return false;
                }
            }
            if (npc.getId() == 4291 && entity.getPosition().getZ() == 2 && !((Player) entity)
                    .getMinigameAttributes().getWarriorsGuildAttributes().enteredTokenRoom()) {
                ((Player) entity).getPacketSender().sendMessage("You cannot reach that.");
                entity.getCombatBuilder().reset(true);
                return false;
            }
     ...
     */
        ...
        if (!projectilePathBlocked && goodDistance) {
            if (strategy.getCombatType() == CombatType.MELEE && RegionClipping.isInDiagonalBlock(b, a)) {
                Direction dir = Direction.direction(a.getPosition().getX(), victim.getX(), a.getPosition().getY(), victim.getY());
                int offsetX = 0;
                int offsetY = 0;
                int size = a.getSize();
        ...
        if (builder.getVictim().isPlayer()) {
            ...
            if (npc.getId() == 8349 && container.getCombatType() == CombatType.MELEE) {
                container.allHits(context -> {
                    ...
                    && (container.getCombatType() == CombatType.MAGIC
                    || container.getCombatType() == CombatType.RANGED)
     ...
     */
    protected static void giveExperience(CombatBuilder builder, CombatContainer container,
                                         int damage) {
    ...
    /**
     * Handles various armor effects for the attacker and victim. the attacker's combat container.
     *
     * @param damage the total amount of damage dealt.
     */
    // TODO: Use abstraction for this, will need it when more effects are added.
    protected static void handleArmorEffects(Character attacker, Character target, int damage,
        ...
        if (attacker.getConstitution() > 0 && damage > 0) {
            ...
            if (target != null && target.isPlayer()) {
                ...
                /** RING OF LIFE **/
                else if ((t2.getEquipment().getItems()[Equipment.RING_SLOT].getId() == 2570 || Emotes.Skillcape_Data.DEFENCE.isWearingCape(t2) || Emotes.Skillcape_Data.MASTER_DEFENCE.isWearingCape(t2)) && t2.getLocation() != Location.DUEL_ARENA
                        && t2.getLocation() != Location.WILDERNESS) {
      ...
      /*
       * if (builder.getVictim().isPlayer()) { Player victim = (Player) builder.getVictim(); // The
       * torags effect is here. if (CombatFactory.fullTorags(builder.getEntity())) {
       * victim.decrementRunEnergy(Misc.inclusiveRandom(1, 100)); victim.performGraphic(new
       * Graphic(399)); return; } // The ahrims effect is here. if
       ...
       * victim.getSkills()[Skills.STRENGTH].decreaseLevel(Utility.inclusiveRandom( 1, 10));
       * Skills.refresh(victim, Skills.STRENGTH); victim.performGraphic(new Graphic(400)); return; }
       * // The karils effect is here. if (CombatFactory.fullKarils(builder.getEntity()) &&
       * victim.getSkills()[Skills.AGILITY].getLevel() >=
       * victim.getSkills()[Skills.AGILITY].getLevelForExperience()) { victim.performGraphic(new
       * Graphic(401)); victim.getSkills()[Skills.AGILITY].decreaseLevel(Utility.inclusiveRandom( 1,
    ...
    /**
     * Handles various prayer effects for the attacker and victim. the attacker's combat container.
     *
     * @param damage the total amount of damage dealt.
     */
    protected static void handlePrayerEffects(Character attacker, Character target, int damage,
                                              CombatType combatType) {
        ...
        if (target.isPlayer() && damage > 0) {
            ...
            if (PrayerHandler.isActivated(victim, PrayerHandler.REDEMPTION) && victim
                    .getConstitution() <= (victim.getSkillManager().getMaxLevel(Skill.CONSTITUTION) / 10)) {
            ...
            if (attacker.isPlayer()) {
                ...
                if (PrayerHandler.isActivated(victim, PrayerHandler.RETRIBUTION)
                        && victim.getConstitution() < 1) {
                    victim.performGraphic(new Graphic(437));
                    if (p.getPosition().isWithinDistance(victim.getPosition(),
                ...
                } else if (CurseHandler.isActivated(victim, CurseHandler.WRATH)
                        && victim.getConstitution() < 1) {
        ...
        if (attacker.isPlayer()) {
            ...
            if (p.getCurseActive()[CurseHandler.LEECH_ATTACK]
                    || p.getCurseActive()[CurseHandler.LEECH_DEFENCE]
                    || p.getCurseActive()[CurseHandler.LEECH_STRENGTH]
                    || p.getCurseActive()[CurseHandler.LEECH_MAGIC]
                ...
                if (i != -1) {
                    ...
                    p.getPacketSender()
                            .sendMessage("You manage to leech your target's " + (i == 8 ? ("energy")
                                    : i == 7 ? ("Special Attack")
        ...
        if (target.isPlayer()) {
            ...
            if (damage > 0 && Misc.getRandom(10) <= 4) {
                int deflectDamage = -1;
                if (CurseHandler.isActivated(victim, CurseHandler.DEFLECT_MAGIC)
    ...
    protected static void handleSpellEffects(Character attacker, Character target, int damage,
                                             CombatType combatType) {
        if (damage <= 0)
            return;
        if (target.isPlayer()) {
    ...
    public static void chargeDragonFireShield(Player player) {
        ...
        player.getPacketSender()
                .sendMessage("Your shield absorbs some of the Dragon's fire and you have "
                        + player.getDfsCharges() + "/50 charges..");
    }

    public static void handleDragonFireShield(final Player player, final Character target) {
        if (player == null || target == null || target.getConstitution() <= 0
                || player.getConstitution() <= 0)
            return;
