package com.runelive.world.content.combat.prayer;
...
/**
 * All of the prayers that can be activated and deactivated. This currently only has support for
 * prayers present in the <b>317 protocol</b>.
 * 
 * @author Gabriel
 */
public class PrayerHandler {
  ...
  /**
   * Represents a prayer's configurations, such as their level requirement, buttonId, configId and
   * drain rate.
   * 
   * @author relex lawl
   */
  private enum PrayerData {
    THICK_SKIN(1, 1, 25000, 83), BURST_OF_STRENGTH(4, 1, 25002, 84), CLARITY_OF_THOUGHT(7, 1, 25004,
        85), SHARP_EYE(8, 1, 25006, 601), MYSTIC_WILL(9, 1, 25008, 602), ROCK_SKIN(10, 2, 25010,
            86), SUPERHUMAN_STRENGTH(13, 2, 25012, 87), IMPROVED_REFLEXES(16, 2, 25014,
                88), RAPID_RESTORE(19, .4, 25016, 89), RAPID_HEAL(22, .6, 25018, 90), PROTECT_ITEM(
                    25, .6, 25020, 91), HAWK_EYE(26, 1.5, 25022, 603), MYSTIC_LORE(27, 2, 25024,
                        604), STEEL_SKIN(28, 4, 25026, 92), ULTIMATE_STRENGTH(31, 4, 25028,
                            93), INCREDIBLE_REFLEXES(34, 4, 25030, 94), PROTECT_FROM_MAGIC(37, 4,
                                25032, 95, 2), PROTECT_FROM_MISSILES(40, 4, 25034, 96,
                                    1), PROTECT_FROM_MELEE(43, 4, 25036, 97, 0), EAGLE_EYE(44, 4,
                                        25038, 605), MYSTIC_MIGHT(45, 4, 25040, 606), RETRIBUTION(
                                            46, 1, 25042, 98, 4), REDEMPTION(49, 2, 25044, 99,
                                                5), SMITE(52, 6, 25046, 100, 685, 6), CHIVALRY(60,
                                                    8, 25048,
                                                    607), PIETY(70, 10, 25050, 608), RIGOUR(80, 11,
                                                        25104, 609), AUGURY(80, 11, 25108, 610);
    ...
    /**
     * The prayer's config id to switch their glow on/off by sending the sendConfig packet.
     */
    private int configId;

    /**
     * The prayer's drain rate as which it will drain the associated player's prayer points.
     */
  ...
  /**
   * Activates said prayer with specified <code>prayerId</code> and de-activates all non-stackable
   * prayers.
   * 
   * @param player The player activating prayer.
   * @param prayerId The id of the prayer being turned on, also known as the ordinal in the
   * respective enum.
   */
  public static void activatePrayer(Player player, final int prayerId) {
    ...
    if (!player.getDragonScimitarTimer().elapsed(5000)) {
      player.getPacketSender().sendMessage(
          "You have been damaged badly with a Dragon Scimitar! Try again in 5 seconds.");
      CurseHandler.deactivateAll(player);
      PrayerHandler.deactivateAll(player);
      return;
    }
    if (player.getLocation() == Location.RECIPE_FOR_DISASTER) {
      player.getPacketSender()
          .sendMessage("For some reason, your prayers do not have any effect in here.");
      CurseHandler.deactivateAll(player);
      PrayerHandler.deactivateAll(player);
      return;
    }
    ...
    if (player.getSkillManager().getCurrentLevel(Skill.PRAYER) <= 0) {
      ...
      player.getPacketSender().sendMessage(
          "You do not have enough Prayer points. You can recharge your points at an altar.");
      return;
    }
    if (player.getSkillManager().getMaxLevel(Skill.PRAYER) < (pd.requirement * 10)) {
      player.getPacketSender().sendConfig(pd.configId, 0);
      player.getPacketSender().sendMessage("You need a Prayer level of at least " + pd.requirement
          + " to use " + pd.getPrayerName() + ".");
      return;
    }
    if (prayerId == CHIVALRY && player.getSkillManager().getMaxLevel(Skill.DEFENCE) < 60) {
      player.getPacketSender().sendConfig(pd.configId, 0);
      player.getPacketSender()
          .sendMessage("You need a Defence level of at least 60 to use Chivalry.");
      return;
    }
    ...
    if (prayerId == RIGOUR && player.getSkillManager().getMaxLevel(Skill.DUNGEONEERING) < 78) {
      player.getPacketSender().sendConfig(pd.configId, 0);
      player.getPacketSender()
          .sendMessage("You need a Dungeoneering level of at least 78 to use Rigour.");
      return;
    }
    if (prayerId == AUGURY && player.getSkillManager().getMaxLevel(Skill.DUNGEONEERING) < 78) {
      player.getPacketSender().sendConfig(pd.configId, 0);
      player.getPacketSender()
          .sendMessage("You need a Dungeoneering level of at least 78 to use Augury.");
      return;
    }
    switch (prayerId) {
      case THICK_SKIN:
    ...
    /*
     * if (player.isPrayerInjured()) { if (prayerId == PROTECT_FROM_MAGIC || prayerId ==
     * PROTECT_FROM_MISSILES || prayerId == PROTECT_FROM_MELEE) {
  ...
  /**
   * Gets the player's current head hint if they activate or deactivate a head prayer.
   * 
  ...
  public static final int THICK_SKIN = 0, BURST_OF_STRENGTH = 1, CLARITY_OF_THOUGHT = 2,
      SHARP_EYE = 3, MYSTIC_WILL = 4, ROCK_SKIN = 5, SUPERHUMAN_STRENGTH = 6, IMPROVED_REFLEXES = 7,
      RAPID_RESTORE = 8, RAPID_HEAL = 9, PROTECT_ITEM = 10, HAWK_EYE = 11, MYSTIC_LORE = 12,
      STEEL_SKIN = 13, ULTIMATE_STRENGTH = 14, INCREDIBLE_REFLEXES = 15, PROTECT_FROM_MAGIC = 16,
      PROTECT_FROM_MISSILES = 17, PROTECT_FROM_MELEE = 18, EAGLE_EYE = 19, MYSTIC_MIGHT = 20,
      RETRIBUTION = 21, REDEMPTION = 22, SMITE = 23, CHIVALRY = 24, PIETY = 25, RIGOUR = 26,
      AUGURY = 27;
  ...
  private static final int[] DEFENCE_PRAYERS =
      {THICK_SKIN, ROCK_SKIN, STEEL_SKIN, CHIVALRY, PIETY, RIGOUR, AUGURY};

  /**
   * Contains every prayer that counts as a strength prayer.
   */
  private static final int[] STRENGTH_PRAYERS =
      {BURST_OF_STRENGTH, SUPERHUMAN_STRENGTH, ULTIMATE_STRENGTH, CHIVALRY, PIETY};
   ...
   */
  public static final int[] OVERHEAD_PRAYERS = {PROTECT_FROM_MAGIC, PROTECT_FROM_MISSILES,
      PROTECT_FROM_MELEE, RETRIBUTION, REDEMPTION, SMITE};
