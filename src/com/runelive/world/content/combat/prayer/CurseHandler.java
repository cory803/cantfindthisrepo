...
public class CurseHandler {
  ...
  public static void handleLeech(Character p, int index, int amount, int maxAmount,
      boolean decrease) {
    if (!p.isPlayer())
      return;
  ...
  public static void activateCurse(Player player, CurseData curse) {
    ...
    if (player.getLocation() == Location.RECIPE_FOR_DISASTER) {
      player.getPacketSender()
          .sendMessage("For some reason, your prayers do not have any effect here.");
      CurseHandler.deactivateAll(player);
      PrayerHandler.deactivateAll(player);
      return;
    }
    ...
    if (player.getSkillManager().getMaxLevel(Skill.PRAYER) < (curse.requirement * 10)) {
      player.getPacketSender().sendConfig(curse.configId, 0);
      // DialogueManager.sendOneStringStatement(p, " " + "You need a @blu@Prayer <col=0>level of " +
      // pd.levelRequirement +" to use @blu@" + pd.getPrayerName());
      player.getPacketSender().sendMessage("You need a Prayer level of at least "
          + curse.requirement + " to use " + curse.name + ".");
      return;
    }
    if (curse == CurseData.TURMOIL && player.getSkillManager().getMaxLevel(Skill.DEFENCE) < 30) {
      player.getPacketSender().sendConfig(curse.configId, 0);
      player.getPacketSender()
          .sendMessage("You need a Defence level of at least 30 to use Turmoil.");
      return;
    }
    switch (curse) {
      case SAP_WARRIOR:
     ...
     * CurseData.DEFLECT_SUMMONING) { player.getPacketSender().sendMessage(
  ...
  public static void deactivateCurse(Player player, CurseData curse) {
    ...
    // SoundEffects.sendSoundEffect(player, SoundEffects.SoundData.DEACTIVATE_PRAYER_OR_CURSE, 10,
    // 0);
    player.getPacketSender().sendConfig(curse.configId, 0);
  ...
  private enum CurseData {
    PROTECT_ITEM(50, .1, 32503, 610,
        new PrayerAnimation(new Animation(12567), new Graphic(2213))), SAP_WARRIOR(50, .3, 32505,
            611), SAP_RANGER(52, .3, 32507, 612), SAP_MAGE(54, .3, 32509,
                613), SAP_SPIRIT(56, .3, 32511, 614), BERSERKER(59, .4, 32513, 615,
                    new PrayerAnimation(new Animation(12589),
                        new Graphic(2266))), DEFLECT_SUMMONING(62, .5, 32515,
                            616), DEFLECT_MAGIC(65, .5, 32517, 617), DEFLECT_MISSILES(68, .5, 32519,
                                618), DEFLECT_MELEE(71, .5, 32521, 619), LEECH_ATTACK(74, 1, 32523,
                                    620), LEECH_RANGED(76, 1, 32525, 621), LEECH_MAGIC(78, 1, 32527,
                                        622), LEECH_DEFENCE(80, 1, 32529, 623), LEECH_STRENGTH(82,
                                            1, 32531, 624), LEECH_ENERGY(84, 1, 32533,
                                                625), LEECH_SPECIAL_ATTACK(86, 1, 32535,
                                                    626), WRATH(89, 1.2, 32537, 627), SOUL_SPLIT(92,
                                                        1.5, 32539, 628), TURMOIL(95, 3, 32541, 629,
                                                            new PrayerAnimation(
                                                                new Animation(12565),
                                                                new Graphic(2226)));
  ...
  private static final CurseData[] ACCURACY_CURSES =
      {CurseData.SAP_WARRIOR, CurseData.LEECH_ATTACK, CurseData.TURMOIL};

  private static final CurseData[] RANGED_CURSES =
      {CurseData.SAP_RANGER, CurseData.LEECH_RANGED, CurseData.TURMOIL};

  private static final CurseData[] MAGIC_CURSES =
      {CurseData.SAP_MAGE, CurseData.LEECH_MAGIC, CurseData.TURMOIL};

  private static final CurseData[] SPECIAL_ATTACK_CURSES =
      {CurseData.SAP_SPIRIT, CurseData.LEECH_SPECIAL_ATTACK};
  ...
  public static final CurseData[] OVERHEAD_CURSES =
      {CurseData.DEFLECT_MAGIC, CurseData.DEFLECT_MISSILES, CurseData.DEFLECT_MELEE,
          CurseData.DEFLECT_SUMMONING, CurseData.WRATH, CurseData.SOUL_SPLIT};

  private static final CurseData[] NON_DEFLECT_OVERHEAD_CURSES =
      {CurseData.WRATH, CurseData.SOUL_SPLIT};
  ...
  public static final int PROTECT_ITEM = CurseData.PROTECT_ITEM.ordinal(),
      ...
      BERSERKER = CurseData.BERSERKER.ordinal(),
      DEFLECT_SUMMONING = CurseData.DEFLECT_SUMMONING.ordinal(),
      DEFLECT_MAGIC = CurseData.DEFLECT_MAGIC.ordinal(),
      DEFLECT_MISSILES = CurseData.DEFLECT_MISSILES.ordinal(),
      DEFLECT_MELEE = CurseData.DEFLECT_MELEE.ordinal(),
      LEECH_ATTACK = CurseData.LEECH_ATTACK.ordinal(),
      LEECH_RANGED = CurseData.LEECH_RANGED.ordinal(),
      LEECH_MAGIC = CurseData.LEECH_MAGIC.ordinal(),
      LEECH_DEFENCE = CurseData.LEECH_DEFENCE.ordinal(),
      LEECH_STRENGTH = CurseData.LEECH_STRENGTH.ordinal(),
      LEECH_ENERGY = CurseData.LEECH_ENERGY.ordinal(),
      LEECH_SPECIAL_ATTACK = CurseData.LEECH_SPECIAL_ATTACK.ordinal(),
      WRATH = CurseData.WRATH.ordinal(), SOUL_SPLIT = CurseData.SOUL_SPLIT.ordinal(),
      TURMOIL = CurseData.TURMOIL.ordinal();
