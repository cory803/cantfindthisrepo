...
public class AvatarOfDestruction {
	...
	public static enum LocationData {
		...
		LOCATION_1(new Position(3107, 3506), "Right of the Edgeville bank", "Edgeville"), LOCATION_2(
				new Position(3087, 3541), "Wilderness ditch, level 3", "Wilderness"), LOCATION_3(
						new Position(2470, 5166), "Enterence of TzHaar dungeon", "TzHaar"), LOCATION_4(
								new Position(3362, 3285), "Behind the Duel Arena", "Duel Arena"), LOCATION_5(
										new Position(2928, 3453), "Enterence of Taverley", "Taverley"),;
	...
	public static void sequence() {
		if (FALLEN_BOSS == null) {
			if (timer.elapsed(TIME)) {
				...
				World.sendMessage(
						"<icon=2><shad=FF8C38> The Avatar of Destruction has spawned at " + locationData.clue + "!");
				// World.getPlayers().forEach(p ->
				// p.getPacketSender().sendString(39162,"@or2@Crashed star:
				// @yel@" +
			...
			// } else {
			// if (FALLEN_BOSS.starObject.getPickAmount() >=
			// MAXIMUM_MINING_AMOUNT) {
			// despawn(false);
			// timer.reset();
			// }
		}
	...
	public static void despawn(boolean respawn) {
		...
		if (FALLEN_BOSS != null) {
			for (Player p : World.getPlayers()) {
				 ...
				 * p.getInteractingObject().getId() ==
				 * FALLEN_BOSS.starObject.getId()) { p.performAnimation(new
				 * Animation(65535));
