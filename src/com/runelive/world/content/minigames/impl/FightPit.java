...
public class FightPit {
   ...
   */
  private static Map<Player, String> playerMap =
      Collections.synchronizedMap(new HashMap<Player, String>());
  ...
  public static boolean endingGame() {
    for (Player player : playerMap.keySet()) {
      if (player != null) {
        if (getListCount("PLAYING") == 1 && getState(player) != null
            && getState(player).equals("PLAYING")) {
  ...
  public static void viewOrb(Player player) {
     ...
     * return; for(int i = 0; i < org.Desolace.util.Constants.SIDEBAR_INTERFACES.length; i++)
     * player.getPacketSender().sendTabInterface(i, -1);
     * player.getPacketSender().sendTabInterface(4, 3209); player.viewingOrb = true;
  ...
  public static void cancelOrbView(Player player) {
    /*
     * if(!player.viewingOrb) return; player.viewingOrb = false; for(int i = 0; i <
     ...
     * player.getSpellbook().getInterfaceId()); player.getPacketSender().sendCameraNeutrality();
