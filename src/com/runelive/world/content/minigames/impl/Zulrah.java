...
public class Zulrah {
	...
	public static void handleZulrahLoot(Player p) {
		...
		p.getPacketSender()
				.sendLootMessage("You have received 1 x " + ItemDefinition.forId(loot).getName() + " from Zulrah.");
		p.getPacketSender().sendLootMessage(
				"You have received some " + ItemDefinition.forId(stackable_drop).getName() + " from Zulrah.");
		if (rareDropRoll == 256) {
	...
	public static void handleZulrahDeath(final Player player, NPC n) {
		if (n.getId() == ZULRAH_GREEN_NPC_ID || n.getId() == ZULRAH_RED_NPC_ID || n.getId() == ZULRAH_BLUE_NPC_ID
				|| n.getId() == ZULRAH_JAD_NPC_ID) {
			if (player.getRegionInstance() != null) {
