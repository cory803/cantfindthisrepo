...
public class WarriorsGuild {
  ...
  private static final int[] DEFENDERS =
      new int[] {8844, 8845, 8846, 8847, 8848, 8849, 8850, 13262};
   ...
   */
  public static boolean itemOnAnimator(final Player player, final Item item,
            ...
            if (player.getInventory().contains(ARMOR_DATA[i][0])
              ...
              if (items != null) {
                ...
                player.setRegionInstance(
                    new RegionInstance(player, RegionInstanceType.WARRIORS_GUILD));
                ...
                TaskManager.submit(new Task(2) {
                  @Override
                  public void execute() {
                    NPC npc_ = new NPC(ARMOR_DATA[index][3],
  ...
  public static void handleDrop(Player player, NPC npc) {
    ...
    if (npc.getId() >= 4278 && npc.getId() <= 4284) {
      ...
      if (armour != null) {
        for (int i : armour)
          GroundItemManager.spawnGroundItem(player, new GroundItem(new Item(i),
        ...
        GroundItemManager.spawnGroundItem(player,
            new GroundItem(new Item(8851, getTokenAmount(npc.getId())), npc.getPosition().copy(),
  ...
  public static int getDefender(Player player) {
    ...
    for (int i = 0; i < DEFENDERS.length; i++) {
      if (player.getInventory().contains(DEFENDERS[i])
          || player.getEquipment().contains(DEFENDERS[i])) {
        foundIndex = i;
      }
    }
    if (foundIndex != 7) {
    ...
    return new Dialogue() {
      ...
      public String[] dialogue() {
        ...
        return new String[] {"I'll release some Cyclops which might drop",
            "" + ItemDefinition.forId(defender).getName().replace(" defender", "")
                + " Defenders for you.",
            "Good luck warrior!"};
      }
  ...
  public static void resetCyclopsCombat(Player player) {
    for (NPC n : player.getLocalNpcs()) {
      ...
      if (n.getId() == 4291 && n.getLocation() == Location.WARRIORS_GUILD
          && n.getCombatBuilder().getVictim() != null
          && n.getCombatBuilder().getVictim() == player) {
