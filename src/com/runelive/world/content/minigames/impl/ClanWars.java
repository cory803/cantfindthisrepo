...
public class ClanWars {
  ...
  public enum Rules {
    // combat rules
    COMBAT_MELEE(false), COMBAT_MAGE(false), COMBAT_RANGE(false), COMBAT_PRAYER(
        false), COMBAT_SUMMONING(false), COMBAT_FOOD(false), COMBAT_POTS(false),

    // time limit
    NO_LIMIT(1000), FIVE_MINUTES(5), TEN_MINUTES(10), THIRTY_MINUTES(30),

    // kills
    FIRST_25(25, true), FIRST_50(50, true), FIRST_75(75, true), FIRST_100(100,
        true), LAST_TEAM_STADING(50, false),
  ...
  public void selectRule(DuelRule duelRule) {
    ...
    if (duelRule == DuelRule.NO_SHIELD) {
      if (player.getEquipment().getItems()[Equipment.WEAPON_SLOT].getId() > 0
          && ItemDefinition.forId(player.getEquipment().getItems()[Equipment.WEAPON_SLOT].getId())
              .isTwoHanded()
          || ItemDefinition
              .forId(playerToChallenge.getEquipment().getItems()[Equipment.WEAPON_SLOT].getId())
              .isTwoHanded())
        slotOccupied = true;
    }
    ...
    playerToChallenge.getDueling().selectedDuelRules[index] =
