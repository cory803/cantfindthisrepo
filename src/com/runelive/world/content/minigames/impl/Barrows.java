...
import com.runelive.engine.task.impl.CeilingCollapseTask;
...
public class Barrows {
    ...
    switch (object.getId()) {
      ...
      case 10284:
        ...
        if (player.getMinigameAttributes().getBarrowsMinigameAttributes().getBarrowsData()[player
          ...
          return true;
        } else if (player.getMinigameAttributes().getBarrowsMinigameAttributes()
            .getBarrowsData()[player.getMinigameAttributes().getBarrowsMinigameAttributes()
                .getRandomCoffin()][1] == 1) {
  ...
  public static void showRiddle(Player player) {
    ...
    player.getMinigameAttributes().getBarrowsMinigameAttributes()
        .setRiddleAnswer(riddles[riddle][7]);
    player.getPacketSender().sendInterface(4543);
  }

  public static void handlePuzzle(Player player, int puzzleButton) {
    if (puzzleButton == player.getMinigameAttributes().getBarrowsMinigameAttributes()
        .getRiddleAnswer()) {
      player.moveTo(new Position(3551, 9694));
      player.getPacketSender().sendMessage("You got the correct answer.");
      player.getPacketSender()
          .sendMessage("A magical force guides you to a chest located in the center room.");
    } else
  ...
  public static void searchCoffin(final Player player, final int obj, final int coffinId, int npcId,
    ...
    if (player.getMinigameAttributes().getBarrowsMinigameAttributes()
        .getBarrowsData()[coffinId][1] == 0) {
      if (player.getLocation() == Location.BARROWS) {
        ...
        npc_.forceChat(player.getPosition().getZ() == -1 ? "You dare disturb my rest!"
  ...
  public static void resetBarrows(Player player) {
    ...
    updateInterface(player);
    player.getMinigameAttributes().getBarrowsMinigameAttributes()
        .setRandomCoffin(getRandomCoffin());
  }

  public static final Object[][] data = {{"Verac The Defiled", 37203},
      {"Torag The Corrupted", 37205}, {"Karil The Tainted", 37207}, {"Guthan The Infested", 37206},
      {"Dharok The Wretched", 37202}, {"Ahrim The Blighted", 37204}};
  ...
  public static void killBarrowsNpc(Player player, NPC n, boolean killed) {
    if (player == null || n == null)
      return;
    if (n.getId() == 58) {
      player.getMinigameAttributes().getBarrowsMinigameAttributes().setKillcount(
          player.getMinigameAttributes().getBarrowsMinigameAttributes().getKillcount() + 1);
      updateInterface(player);
      return;
    }
    ...
    if (killed && player.getRegionInstance() != null) {
      player.getMinigameAttributes().getBarrowsMinigameAttributes()
          .getBarrowsData()[arrayIndex][1] = 2;
      player.getMinigameAttributes().getBarrowsMinigameAttributes().setKillcount(
          player.getMinigameAttributes().getBarrowsMinigameAttributes().getKillcount() + 1);
      player.getRegionInstance().getNpcsList().remove(player);
    } else if (arrayIndex >= 0)
      player.getMinigameAttributes().getBarrowsMinigameAttributes()
          .getBarrowsData()[arrayIndex][1] = 0;
    updateInterface(player);
  }
  ...
  public static void updateInterface(Player player) {
    for (int i = 0; i < data.length; i++) {
      boolean killed =
          player.getMinigameAttributes().getBarrowsMinigameAttributes().getBarrowsData()[i][1] == 2;
      String s = killed ? "@gre@" : "@red@";
      player.getPacketSender().sendString((int) data[i][1], "" + s + "" + (String) data[i][0]);
    }
    player.getPacketSender().sendString(37208, "Killcount: "
        ...
        if (player.getInventory().getSlot(items.getId()) > 0) {
          if (items.getId() == brokenBarrows[i][1]) {
            if (totalCost + 45000 > money) {
              breakLoop = true;
              player.getPacketSender()
                  .sendMessage("You need at least 45000 coins to fix this item.");
              break;
            } else {
              totalCost += 45000;
  ...
  public static int barrows[] = {4708, 4710, 4712, 4714, 4716, 4718, 4720, 4722, 4724, 4726, 4728,
          4730, 4732, 4734, 4736, 4738, 4745, 4747, 4749, 4751, 4753, 4755, 4757, 4759, 4708, 4710, 4712, 4714, 4716, 4718, 4720, 4722, 4724, 4726, 4728,
          4730, 4732, 4734, 4736, 4738, 4745, 4747, 4749, 4751, 4753, 4755, 4757, 4759, 11613};

  public static final int[][] brokenBarrows = {{4708, 4860}, {4710, 4866}, {4712, 4872},
      {4714, 4878}, {4716, 4884}, {4720, 4896}, {4718, 4890}, {4720, 4896}, {4722, 4902},
      {4732, 4932}, {4734, 4938}, {4736, 4944}, {4738, 4950}, {4724, 4908}, {4726, 4914},
      {4728, 4920}, {4730, 4926}, {4745, 4956}, {4747, 4926}, {4749, 4968}, {4751, 4994},
      {4753, 4980}, {4755, 4986}, {4757, 4992}, {4759, 4998}};

  public static final int[][] COFFIN_AND_BROTHERS =
      {{6823, 2030}, {6772, 2029}, {6822, 2028}, {6773, 2027}, {6771, 2026}, {6821, 2025}};
  ...
  public static final Position[] UNDERGROUND_SPAWNS = {new Position(3569, 9677),
      new Position(3535, 9677), new Position(3534, 9711), new Position(3569, 9712)};
