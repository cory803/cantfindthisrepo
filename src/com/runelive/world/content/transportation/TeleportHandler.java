...
public class TeleportHandler {
  ...
  public static void teleportPlayer(final Player player,
    ...
    if (!player.getClickDelay().elapsed(4500)
      ...
      return;
    player.setTeleporting(true).getMovementQueue().setLockMovement(true)
            .reset();
        ...
        switch (teleportType) {
          case LEVER:
            if (tick == 0)
              ...
              player.moveTo(targetLocation).setPosition(
                      targetLocation);
          ...
          default:
            if (tick == teleportType.getStartTick()) {
              ...
              if (Dungeoneering.doingDungeoneering(player)) {
                final Position dungEntrance = player
                        .getMinigameAttributes()
                        .getDungeoneeringAttributes().getParty()
                        .getDungeoneeringFloor().getEntrance()
                        .copy().setZ(player.getPosition().getZ());
                player.moveTo(dungEntrance).setPosition(
                        dungEntrance);
              } else {
                player.moveTo(targetLocation).setPosition(
                        targetLocation);
  ...
  public static boolean interfaceOpen(Player player) {
    if (player.getInterfaceId() > 0 && player.getInterfaceId() != 50100) {
      player.getPacketSender()
          .sendMessage("Please close the interface you have open before opening another.");
      return true;
    }
    return false;
  }
  ...
  public static boolean checkReqs(Player player, Position targetLocation,
    ...
    if (player.getLocation() == null) {
      player.getPacketSender().sendInterfaceRemoval();
      player.getPacketSender().sendMessage(
          "The location you have tryed to teleport to is null, please report @ :;support!");
      return false;
    }
