...
public class PunishmentCommand extends StaffCommand {
	...
	public boolean execute(Player player, String key, String input) throws Exception {
		...
		case "kick":
			if (punishee != null) {
				...
				if (punishee.getLocation() != Location.WILDERNESS) {
					...
					PlayerLogs.log(player.getUsername(), "" + player.getUsername() + " just kicked " + punishee.getUsername() + "!");
				}
			} else {
		...
		case "banvote":
			...
			player.getPacketSender().sendMessage("You have banned " + punishee.getUsername() + " from voting.");
			break;
		case "unbanvote":
			if (!PlayerPunishment.isVoteBanned(playerName)) {
			...
			player.getPacketSender().sendMessage("You have unbanned " + punishee.getUsername() + " from voting.");
			break;
		case "mute":
		...
		case "ipmute":
			...
			punishee.getPacketSender().sendMessage("You have been ip muted! Please appeal on the forums.");
			break;
		case "unipmute":
			if (!PlayerPunishment.isIpMuted(playerName)) {
		...
		case "ipban":
			if (punishee != null) {
				if (PlayerPunishment.isIpBanned(punishee.getHostAddress())) {
					player.getPacketSender().sendMessage("Player " + playerName + " already has an active ip ban on " + punishee.getHostAddress() + ".");
					return false;
				}
				PlayerPunishment.ipBan(punishee.getHostAddress());
				PlayerPunishment.ban(playerName);
				World.deregister(punishee);
				player.getPacketSender().sendMessage("Player " + playerName + " was successfully banned on ip " + punishee.getHostAddress() + "!");
			} else {
				player.getPacketSender().sendMessage("Player " + playerName + " not found.");
				return false;
			}
		...
		case "unipban":
			if (punishee != null) {
				if (!PlayerPunishment.isIpBanned(punishee.getHostAddress())) {
					player.getPacketSender().sendMessage("Player " + playerName + " does not have an active ip ban on " + punishee.getHostAddress() + ".");
					return false;
				}
				PlayerPunishment.unIpBan(punishee.getHostAddress());
				player.getPacketSender().sendMessage("Player " + playerName + " was successfully unipbanned on ip " + punishee.getHostAddress() + "!");
			} else {
				player.getPacketSender().sendMessage("Player " + playerName + " not found.");
				return false;
			}
		...
		case "cpuban":
			if (punishee != null) {
				if (PlayerPunishment.isMacBanned(punishee.getMacAddress())) {
					player.getPacketSender().sendMessage("Player " + playerName + " already has an active mac ban on " + punishee.getMacAddress() + ".");
					return false;
				}
		...
		case "uncpuban":
			if (punishee != null) {
				if (!PlayerPunishment.isMacBanned(punishee.getMacAddress())) {
					player.getPacketSender().sendMessage("Player " + playerName + " does not have an active mac ban on " + punishee.getMacAddress() + ".");
					return false;
				}
				PlayerPunishment.unMacBan(punishee.getMacAddress());
				player.getPacketSender().sendMessage("Player " + playerName + " was successfully un mac banned on mac " + punishee.getMacAddress() + "!");
			} else {
				player.getPacketSender().sendMessage("Player " + playerName + " not found.");
				return false;
			}
