...
public class Packet {
  ...
  public int readSingleInt() {
    byte firstByte = readByte(), secondByte = readByte(), thirdByte = readByte(),
        fourthByte = readByte();
  ...
  public int readDoubleInt() {
    int firstByte = readByte() & 0xFF, secondByte = readByte() & 0xFF,
        thirdByte = readByte() & 0xFF, fourthByte = readByte() & 0xFF;
  ...
  public int readSmart() {
    return buffer.getByte(buffer.readerIndex()) < 128 ? readByte() & 0xFF
        : (readShort() & 0xFFFF) - 32768;
  }
  ...
  public int readSignedSmart() {
    return buffer.getByte(buffer.readerIndex()) < 128 ? (readByte() & 0xFF) - 64
        : (readShort() & 0xFFFF) - 49152;
  }
