...
public class NPCOptionPacketListener implements PacketListener {
          ...
          public void execute() {
            ...
            switch (npc.getId()) {
              ...
              case 2305:
                ...
                } else if (player.getMinigameAttributes().getFarmQuestAttributes()
                    .getQuestParts() == 1) {
              ...
              case 4663:
                ...
                } else if (player.getMinigameAttributes().getClawQuestAttributes()
                    .getQuestParts() == 4) {
                  ...
                  if (player.getMinigameAttributes().getClawQuestAttributes().getSamples() >= player
                    ...
                    if (!player.getInventory().isFull()) {
                      ...
                      player.getPacketSender().sendMessage(
                          "Denath takes the samples and converts it into a bravery potion.");
                      player.getPacketSender().sendMessage(
              ...
              case 2127:
                if (!player.getRights().isStaff()) {
                  player.getPacketSender().sendMessage(
                      "Player Owned Shops are currently in beta for staff to test bugs.");
                  player.getPacketSender().sendMessage(
              ...
              case 291:
                if (player.getMinigameAttributes().getFarmQuestAttributes().getQuestParts() == 1) {
                  ...
                  player.getPacketSender()
                      .sendMessage("I don't need to pickpocket this farmer right now.");
                }
                break;
              case 2947:
                if (player.getMinigameAttributes().getClawQuestAttributes().getQuestParts() == 1
                  ...
                  player.getPacketSender()
                      .sendMessage("I cannot pick pocket this woman and I'm not sure why.");
                }
                break;
              case 4646:
                if (player.getMinigameAttributes().getClawQuestAttributes().getQuestParts() == 0) {
                  ...
                  if (player.getInventory().contains(691)) {
                    ...
                    DialogueManager.sendStatement(player,
                        "Show me the proof when you've killed the beast.");
                  }
                } else {
                  DialogueManager.sendStatement(player,
              ...
              case 3789:
                ...
                player.getPacketSender().sendString(18729, "Commendations: "
                    + Integer.toString(player.getPointsHandler().getCommendations()));
                break;
              case 2948:
  ...
  private static void attackNPC(Player player, Packet packet) {
    ...
    if (player.getZulrahRotatingProcess() && (interact.getId() == Zulrah.ZULRAH_GREEN_NPC_ID
        || interact.getId() == Zulrah.ZULRAH_RED_NPC_ID
        || interact.getId() == Zulrah.ZULRAH_BLUE_NPC_ID
            ...
            switch (npc.getId()) {
              case 4646:
                if (player.getMinigameAttributes().getClawQuestAttributes().getQuestParts() == 8) {
                  if (!player.getInventory().isFull()) {
                    ...
                    player.getPacketSender()
                        .sendMessage("You need 1 slot available for this teleport");
                  }
                } else if (player.getMinigameAttributes().getClawQuestAttributes()
                    .getQuestParts() == 10) {
                  player.moveTo(new Position(2660, 3306, 0));
              ...
              case 4375:
                if (player.getLastBoss() == 0) {
                  player.getPacketSender()
                    ...
                    || player.getClanChatName().equalsIgnoreCase("Ikov") && !player.isBossSolo()) {
                  player.getPacketSender().sendMessage(
                      "Your last boss spawn was in a clan, you currently are not in one!");
              ...
              case 8591:
                if (!player.getMinigameAttributes().getNomadAttributes().hasFinishedPart(1)) {
                  player.getPacketSender().sendMessage(
                      "You must complete Nomad's quest before being able to use this shop.");
                  return;
                }
          ...
          public void execute() {
            switch (npc.getId()) {
              ...
              case 961:
                if (player.getDonorRights() == 0) {
                  player.getPacketSender()
                      .sendMessage("This feature is currently only available for members.");
                  return;
                }
                ...
                // player.getPacketSender().sendMessage("Your special attack energy has been
                // restored.");
                // }
                ...
                if (restore) {
                  ...
                  player.getPacketSender()
                      .sendMessage("Your stats do not need to be restored at the moment.");
                break;
              case 705:
  ...
  public static final int ATTACK_NPC = 72, FIRST_CLICK_OPCODE = 155, MAGE_NPC = 131,
