...
import com.ikov.world.content.grandexchange.GrandExchangeSlotState;
  ...
  public static void handleLogin(Player player) {
    ...
    if (GameSettings.DOUBLE_EXP) {
      player.getPacketSender().sendMessage(
          "@bla@Welcome to Ikov! We're currently in Double EXP mode! (@red@X2.0@bla@)");
    } else if (GameSettings.INSANE_EXP) {
      player.getPacketSender().sendMessage(
          "@bla@Welcome to Ikov! We're currently in Insane EXP mode! (@red@X8.0@bla@)");
    } else {
      player.getPacketSender().sendMessage(
          "@bla@Welcome to Ikov! We're currently in Normal EXP mode! (@red@X1.0@bla@)");
    }
    ...
    if (GameSettings.DOUBLE_POINTS) {
      player.getPacketSender().sendMessage(
          "<img=4> <col=008FB2>Ikov currently has a double points event going on, make sure to use it!");
    }

    if (GameSettings.DOUBLE_VOTE_TOKENS) {
      player.getPacketSender().sendMessage(
          "<img=4> <col=008FB2>Ikov currently has a double vote rewards event going on, make sure to use it!");
    }

    if (GameSettings.TRIPLE_VOTE_TOKENS) {
      player.getPacketSender().sendMessage(
          "<img=4> <col=008FB2>Ikov currently has a triple vote rewards event going on, make sure to use it!");
    }
