...
public class GroundItemManager {
  ...
  /*
   * Our list which holds all grounditems, used an CopyOnWriteArrayList to prevent modification
   * issues TODO: Change into a queue of some sort
   */
  private static CopyOnWriteArrayList<GroundItem> groundItems =
      new CopyOnWriteArrayList<GroundItem>();
  ...
  public static void spawnGroundItem(Player p, GroundItem g) {
    ...
    if (ItemDefinition.forId(item.getId()).isStackable()) {
      ...
      if (it != null) {
        it.getItem()
            .setAmount(it.getItem().getAmount() + g.getItem().getAmount() > Integer.MAX_VALUE
    ...
    /*
     * if(Misc.getMinutesPlayed(p) < 15) { g.setGlobalStatus(false); g.setGoGlobal(false); }
     */
    add(g, true);
  }
  ...
  public static void add(GroundItem groundItem, boolean listGItem) {
    if (groundItem.isGlobal()) {
      for (Player p : World.getPlayers()) {
        if (p == null)
          continue;
        if (groundItem.getPosition().getZ() == p.getPosition().getZ()
            && p.getPosition().distanceToPoint(groundItem.getPosition().getX(),
                groundItem.getPosition().getY()) <= 120)
          p.getPacketSender().createGroundItem(groundItem.getItem().getId(),
              groundItem.getPosition().getX(), groundItem.getPosition().getY(),
              groundItem.getItem().getAmount());
      }
    } else {
      Player person = World.getPlayerByName(groundItem.getOwner());
      if (person != null && groundItem.getPosition().getZ() == person.getPosition().getZ()
          && person.getPosition().distanceToPoint(groundItem.getPosition().getX(),
              groundItem.getPosition().getY()) <= 120)
        person.getPacketSender().createGroundItem(groundItem.getItem().getId(),
            groundItem.getPosition().getX(), groundItem.getPosition().getY(),
            groundItem.getItem().getAmount());
    }
    if (listGItem) {
  ...
  public static void pickupGroundItem(Player p, Item item, Position position) {
    ...
    if (gt == null || gt.hasBeenPickedUp() || !groundItems.contains(gt)) // last one isn't needed,
                                                                         // but hey, just trying to
                                                                         // be safe
      return;
    else {
      if (p.getGameMode() != GameMode.NORMAL && !Dungeoneering.doingDungeoneering(p)) {
        if ((gt.getOwner() != null && !gt.getOwner().equals("null")
            && !gt.getOwner().equals(p.getUsername()))
            || (gt.getIronman() && p.getGameMode() == GameMode.IRONMAN)) {
          p.getPacketSender()
              .sendMessage("You cannot pick this item up because it was not spawned for you.");
          return;
        }
      }
      ...
      PlayerLogs.log(p.getUsername(),
          "Player looting item: " + item.getDefinition().getName() + " (" + item.getId()
              + "), amount: " + item.getAmount() + " Owner: " + gt.getOwner() + "");
    }
  }

  /**
   * Handles a region change for a player. This method respawns all grounditems for a player who has
   * changed region.
   * 
  ...
  public static void handleRegionChange(Player p) {
    for (GroundItem gi : groundItems) {
      if (gi == null)
        continue;
      p.getPacketSender().removeGroundItem(gi.getItem().getId(), gi.getPosition().getX(),
          gi.getPosition().getY(), gi.getItem().getAmount());
    }
    for (GroundItem gi : groundItems) {
      if (gi == null || p.getPosition().getZ() != gi.getPosition().getZ() || p.getPosition()
          .distanceToPoint(gi.getPosition().getX(), gi.getPosition().getY()) > 120)
        continue;
