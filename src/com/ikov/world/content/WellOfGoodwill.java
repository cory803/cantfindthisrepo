  ...
  public static void donate(Player player, int amount) {
    ...
    if (!usePouch && player.getInventory().getAmount(995) < amount) {
      DialogueManager.sendStatement(player,
          "You do not have that much money in your inventory or money pouch.");
      return;
    }
    if (usePouch) {
    ...
    if (getMissingAmount() <= 0) {
      ...
      World.sendMessage(
          "<img=4> <col=6666FF>It is now granting everyone 2 hours of 30% bonus experience.");
      // World.getPlayers().forEach(p -> p.getPacketSender().sendString(39163, "@or2@Well of
      // Goodwill: @yel@Active"));
    }
  }
  ...
  public static void updateState() {
    if (STATE == WellState.FULL) {
      if (getMinutesRemaining() <= 0) {
        World.sendMessage(
            "<img=4> <col=6666FF>The Well of Goodwill is no longer granting bonus experience.");
        World.getPlayers()
