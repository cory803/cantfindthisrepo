...
public class MoneyPouch {
	...
	public static boolean depositMoney(Player plr, int amount) {
		...
		if (validateAmount(plr, amount)) {
			...
			if (addedMoney > Long.MAX_VALUE) {
				...
				plr.getPacketSender().sendString(8135, ""+plr.getMoneyInPouch());
				plr.getPacketSender().sendMessage("You've added "+currencyFormatter.format(canStore)+" coins to your money pouch.");
				return true;
			} else {
				plr.getInventory().delete(995, amount);
				plr.setMoneyInPouch(plr.getMoneyInPouch() + amount);
				plr.getPacketSender().sendString(8135, ""+plr.getMoneyInPouch());
				plr.getPacketSender().sendMessage("You've added "+currencyFormatter.format(amount)+" coins to your money pouch.");
				return true;
			}
		} else {
			plr.getPacketSender().sendMessage("You do not seem to have " +currencyFormatter.format(amount)+" coins in your inventory.");
			return false;
		}
	}
	...
	public static boolean withdrawMoney(Player plr, long amount) {
		...
		if(plr.getLocation() == Location.DUNGEONEERING) {
			plr.getPacketSender().sendMessage("You cannot withdraw from your pouch in dungeoneering. You will lose this money.");
			return false;
		}
...
//		Disabled ^ because players would be able to add more than max stack into trade/stake
		...
		if ((plr.getInventory().getAmount(995) + amount) < Integer.MAX_VALUE) {
			...
			plr.getPacketSender().sendString(8135, ""+plr.getMoneyInPouch());
			plr.getPacketSender().sendMessage("You could only withdraw "+currencyFormatter.format(canWithdraw)+" coins.");
			if(allowWithdraw)
				plr.getPacketSender().sendItemContainer(plr.getInventory(), 3322);
			return true;
		}
