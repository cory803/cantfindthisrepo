...
public class ItemDegrading {
    ...
    public static boolean handleItemDegrading(Player p, DegradingItem d) {
        ...
        if (equipId == d.nonDeg || equipId == d.deg) {
            ...
            p.getEquipment()
                    .setItem(d.equipSlot,
                            new Item(degradeCompletely ? -1 : d.deg))
                    .refreshItems();
            p.getUpdateFlag().flag(Flag.APPEARANCE);
            String ext = !degradeCompletely ? "degraded slightly"
                    : "turned into dust";
            p.getPacketSender().sendMessage(
                    "Your "
                            + ItemDefinition.forId(equipId).getName()
                            .replace(" (deg)", "") + " has " + ext
                            + "!");
  ...
  public enum DegradingItem {
    ...
    STATIUS_FULL_HELM(13896, 13898, Equipment.HEAD_SLOT, 600), STATIUS_PLATEBODY(
            13884, 13886, Equipment.BODY_SLOT, 600), STATIUS_PLATELEGS(
            13890, 13892, Equipment.LEG_SLOT, 600), STATIUS_WARHAMMER(
            13902, 13904, Equipment.WEAPON_SLOT, 600),

    VESTAS_CHAINBODY(13887, 13889, Equipment.BODY_SLOT, 600), VESTAS_PLATESKIRT(
            13893, 13895, Equipment.LEG_SLOT, 600), VESTAS_LONGSWORD(13899,
            13901, Equipment.WEAPON_SLOT, 600), VESTAS_SPEAR(13905, 13907,
            Equipment.WEAPON_SLOT, 600),

    ZURIELS_HOOD(13864, 13866, Equipment.HEAD_SLOT, 600), ZURIELS_ROBE_TOP(
            13858, 13860, Equipment.BODY_SLOT, 600), ZURIELS_ROBE_BOTTOM(
            13861, 13863, Equipment.LEG_SLOT, 600), ZURIELS_STAFF(13867,
            13869, Equipment.WEAPON_SLOT, 600),

    MORRIGANS_COIF(13876, 13878, Equipment.HEAD_SLOT, 600), MORRIGANS_LEATHER_BODY(
            13870, 13872, Equipment.BODY_SLOT, 600), MORRIGANS_LEATHER_CHAPS(
            13873, 13875, Equipment.LEG_SLOT, 600),

    // Statius's equipment

    CORRUPT_STATIUS_FULL_HELM(13920, 13922, Equipment.HEAD_SLOT, 200), CORRUPT_STATIUS_PLATEBODY(
            13908, 13910, Equipment.BODY_SLOT, 200), CORRUPT_STATIUS_PLATELEGS(13914, 13916,
            Equipment.LEG_SLOT,
    ...
    CORRUPT_VESTAS_CHAINBODY(13911, 13913, Equipment.BODY_SLOT, 200), CORRUPT_VESTAS_PLATESKIRT(
            13917, 13919, Equipment.LEG_SLOT, 200), CORRUPT_VESTAS_LONGSWORD(13923, 13925,
    ...
    CORRUPT_ZURIELS_HOOD(13938, 13940, Equipment.HEAD_SLOT, 200), CORRUPT_ZURIELS_ROBE_TOP(13932,
            13934, Equipment.BODY_SLOT, 200), CORRUPT_ZURIELS_ROBE_BOTTOM(13935, 13937,
            Equipment.LEG_SLOT,
