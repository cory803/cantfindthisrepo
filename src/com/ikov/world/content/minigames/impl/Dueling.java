...
public class Dueling {
  ...
  public void challengePlayer(Player playerToDuel) {
    ...
    if (duelingStatus == 0 && challenged && duelingWith == playerToDuel.getIndex()
      ...
      if (duelingStatus == 0) {
        ...
        player.getPacketSender()
            .sendMessage("You must decline this duel before accepting another one!");
      }
    } else if (duelingStatus == 0) {
      playerToDuel.getPacketSender().sendMessage(player.getUsername() + ":duelreq:");
      player.getPacketSender()
          .sendMessage("You have sent a duel request to " + playerToDuel.getUsername() + ".");
    }
  }
  ...
  public void stakeItem(int itemId, int amount, int slot) {
    if (slot < 0)
      return;
    /*
     * if(Misc.getMinutesPlayed(player) < 15) { player.getPacketSender().sendMessage(
     * "You must have played for at least 15 minutes in order to stake items."); return; }
     */
    if (!getCanOffer())
      return;
    resetAcceptedStake();
    ...
    if (player.getRights() != PlayerRights.OWNER && playerToDuel.getRights() != PlayerRights.OWNER
      ...
      if (!new Item(itemId).tradeable()) {
        player.getPacketSender()
            .sendMessage("This item is currently untradeable and cannot be traded.");
        return;
      }
    }
    ...
    if (player.getGameMode() == GameMode.HARDCORE_IRONMAN) {
      player.getPacketSender()
          .sendMessage("You're dumb for trying to trade a Hardcore Ironman. -Sam");
      return;
    }
    ...
    if (playerToDuel.getGameMode() == GameMode.HARDCORE_IRONMAN) {
      player.getPacketSender()
          .sendMessage("You're dumb for trying to stake a Hardcore Ironman. -Sam");
      return;
    }
  ...
  public void removeStakedItem(int itemId, int amount) {
    ...
    /*
     * if (Item.itemStackable[itemID]) { if (playerToDuel.getInventory().getFreeSlots() - 1 <
     * (c.duelSpaceReq)) { c.sendMessage("You have too many rules set to remove that item."); return
     * false; } }
     */
    player.getPacketSender().sendClientRightClickRemoval();
    if (!ItemDefinition.forId(itemId).isStackable()) {
    ...
    playerToDuel.getPacketSender().sendMessage("@red@Your opponent has removed an item from the stake please review the staked items.");
    player.getPacketSender().sendInterfaceItems(6670, playerToDuel.getDueling().stakedItems);
    player.getPacketSender().sendInterfaceItems(6669, player.getDueling().stakedItems);
    playerToDuel.getPacketSender().sendInterfaceItems(6670, player.getDueling().stakedItems);
    playerToDuel.getPacketSender().sendString(6684, "");
  ...
  public void selectRule(DuelRule duelRule) {
    ...
    if (duelRule == DuelRule.NO_SHIELD) {
      if (player.getEquipment().getItems()[Equipment.WEAPON_SLOT].getId() > 0
          && ItemDefinition.forId(player.getEquipment().getItems()[Equipment.WEAPON_SLOT].getId())
              .isTwoHanded()
          || ItemDefinition
              .forId(playerToDuel.getEquipment().getItems()[Equipment.WEAPON_SLOT].getId())
              .isTwoHanded())
        slotOccupied = true;
    }
    ...
    if (!alreadySet && player.getInventory().getFreeSlots() < spaceRequired) {
      player.getPacketSender()
          .sendMessage("You do not have enough free inventory space to set this rule.");
      return;
    }
    if (!alreadySet && playerToDuel.getInventory().getFreeSlots() < spaceRequired) {
      player.getPacketSender().sendMessage("" + playerToDuel.getUsername()
    ...
    playerToDuel.getDueling().selectedDuelRules[index] =
        player.getDueling().selectedDuelRules[index];
  ...
  public static boolean twoDuelers(Player p1, Player p2) {
    ...
    for (Player player : World.getPlayers()) {
      if (player == null)
        continue;
      if (player.getDueling().inDuelWith == p1.getIndex()
          || player.getDueling().inDuelWith == p2.getIndex()) {
        count++;
      }
    }
    return count == 2;
  ...
  public void confirmDuel() {
    ...
    if (playerToDuel == null)
      ...
      if (!twoDuelers(player, playerToDuel)) {
        player.getPacketSender()
            .sendMessage("An error has occured. Please try requesting a new duel.");
        return;
      }
    }
    String itemId = "";
      ...
      switch (button) {
        case 6674:
          ...
          if (player.getDueling().selectedDuelRules[DuelRule.NO_MELEE.ordinal()]
            ...
            player.getPacketSender().sendMessage(
                "You won't be able to attack the other player with the current rules.");
            return true;
          }
          ...
          if (player.getDueling().duelingStatus == 2
              && playerToDuel.getDueling().duelingStatus == 2) {
        ...
        case 6520:
          ...
          if (playerToDuel.getDueling().duelingStatus == 4
              && player.getDueling().duelingStatus == 4) {
  ...
  public void startDuel() {
    ...
    if (selectedDuelRules[DuelRule.NO_WEAPON.ordinal()]
        || selectedDuelRules[DuelRule.NO_SHIELD.ordinal()]) {
      if (player.getEquipment().getItems()[Equipment.WEAPON_SLOT].getId() > 0) {
  ...
  public static boolean checkDuel(Player playerToDuel, int statusReq) {
    boolean goodInterfaceId = playerToDuel.getInterfaceId() == -1
        || playerToDuel.getInterfaceId() == 6575 || playerToDuel.getInterfaceId() == 6412;
    if (playerToDuel.getDueling().duelingStatus != statusReq || playerToDuel.isBanking()
        || playerToDuel.isShopping() || playerToDuel.getConstitution() <= 0
        || playerToDuel.isResting() || !goodInterfaceId)
      return false;
    return true;
  }
  ...
  public boolean getCanOffer() {
    return canOffer && player.getInterfaceId() == 6575 && !player.isBanking()
  ...
  public static enum DuelRule {
    NO_RANGED(16, 6725, -1, -1), NO_MELEE(32, 6726, -1, -1), NO_MAGIC(64, 6727, -1,
        -1), NO_SPECIAL_ATTACKS(8192, 7816, -1, -1), LOCK_WEAPON(4096, 670, -1, -1), NO_FORFEIT(1,
            6721, -1, -1), NO_POTIONS(128, 6728, -1, -1), NO_FOOD(256, 6729, -1, -1), NO_PRAYER(512,
                6730, -1, -1), NO_MOVEMENT(2, 6722, -1, -1), OBSTACLES(1024, 6732, -1, -1),

    NO_HELM(/* 16384 */643, 13813, 1, Equipment.HEAD_SLOT), NO_CAPE(/* 32768 */644, 13814, 1,
        Equipment.CAPE_SLOT), NO_AMULET(/* 65536 */645, 13815, 1,
            Equipment.AMULET_SLOT), NO_AMMUNITION(/* 134217728 */656, 13816, 1,
                Equipment.AMMUNITION_SLOT), NO_WEAPON(/* 131072 */646, 13817, 1,
                        ...
                        Equipment.BODY_SLOT), NO_SHIELD(/* 524288 */648, 13819, 1,
                            Equipment.SHIELD_SLOT), NO_LEGS(/* 2097152 */650, 13820, 1,
                                Equipment.LEG_SLOT), NO_RING(/* 67108864 */655, 13821, 1,
                                    Equipment.RING_SLOT), NO_BOOTS(/* 16777216 */653, 13822, 1,
                                        Equipment.FEET_SLOT), NO_GLOVES(/* 8388608 */652, 13823, 1,
