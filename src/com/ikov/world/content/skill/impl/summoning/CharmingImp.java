...
public class CharmingImp {
		...
		player.getPacketSender()
				...
				.sendMessage(
						"<img=5> <col=996633>Your configuration for "
								+ ItemDefinition.forId(getCharmForIndex(index))
								.getName() + "s has been saved.");*/
	...
	private static boolean sendToInvo(Player player, int itemId, int amount) {
		if (!player.getInventory().contains(itemId)
			...
			player.getPacketSender()
					.sendMessage(
							"Your inventory is full, the Charming imp is unable to pick up any charms!");
			return false;
		}
	...
	private static void sendMessage(Player player, int config, int itemId,
									int amount) {
		...
		switch (config) {
			case 0:
				//player.getPacketSender().sendMessage("Your charming imp loots your charms and grants you some experience too!");
			...
			switch (state) {
				case 0:
					player.getPacketSender().sendMessage(
							"<img=5> <col=996633>Your Charming imp is placing all "
									+ ItemDefinition.forId(charm).getName()
									+ "s it finds in your inventory.");
					break;
				case 1:
					player.getPacketSender().sendMessage(
							"<img=5> <col=996633>Your Charming imp is turning all "
									+ ItemDefinition.forId(charm).getName()
									+ "s it finds into Summoning exp.");
					break;
			}
		}
	}
