...
import com.ikov.world.content.Emotes.Skillcape_Data;

/**
 * The summoning skill is based upon creating pouches that contain certain 'familiars', which you
 * can then summon and use their abilities as a form of 'assistance'.
 * 
...
public class Summoning {
    ...
    if (login || player.getSkillManager().getMaxLevel(Skill.SUMMONING) >= familiar.levelRequired) {
      ...
      if (!login) {
        if (player.getSkillManager()
            .getCurrentLevel(Skill.SUMMONING) >= familiar.summoningPointsRequired) {
          ...
          if (renew && getFamiliar() != null) {
            player.getPacketSender().sendMessage(
                    "You have renewed your familiar.");
            getFamiliar().setDeathTimer((int)
                    (SummoningData.getFollowerTimer(familiar.npcId) * (Skillcape_Data.SUMMONING.isWearingCape(player) || Skillcape_Data.MASTER_SUMMONING.isWearingCape(player) ? 1.15 : 1)));
            getFamiliar().getSummonNpc().performGraphic(
                    new Graphic(1315));
            getFamiliar().getSummonNpc().setConstitution(
          ...
          player.getPacketSender()
                  .sendMessage(
                          "You do not have enough Summoning points to summon this familiar.");
          player.getPacketSender()
                  .sendMessage(
                          "You can recharge your Summoning points at an obelisk.");
          return;
        }
      }
      ...
      player.getPacketSender().sendNpcHeadOnInterface(
          player.getSummoning().getFamiliar().getSummonNpc().getId(), 54021); // 60 = invisable head
                                                                              // to remove it
      player.getPacketSender().sendString(0, "[SUMMOtrue");

      player.getLastSummon().reset();
    } else {
      player.getPacketSender().sendMessage("You need a Summoning level of at least "
          + familiar.levelRequired + " to summon this familiar.");
    }
  }
        ...
        if (!familiar.isPet()) {
          if (clockTimer >= 2) {
            if (familiar.getDeathTimer() > 0) {
              ...
              player.getPacketSender().sendString(54043, "" + Summoning
                  .getTimer(familiar.getDeathTimer() > 0 ? familiar.getDeathTimer() : 0));
  ...
  public void toInventory() {
    ...
    if ((!player.busy() || player.getInterfaceId() == BeastOfBurden.INTERFACE_ID)
      ...
      if (getFamiliar() == null
          || !SummoningData.beastOfBurden(getFamiliar().getSummonNpc().getId()) || bob == null) {
        player.getPacketSender().sendMessage("You do not have a Beast of Burden.");
        return;
      }
      if (bob.getValidItems().size() == 0) {
        player.getPacketSender()
            .sendMessage("Your familiar is not currently holding any items for you.");
        return;
      }
  ...
  public void moveFollower(boolean forced) {
    if (getFamiliar() != null && getFamiliar().getSummonNpc() != null) {
      final Position movePos = new Position(player.getPosition().getX(),
          player.getPosition().getY() + 1, player.getPosition().getZ());
      if (forced || canSpawn(getFamiliar().getSummonNpc(), movePos)) {
        ...
        getFamiliar().getSummonNpc()
            .setLocation(Location.getLocation(getFamiliar().getSummonNpc()));
        player.getLastSummon().reset();
        if (!forced) {
