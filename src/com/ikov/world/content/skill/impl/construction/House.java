...
public class House extends RegionInstance {
  ...
  public void process() {
    House house = player.getRegionInstance().getType() == RegionInstanceType.CONSTRUCTION_HOUSE
        ? (House) player.getRegionInstance()
        : ((HouseDungeon) player.getRegionInstance()).getHouse();
      ...
      if (player.getPosition().getX() == converted[0] && player.getPosition().getY() == converted[1]
        ...
        for (HouseFurniture pf_ : house.getOwner().getHouseFurniture()) {
          if (pf_.getRoomX() == myTiles[0] && pf_.getRoomY() == myTiles[1]
              && pf_.getHotSpotId() == 91) {
            int[] coords = Construction.getConvertedCoords(pf_.getStandardXOff(),
                pf_.getStandardYOff(), myTiles, r);
  ...
  /*
   * public void playerKilled(Player p) { int[] myTiles = Construction.getMyChunk(p);
   * if(p.combatRingType > 0) { p.getPacketSender().sendInteractionOption("null", 3, true);
   * p.combatRingType = 0; p.moveTo(new Position(ConstructionData.BASE_X+(myTiles[0]*8)+1,
   * ConstructionData.BASE_Y+(myTiles[1]*8)+1, p.getPosition().getZ())); } else { PlayerFurniture
   * portal = Construction.findNearestPortal(p); int toX =
   * ConstructionData.BASE_X+((portal.getRoomX()+1)*8); int toY =
   * ConstructionData.BASE_Y+((portal.getRoomY()+1)*8); p.moveTo(new Position(toX+2, toY+2, 0)); } }
   */
