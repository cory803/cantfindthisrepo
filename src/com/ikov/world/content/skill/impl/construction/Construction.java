...
public class Construction {
    ...
    TaskManager.submit(new Task(1, me, true) {
      ...
      public void execute() {
        ...
        switch (ticks) {
          case 1:
            me.getPacketSender()
                .constructMapRegion(((House) houseOwner.getRegionInstance()).getPalette());
            break;
          case 2:
            ...
            if (me.getConstructionCoords() != null) {
              me.moveTo(
                  new Position(me.getConstructionCoords()[0], me.getConstructionCoords()[1], 0));
      ...
      for (int x = 0; x < 13; x++) {
        for (int y = 0; y < 13; y++) {
          ...
          if (p.getHouseRooms()[z][x][y].getX() == 0)
            continue;
          PaletteTile tile =
              new PaletteTile(p.getHouseRooms()[z][x][y].getX(), p.getHouseRooms()[z][x][y].getY(),
                  p.getHouseRooms()[z][x][y].getZ(), p.getHouseRooms()[z][x][y].getRotation());
    ...
    for (int x = 0; x < 13; x++) {
      for (int y = 0; y < 13; y++) {
        ...
        if (p.getHouseRooms()[4][x][y] == null) {
          tile = new PaletteTile(ConstructionData.RoomData.DUNGEON_EMPTY.getX(),
              ConstructionData.RoomData.DUNGEON_EMPTY.getY(), 0, 0);
        } else {
          tile =
              new PaletteTile(p.getHouseRooms()[4][x][y].getX(), p.getHouseRooms()[4][x][y].getY(),
  ...
  public static void placeNPCs(Player p) {
    ...
    for (HouseFurniture pf : p.getHouseFurniture()) {
      ...
      HotSpots hs = HotSpots.forHotSpotIdAndCoords(pf.getHotSpotId(), pf.getStandardXOff(),
          pf.getStandardYOff(), room);
      int actualX = ConstructionData.BASE_X + (pf.getRoomX() + 1) * 8;
      actualX +=
          ConstructionData.getXOffsetForObjectId(pf.getFurnitureId(), hs, room.getRotation());
      int actualY = ConstructionData.BASE_Y + (pf.getRoomY() + 1) * 8;
      actualY +=
          ConstructionData.getYOffsetForObjectId(pf.getFurnitureId(), hs, room.getRotation());
  ...
  public static void placeAllFurniture(Player p, int x, int y, int z) {
    ...
    for (HouseFurniture pf : owner.getHouseFurniture()) {
      ...
      doFurniturePlace(hs, f, hsses, getMyChunkFor(actualX, actualY), actualX, actualY,
          room.getRotation(), p, false, z);
    }
  }
  ...
  public static void placeAllFurniture(Player p, int heightLevel) {
    ...
    for (HouseFurniture pf : owner.getHouseFurniture()) {
      ...
      ArrayList<HotSpots> hsses = HotSpots.forObjectId_3(f.getHotSpotId());
      doFurniturePlace(hs, f, hsses, getMyChunkFor(actualX, actualY), actualX, actualY,
          room.getRotation(), p, false, heightLevel);
    }
  }

  public static void doFurniturePlace(HotSpots s, Furniture f, ArrayList<HotSpots> hsses,
      int[] myTiles, int actualX, int actualY, int roomRot, Player p, boolean placeBack,
      int height) {
    int portalId = -1;
    ...
    if (s.getHotSpotId() == 92) {
      ...
      int type = 22, leftObject = 0, rightObject = 0, upperObject = 0, downObject = 0,
          middleObject = 0, veryMiddleObject = 0, cornerObject = 0;
      ...
      p.getPacketSender().sendObject_cons(actualX, actualY, placeBack ? 15348 : cornerObject, 1,
          type, height);
      p.getPacketSender().sendObject_cons(actualX, actualY + 1, placeBack ? 15348 : leftObject, 1,
          type, height);
      p.getPacketSender().sendObject_cons(actualX, actualY + 2, placeBack ? 15348 : leftObject, 1,
          type, height);
      p.getPacketSender().sendObject_cons(actualX, actualY + 3, placeBack ? 15348 : cornerObject, 2,
          type, height);
      p.getPacketSender().sendObject_cons(actualX + 1, actualY + 3, placeBack ? 15348 : upperObject,
          2, type, height);
      p.getPacketSender().sendObject_cons(actualX + 2, actualY + 3, placeBack ? 15348 : upperObject,
          2, type, height);
      p.getPacketSender().sendObject_cons(actualX + 3, actualY + 3,
          placeBack ? 15348 : cornerObject, 3, type, height);
      p.getPacketSender().sendObject_cons(actualX + 3, actualY + 2, placeBack ? 15348 : rightObject,
          3, type, height);
      p.getPacketSender().sendObject_cons(actualX + 3, actualY + 1, placeBack ? 15348 : rightObject,
          3, type, height);
      p.getPacketSender().sendObject_cons(actualX + 3, actualY, placeBack ? 15348 : cornerObject, 0,
          type, height);
      p.getPacketSender().sendObject_cons(actualX + 2, actualY, placeBack ? 15348 : downObject, 0,
          type, height);
      p.getPacketSender().sendObject_cons(actualX + 1, actualY, placeBack ? 15348 : downObject, 0,
          type, height);
      ...
      if (veryMiddleObject != 0)
        p.getPacketSender().sendObject_cons(actualX + 1, actualY + 2, veryMiddleObject, 0, 10,
            height);
      ...
      p.getPacketSender().sendObject_cons(actualX, actualY + 1,
          placeBack ? 15352 : f.getFurnitureId(), 0, 0, height);
      p.getPacketSender().sendObject_cons(actualX, actualY, placeBack ? 15352 : f.getFurnitureId(),
          3, 2, height);
      ...
      // south walls
      p.getPacketSender().sendObject_cons(actualX, actualY, placeBack ? 15369 : f.getFurnitureId(),
          3, 2, height);
      p.getPacketSender().sendObject_cons(actualX + 1, actualY,
      ...
      p.getPacketSender().sendObject_cons(actualX, actualY, placeBack ? 15277 : combatringStrings,
          1, 3, height);
      p.getPacketSender().sendObject_cons(actualX, actualY + 4,
      ...
      if (f.getFurnitureId() == 13145) {
        p.getPacketSender().sendObject_cons(actualX + 1, actualY + 1, placeBack ? 6951 : 13145, 0,
            0, height);
        p.getPacketSender().sendObject_cons(actualX + 2, actualY + 1, placeBack ? 6951 : 13145, 0,
            0, height);
        p.getPacketSender().sendObject_cons(actualX + 1, actualY, placeBack ? 6951 : 13145, 1, 0,
            height);
        p.getPacketSender().sendObject_cons(actualX + 1, actualY + 2, placeBack ? 6951 : 13145, 3,
            0, height);
        if (!placeBack)
          p.getPacketSender().sendObject_cons(actualX + 1, actualY + 1, 13147, 0, 22, height);

        p.getPacketSender().sendObject_cons(actualX + 3, actualY + 3, placeBack ? 6951 : 13145, 0,
            0, height);
        p.getPacketSender().sendObject_cons(actualX + 4, actualY + 3, placeBack ? 6951 : 13145, 0,
            0, height);
        p.getPacketSender().sendObject_cons(actualX + 3, actualY + 2, placeBack ? 6951 : 13145, 1,
            0, height);
        p.getPacketSender().sendObject_cons(actualX + 3, actualY + 4, placeBack ? 6951 : 13145, 3,
            0, height);
        if (!placeBack)
      ...
      for (int x = 0; x < s.getCarpetDim().getWidth() + 1; x++) {
        for (int y = 0; y < s.getCarpetDim().getHeight() + 1; y++) {
          ...
          int offsetY = ConstructionData.BASE_Y + (myTiles[1] * 8);
          offsetX += ConstructionData.getXOffsetForObjectId(f.getFurnitureId(),
              s.getXOffset() + x - 1, s.getYOffset() + y - 1, roomRot, s.getRotation(roomRot));
          offsetY += ConstructionData.getYOffsetForObjectId(f.getFurnitureId(),
              s.getXOffset() + x - 1, s.getYOffset() + y - 1, roomRot, s.getRotation(roomRot));
          if (isEdge)
            p.getPacketSender().sendObject_cons(offsetX, offsetY,
  ...
  public static void handleFifthObjectClick(int obX, int obY, int objectId, Player p) {
    switch (objectId) {
      ...
      case 13505:
        ...
        Room room =
            p.getRegionInstance().getOwner().getHouseRooms()[1][myTiles[0] - 1][myTiles[1] - 1];
        if (room == null) {
    ...
    if (f.getAdditionalSkillRequirements() != null) {
      for (int ii = 0; ii < f.getAdditionalSkillRequirements().length; ii++) {
        if (p.getSkillManager()
            .getCurrentLevel(Skill.forId(f.getAdditionalSkillRequirements()[ii][0])) < f
                .getAdditionalSkillRequirements()[ii][1]) {
          return "You need a "
              + Skill.forId(f.getAdditionalSkillRequirements()[ii][0]).getFormatName()
              + " of at least " + f.getAdditionalSkillRequirements()[ii][1] + ""
              + " to build this.";
        }
      }
    }
    ...
    switch (buttonId) {
      ...
      case 2471:
        ...
        if (p.getDialogueActionId() == 442) {
          ...
          if (p.getHouseRooms()[0][0][0] == null) {
            p.getPacketSender()
                .sendMessage("You don't own a house. Talk to the House Agent to buy one.");
            return true;
          }
      ...
      case 2482:
        ...
        if (p.getDialogueActionId() == 644) {
          ...
          p.moveTo(new Position(ConstructionConstants.EDGEVILLE_X,
              ConstructionConstants.EDGEVILLE_Y, 0));
          return true;
        }
        break;
      case 2483:
        ...
        if (p.getDialogueActionId() == 644) {
          p.getRegionInstance().remove(p);
          p.moveTo(
              new Position(ConstructionConstants.KARAMJA_X, ConstructionConstants.KARAMJA_Y, 0));
          return true;
        }
        break;
      case 2484:
        ...
        if (p.getDialogueActionId() == 644) {
          p.getRegionInstance().remove(p);
          p.moveTo(new Position(ConstructionConstants.DRAYNOR.getX(),
              ConstructionConstants.DRAYNOR.getY(), 0));
          return true;
        }
        break;
      case 2485:
        ...
        if (p.getDialogueActionId() == 644) {
          p.getRegionInstance().remove(p);
          p.moveTo(new Position(ConstructionConstants.AL_KHARID_X,
              ConstructionConstants.AL_KHARID_Y, 0));
          return true;
        }
        break;
      case 2494:
      ...
      case 2461:
        ...
        if (p.getDialogueActionId() == 444) {
          ...
          if (roomCount < 2) {
            DialogueManager.start(p,
                ConstructionDialogues.hireServantDeclineDialogue(p, b.getNpcId(), "room"));
            return true;
          }
          DialogueManager.start(p,
              ConstructionDialogues.hireServantMakeDealDialogue(p, b.getNpcId()));
          return true;
        }
        ...
        if (p.getDialogueActionId() == 419) {
          ...
          int myTiles[] = getMyChunk(p);
          Room room =
              p.getRegionInstance().getOwner().getHouseRooms()[4][myTiles[0] - 1][myTiles[1] - 1];
          if (room != null) {
        ...
        if (p.getDialogueActionId() == 438) {
          ...
          if (room != null) {
            p.getPacketSender().sendMessage(
                "You did something retarded and now there is a under you for some reason.");
            p.getPacketSender().sendInterfaceRemoval();
          } else {
        ...
        if (p.getDialogueActionId() == 416) {
          ...
          int myTiles[] = getMyChunk(p);
          Room room =
              p.getRegionInstance().getOwner().getHouseRooms()[0][myTiles[0] - 1][myTiles[1] - 1];
          Room room_1 =
              p.getRegionInstance().getOwner().getHouseRooms()[1][myTiles[0] - 1][myTiles[1] - 1];
          if (room.getType() != ConstructionData.EMPTY) {
            ...
            createRoom(room_1.getType() == ConstructionData.SKILL_HALL_DOWN
                ? ConstructionData.SKILL_ROOM : ConstructionData.QUEST_ROOM, p, 101);
            p.getPacketSender().sendInterfaceRemoval();
          }
          return true;
        }
        ...
        if (p.getDialogueActionId() == 414) {
          ...
          if (room != null) {
            ...
            createRoom(room_1.getType() == ConstructionData.SKILL_ROOM
                ? ConstructionData.SKILL_HALL_DOWN : ConstructionData.QUEST_HALL_DOWN, p, 100);
            p.getPacketSender().sendInterfaceRemoval();
          }
          return true;
        }
  ...
  public static boolean handleRemoveClick(int obX, int obY, int objectId, Player p) {
    ...
    if (objectId == 13126 || objectId == 13127 || objectId == 13128 || objectId == 13132
        || objectId == 13133 || objectId == 13134 || objectId == 13135 || objectId == 13136
        || objectId == 13137 || objectId == 13138 || objectId == 13139 || objectId == 13140
        || objectId == 13145 || objectId == 13147 || objectId == 13142 || objectId == 13143
        || objectId == 13144) {
  ...
  public static void createRoom(int roomType, Player p, int toHeight) {
    ...
    if (toHeight == 100) {
      ...
      for (HouseFurniture furn : p.getHouseFurniture()) {
        if (furn.getRoomX() == myTiles[0] - 1 && furn.getRoomY() == myTiles[1] - 1
            && furn.getRoomZ() == 0) {
      ...
      doFurniturePlace(HotSpots.SKILL_HALL_STAIRS_1, Furniture.forFurnitureId(stairId), null,
          myTiles, ConstructionData.BASE_X + (myTiles[0] * 8) + 3,
          ConstructionData.BASE_Y + (myTiles[1] * 8) + 3, rotation, p, false, 1);
    ...
    if (toHeight == 1) {
      Room r = p.getRegionInstance().getOwner().getHouseRooms()[0][(myTiles[0] - 1)
          + xOff][(myTiles[1] - 1) + yOff];
    ...
    if (toHeight == 4 || p.inConstructionDungeon()) {
      house.getSecondaryPalette().setTile((myTiles[0] - 1) + xOff, (myTiles[1] - 1) + yOff, 0,
          tile);
    } else {
    ...
    createPalette(p);
    /*
     * if (p.getFields().inDungeon()) { p.getPacketSender().constructMapRegionForConstruction(
  ...
  public static void rotateRoom(int wise, Player p) {
    ...
    int toRot =
        (wise == 0 ? RoomData.getNextEligibleRotationClockWise(rd, direction, r.getRotation())
  ...
  public static void deleteRoom(Player p, int toHeight) {
    ...
    int roomType =
        p.inConstructionDungeon() ? ConstructionData.DUNGEON_EMPTY : ConstructionData.EMPTY;
  ...
  public static void handleInterfaceCrosses(ArrayList<Furniture> items, Player c, HotSpots hs) {
    ...
    for (Furniture f : items) {
      ...
      for (int i1 = 0; i1 < f.getRequiredItems().length; i1++) {
        c.getPacketSender().sendString((38276 + i2) + (i - 1000) * 6, f.getRequiredItems()[i1][1]
          ...
          for (HouseFurniture pf : c.getHouseFurniture()) {
            if (pf.getRoomX() == myTiles[0] - 1 && pf.getRoomY() == myTiles[1] - 1) {
              if (pf.getHotSpot(c.getHouseRooms()[c.inConstructionDungeon() ? 4
                  : c.getPosition().getZ()][myTiles[0] - 1][myTiles[1] - 1].getRotation()) == hs) {
                if (pf.getFurnitureId() != fur.getFurnitureId()) {
                  canMake = false;
                } else {
                  canMake = true;
  ...
  public static void handleItemClick(int itemID, Player p) {
    ...
    doFurniturePlace(s, f, hsses, myTiles, actualX, actualY, roomRot, p, false,
        p.getPosition().getZ());
    HouseFurniture pf = new HouseFurniture(myTiles[0] - 1, myTiles[1] - 1, toHeight,
        s.getHotSpotId(), f.getFurnitureId(), s.getXOffset(), s.getYOffset());
    p.getHouseFurniture().add(pf);
    p.getPacketSender().sendInterfaceRemoval();
    ...
    for (int x = 0; x < 13; x++) {
      for (int y = 0; y < 13; y++) {
        ...
        if (p.getPosition().getX() >= minX && p.getPosition().getX() <= maxX
            && p.getPosition().getY() >= minY && p.getPosition().getY() <= maxY) {
          return new int[] {x, y};
        }
      }
    }
