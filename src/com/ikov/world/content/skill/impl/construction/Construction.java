...
import com.ikov.world.content.skill.impl.construction.ConstructionData.HouseLocation;
import com.ikov.world.content.skill.impl.construction.ConstructionData.HouseTheme;
import com.ikov.world.content.skill.impl.construction.ConstructionData.Portals;
import com.ikov.world.content.skill.impl.construction.ConstructionData.RoomData;
...
public class Construction {
	...
	public static void moveHouse(Player p, HouseLocation location) {
		if (p.getHouseRooms()[0][0][0] == null)
			return;
		if (p.getHouseLocation() == location) {
			p.getPacketSender().sendMessage("Your house is already located there!");
			p.getPacketSender().sendInterfaceRemoval();
		} else {
			if (p.getInventory().getAmount(995) >= location.getCost()) {
				p.getInventory().delete(995, location.getCost());
				p.setHouseLocation(location);
				p.getPacketSender().sendMessage("Your house has succesfully been moved to " + location.toString() + ".");
				p.getPacketSender().sendInterfaceRemoval();
			} else {
				DialogueManager.start(p, 195);
			}
		}
	}
	...
	public static void buyHouse(Player p) {
		if (p.getHouseRooms()[0][0][0] != null)
			return;
		if (p.getInventory().getAmount(995) >= ConstructionConstants.HOUSE_PRICE) {
			p.getInventory().delete(995, ConstructionConstants.HOUSE_PRICE);
			newHouse(p);
			DialogueManager.start(p, 192);
		} else {
			DialogueManager.start(p, 195);
		}
	}

	public static void switchTheme(Player p, HouseTheme theme) {
		if (p.getHouseRooms()[0][0][0] == null)
			return;
		if (p.getHouseTheme() == theme) {
			p.getPacketSender().sendMessage("Your house is already based on that theme!");
			p.getPacketSender().sendInterfaceRemoval();
		} else {
			if (p.getInventory().getAmount(995) >= theme.getCost()) {
				p.getInventory().delete(995, theme.getCost());
				p.setHouseTheme(theme);
				p.getPacketSender().sendMessage("Your house has succesfully been redecorated as " + theme.toString() + ".");
				for (int x = 0; x < 13; x++)
					for (int y = 0; y < 13; y++)
						p.getHouseRooms()[0][x][y].setTheme(theme);
				p.getPacketSender().sendInterfaceRemoval();
			} else {
				DialogueManager.start(p, new Dialogue() {
					...
					public String[] dialogue() {
						return new String[] { "You need " + theme.getCost() + " coins to buy that." };
					}
	...
	public static void newHouse(Player p) {
		...
		HouseFurniture pf = new HouseFurniture(7, 7, 0, HotSpots.CENTREPIECE.getHotSpotId(), Furniture.EXIT_PORTAL.getFurnitureId(), HotSpots.CENTREPIECE.getXOffset(),
				HotSpots.CENTREPIECE.getYOffset());
		p.getHouseFurniture().clear();
				...
				switch (ticks) {
					...
					case 5:
						...
						if (me.getConstructionCoords() != null) {
							me.moveTo(new Position(me.getConstructionCoords()[0], me.getConstructionCoords()[1], me.getConstructionCoords()[2]));
	...
	public static HouseFurniture findNearestPortal(Player p) {
		...
		HouseFurniture closest = null;
		ArrayList<HouseFurniture> portals = new ArrayList<HouseFurniture>();
		...
		if (portals.size() > 0)
			closest = portals.get(0);

		for (HouseFurniture hf : portals) {
			if (new Position(hf.getRoomX(), hf.getRoomY()).getDistance(p.getPosition().setZ(0)) < new Position(closest.getRoomX(), closest.getRoomY()).getDistance(p.getPosition().setZ(0))) {
				closest = hf;
			}
		}
			...
			for (int x = 0; x < 13; x++) {
				for (int y = 0; y < 13; y++) {
					...
					if (p.getHouseRooms()[z][x][y].getX() == 0)
						continue;
					PaletteTile tile = new PaletteTile(p.getHouseRooms()[z][x][y].getX(), p.getHouseRooms()[z][x][y].getY(), p.getHouseRooms()[z][x][y].getZ(),
	...
	public static void placeAllFurniture(Player p, int x, int y, int z) {
		...
		for (HouseFurniture pf : owner.getHouseFurniture()) {
			...
			doFurniturePlace(hs, f, hsses, getMyChunkFor(actualX, actualY), actualX, actualY, room.getRotation(), p, false, z, false);
		}
	}
	...
	public static void placeAllFurniture(Player p, int heightLevel) {
		...
		for (HouseFurniture pf : owner.getHouseFurniture()) {
			...
			Furniture f = Furniture.forFurnitureId(pf.getFurnitureId());
			ArrayList<HotSpots> hsses = HotSpots.forObjectId_3(f.getHotSpotId());
			doFurniturePlace(hs, f, hsses, getMyChunkFor(actualX, actualY), actualX, actualY, room.getRotation(), p, false, heightLevel, remove);
		}
	}

	public static void doFurniturePlace(HotSpots s, Furniture f, ArrayList<HotSpots> hsses, int[] myTiles, int actualX, int actualY, int roomRot, Player p, boolean placeBack, int height, boolean remove) {
		int portalId = -1;
		...
		if (s.getHotSpotId() == 92) {
			...
			if (s.getObjectId() == 15329 || s.getObjectId() == 15328) {
				p.getPacketSender().sendObject_cons(actualX, actualY, s.getObjectId() == 15328 ? (placeBack ? 15328 : f.getFurnitureId()) : (placeBack ? 15329 : f.getFurnitureId() + 1),
						s.getRotation(roomRot), 0, height);
				offsetX += ConstructionData.getXOffsetForObjectId(f.getFurnitureId(), s.getXOffset() + (s.getObjectId() == 15329 ? 1 : -1), s.getYOffset(), roomRot, s.getRotation(0));
				offsetY += ConstructionData.getYOffsetForObjectId(f.getFurnitureId(), s.getXOffset() + (s.getObjectId() == 15329 ? 1 : -1), s.getYOffset(), roomRot, s.getRotation(0));
				p.getPacketSender().sendObject_cons(offsetX, offsetY, s.getObjectId() == 15329 ? (placeBack ? 15328 : f.getFurnitureId()) : (placeBack ? 15329 : f.getFurnitureId() + 1),
						s.getRotation(roomRot), 0, height);

			}
			if (s.getObjectId() == 15326 || s.getObjectId() == 15327) {
				p.getPacketSender().sendObject_cons(actualX, actualY, s.getObjectId() == 15327 ? (placeBack ? 15327 : f.getFurnitureId() + 1) : (placeBack ? 15326 : f.getFurnitureId()),
						s.getRotation(roomRot), 0, height);
				offsetX += ConstructionData.getXOffsetForObjectId(f.getFurnitureId(), s.getXOffset() + (s.getObjectId() == 15326 ? 1 : -1), s.getYOffset(), roomRot, s.getRotation(0));
				offsetY += ConstructionData.getYOffsetForObjectId(f.getFurnitureId(), s.getXOffset() + (s.getObjectId() == 15326 ? 1 : -1), s.getYOffset(), roomRot, s.getRotation(0));
				p.getPacketSender().sendObject_cons(offsetX, offsetY, s.getObjectId() == 15326 ? (placeBack ? 15327 : f.getFurnitureId() + 1) : (placeBack ? 15326 : f.getFurnitureId()),
						s.getRotation(roomRot), 0, height);
			...
			p.getPacketSender().sendObject_cons(actualX, actualY + 3, placeBack ? 15348 : cornerObject, 2, type, height);
			p.getPacketSender().sendObject_cons(actualX + 1, actualY + 3, placeBack ? 15348 : upperObject, 2, type, height);
			p.getPacketSender().sendObject_cons(actualX + 2, actualY + 3, placeBack ? 15348 : upperObject, 2, type, height);
			p.getPacketSender().sendObject_cons(actualX + 3, actualY + 3, placeBack ? 15348 : cornerObject, 3, type, height);
			p.getPacketSender().sendObject_cons(actualX + 3, actualY + 2, placeBack ? 15348 : rightObject, 3, type, height);
			p.getPacketSender().sendObject_cons(actualX + 3, actualY + 1, placeBack ? 15348 : rightObject, 3, type, height);
			p.getPacketSender().sendObject_cons(actualX + 3, actualY, placeBack ? 15348 : cornerObject, 0, type, height);
			...
			p.getPacketSender().sendObject_cons(actualX + 1, actualY + 1, placeBack ? 15348 : middleObject, 0, type, height);
			p.getPacketSender().sendObject_cons(actualX + 2, actualY + 1, placeBack ? 15348 : middleObject, 0, type, height);
			if (veryMiddleObject != 0)
				p.getPacketSender().sendObject_cons(actualX + 1, actualY + 2, veryMiddleObject, 0, 10, height);
			p.getPacketSender().sendObject_cons(actualX + 1, actualY + 2, placeBack ? 15348 : middleObject, 0, type, height);
			p.getPacketSender().sendObject_cons(actualX + 2, actualY + 2, placeBack ? 15348 : middleObject, 0, type, height);

		} else if (s.getHotSpotId() == 86) {
			actualX = ConstructionData.BASE_X + (myTiles[0] * 8) + 2;
			actualY = ConstructionData.BASE_Y + (myTiles[1] * 8) + 2;

			p.getPacketSender().sendObject_cons(actualX + 1, actualY, placeBack ? 15352 : f.getFurnitureId(), 3, 0, height);
			p.getPacketSender().sendObject_cons(actualX + 2, actualY, placeBack ? 15352 : f.getFurnitureId(), 3, 0, height);
			p.getPacketSender().sendObject_cons(actualX + 3, actualY, placeBack ? 15352 : f.getFurnitureId(), 2, 2, height);
			p.getPacketSender().sendObject_cons(actualX + 3, actualY + 1, placeBack ? 15352 : f.getFurnitureId(), 2, 0, height);
			p.getPacketSender().sendObject_cons(actualX + 3, actualY + 2, placeBack ? 15352 : f.getFurnitureId() + 1, 2, 0, height);
			p.getPacketSender().sendObject_cons(actualX + 3, actualY + 3, placeBack ? 15352 : f.getFurnitureId(), 1, 2, height);
			p.getPacketSender().sendObject_cons(actualX + 2, actualY + 3, placeBack ? 15352 : f.getFurnitureId(), 1, 0, height);
			p.getPacketSender().sendObject_cons(actualX + 1, actualY + 3, placeBack ? 15352 : f.getFurnitureId(), 1, 0, height);
			p.getPacketSender().sendObject_cons(actualX, actualY + 3, placeBack ? 15352 : f.getFurnitureId(), 0, 2, height);
			p.getPacketSender().sendObject_cons(actualX, actualY + 2, placeBack ? 15352 : f.getFurnitureId(), 0, 0, height);
			p.getPacketSender().sendObject_cons(actualX, actualY + 1, placeBack ? 15352 : f.getFurnitureId(), 0, 0, height);
			...
			p.getPacketSender().sendObject_cons(actualX + 1, actualY, placeBack ? 15369 : f.getFurnitureId(), 3, 0, height);
			p.getPacketSender().sendObject_cons(actualX + 2, actualY, placeBack ? 15369 : f.getFurnitureId(), 3, 0, height);
			p.getPacketSender().sendObject_cons(actualX + 5, actualY, placeBack ? 15369 : f.getFurnitureId(), 3, 0, height);
			p.getPacketSender().sendObject_cons(actualX + 6, actualY, placeBack ? 15369 : f.getFurnitureId(), 3, 0, height);
			p.getPacketSender().sendObject_cons(actualX + 7, actualY, placeBack ? 15369 : f.getFurnitureId(), 2, 2, height);
			// north walls
			p.getPacketSender().sendObject_cons(actualX, actualY + 7, placeBack ? 15369 : f.getFurnitureId(), 0, 2, height);
			p.getPacketSender().sendObject_cons(actualX + 1, actualY + 7, placeBack ? 15369 : f.getFurnitureId(), 1, 0, height);
			p.getPacketSender().sendObject_cons(actualX + 2, actualY + 7, placeBack ? 15369 : f.getFurnitureId(), 1, 0, height);
			p.getPacketSender().sendObject_cons(actualX + 5, actualY + 7, placeBack ? 15369 : f.getFurnitureId(), 1, 0, height);
			p.getPacketSender().sendObject_cons(actualX + 6, actualY + 7, placeBack ? 15369 : f.getFurnitureId(), 1, 0, height);
			p.getPacketSender().sendObject_cons(actualX + 7, actualY + 7, placeBack ? 15369 : f.getFurnitureId(), 1, 2, height);
			// left walls
			p.getPacketSender().sendObject_cons(actualX, actualY + 1, placeBack ? 15369 : f.getFurnitureId(), 0, 0, height);
			p.getPacketSender().sendObject_cons(actualX, actualY + 2, placeBack ? 15369 : f.getFurnitureId(), 0, 0, height);
			p.getPacketSender().sendObject_cons(actualX, actualY + 5, placeBack ? 15369 : f.getFurnitureId(), 0, 0, height);
			p.getPacketSender().sendObject_cons(actualX, actualY + 6, placeBack ? 15369 : f.getFurnitureId(), 0, 0, height);
			// right walls
			p.getPacketSender().sendObject_cons(actualX + 7, actualY + 1, placeBack ? 15369 : f.getFurnitureId(), 2, 0, height);
			p.getPacketSender().sendObject_cons(actualX + 7, actualY + 2, placeBack ? 15369 : f.getFurnitureId(), 2, 0, height);
			p.getPacketSender().sendObject_cons(actualX + 7, actualY + 5, placeBack ? 15369 : f.getFurnitureId(), 2, 0, height);
			p.getPacketSender().sendObject_cons(actualX + 7, actualY + 6, placeBack ? 15369 : f.getFurnitureId(), 2, 0, height);
		} else if (s.getHotSpotId() == 77) {
			actualX = ConstructionData.BASE_X + (myTiles[0] * 8);
			actualY = ConstructionData.BASE_Y + (myTiles[1] * 8);
			// left down corner
			p.getPacketSender().sendObject_cons(actualX, actualY, placeBack ? 15372 : f.getFurnitureId() + 1, 3, 10, height);
			p.getPacketSender().sendObject_cons(actualX + 1, actualY, placeBack ? 15371 : f.getFurnitureId() + 2, 0, 10, height);
			p.getPacketSender().sendObject_cons(actualX + 2, actualY, placeBack ? 15370 : f.getFurnitureId(), 0, 10, height);
			p.getPacketSender().sendObject_cons(actualX, actualY + 1, placeBack ? 15371 : f.getFurnitureId() + 2, 1, 10, height);
			p.getPacketSender().sendObject_cons(actualX, actualY + 2, placeBack ? 15370 : f.getFurnitureId(), 3, 10, height);
			// right down corner
			p.getPacketSender().sendObject_cons(actualX + 7, actualY, placeBack ? 15372 : f.getFurnitureId() + 1, 2, 10, height);
			p.getPacketSender().sendObject_cons(actualX + 6, actualY, placeBack ? 15371 : f.getFurnitureId() + 2, 0, 10, height);
			p.getPacketSender().sendObject_cons(actualX + 5, actualY, placeBack ? 15370 : f.getFurnitureId(), 2, 10, height);
			p.getPacketSender().sendObject_cons(actualX + 7, actualY + 1, placeBack ? 15371 : f.getFurnitureId() + 2, 3, 10, height);
			p.getPacketSender().sendObject_cons(actualX + 7, actualY + 2, placeBack ? 15370 : f.getFurnitureId(), 3, 10, height);
			// upper left corner
			p.getPacketSender().sendObject_cons(actualX, actualY + 7, placeBack ? 15372 : f.getFurnitureId() + 1, 0, 10, height);
			p.getPacketSender().sendObject_cons(actualX + 1, actualY + 7, placeBack ? 15371 : f.getFurnitureId() + 2, 0, 10, height);
			p.getPacketSender().sendObject_cons(actualX + 2, actualY + 7, placeBack ? 15370 : f.getFurnitureId(), 0, 10, height);
			p.getPacketSender().sendObject_cons(actualX, actualY + 6, placeBack ? 15371 : f.getFurnitureId() + 2, 1, 10, height);
			p.getPacketSender().sendObject_cons(actualX, actualY + 5, placeBack ? 15370 : f.getFurnitureId(), 1, 10, height);
			// upper right corner
			p.getPacketSender().sendObject_cons(actualX + 7, actualY + 7, placeBack ? 15372 : f.getFurnitureId() + 1, 1, 10, height);
			p.getPacketSender().sendObject_cons(actualX + 6, actualY + 7, placeBack ? 15371 : f.getFurnitureId() + 2, 0, 10, height);
			p.getPacketSender().sendObject_cons(actualX + 5, actualY + 7, placeBack ? 15370 : f.getFurnitureId(), 2, 10, height);
			p.getPacketSender().sendObject_cons(actualX + 7, actualY + 6, placeBack ? 15371 : f.getFurnitureId() + 2, 3, 10, height);
			p.getPacketSender().sendObject_cons(actualX + 7, actualY + 5, placeBack ? 15370 : f.getFurnitureId(), 1, 10, height);
			...
			p.getPacketSender().sendObject_cons(actualX + 2, actualY + 3, placeBack ? 15292 : combatringFloorsInner, 0, 22, height);
			p.getPacketSender().sendObject_cons(actualX + 3, actualY + 3, placeBack ? 15292 : combatringFloorsInner, 0, 22, height);
			p.getPacketSender().sendObject_cons(actualX + 3, actualY + 2, placeBack ? 15292 : combatringFloorsInner, 0, 22, height);
			p.getPacketSender().sendObject_cons(actualX + 2, actualY + 2, placeBack ? 15292 : combatringFloorsInner, 0, 22, height);
			p.getPacketSender().sendObject_cons(actualX + 2, actualY + 1, placeBack ? 15291 : combatringFloorsOuter, 3, 22, height);
			p.getPacketSender().sendObject_cons(actualX + 3, actualY + 1, placeBack ? 15291 : combatringFloorsOuter, 3, 22, height);
			p.getPacketSender().sendObject_cons(actualX + 2, actualY + 4, placeBack ? 15291 : combatringFloorsOuter, 1, 22, height);
			p.getPacketSender().sendObject_cons(actualX + 3, actualY + 4, placeBack ? 15291 : combatringFloorsOuter, 1, 22, height);
			p.getPacketSender().sendObject_cons(actualX + 4, actualY + 3, placeBack ? 15291 : combatringFloorsOuter, 2, 22, height);
			p.getPacketSender().sendObject_cons(actualX + 4, actualY + 2, placeBack ? 15291 : combatringFloorsOuter, 2, 22, height);
			p.getPacketSender().sendObject_cons(actualX + 1, actualY + 3, placeBack ? 15291 : combatringFloorsOuter, 0, 22, height);
			p.getPacketSender().sendObject_cons(actualX + 1, actualY + 2, placeBack ? 15291 : combatringFloorsOuter, 0, 22, height);
			p.getPacketSender().sendObject_cons(actualX + 4, actualY + 1, placeBack ? 15289 : combatringFloorsCorner, 3, 22, height);
			p.getPacketSender().sendObject_cons(actualX + 4, actualY + 4, placeBack ? 15289 : combatringFloorsCorner, 2, 22, height);
			p.getPacketSender().sendObject_cons(actualX + 1, actualY + 4, placeBack ? 15289 : combatringFloorsCorner, 1, 22, height);
			p.getPacketSender().sendObject_cons(actualX + 1, actualY + 1, placeBack ? 15289 : combatringFloorsCorner, 0, 22, height);
			p.getPacketSender().sendObject_cons(actualX, actualY + 4, placeBack ? 15277 : combatringStrings, 3, 0, height);
			p.getPacketSender().sendObject_cons(actualX, actualY + 1, placeBack ? 15277 : combatringStrings, 3, 0, height);
			p.getPacketSender().sendObject_cons(actualX + 5, actualY + 4, placeBack ? 15277 : combatringStrings, 3, 0, height);
			p.getPacketSender().sendObject_cons(actualX + 5, actualY + 1, placeBack ? 15277 : combatringStrings, 0, 3, height);
			p.getPacketSender().sendObject_cons(actualX + 1, actualY, placeBack ? 15277 : combatringStrings, 1, 0, height);
			p.getPacketSender().sendObject_cons(actualX + 2, actualY, placeBack ? 15277 : combatringStrings, 1, 0, height);
			p.getPacketSender().sendObject_cons(actualX + 3, actualY, placeBack ? 15277 : combatringStrings, 1, 0, height);
			p.getPacketSender().sendObject_cons(actualX + 4, actualY, placeBack ? 15277 : combatringStrings, 1, 0, height);
			p.getPacketSender().sendObject_cons(actualX + 5, actualY, placeBack ? 15277 : combatringStrings, 0, 3, height);
			p.getPacketSender().sendObject_cons(actualX + 1, actualY + 5, placeBack ? 15277 : combatringStrings, 3, 0, height);
			p.getPacketSender().sendObject_cons(actualX + 2, actualY + 5, placeBack ? 15277 : combatringStrings, 3, 0, height);
			p.getPacketSender().sendObject_cons(actualX + 3, actualY + 5, placeBack ? 15277 : combatringStrings, 3, 0, height);
			p.getPacketSender().sendObject_cons(actualX + 4, actualY + 5, placeBack ? 15277 : combatringStrings, 3, 0, height);
			p.getPacketSender().sendObject_cons(actualX + 5, actualY + 5, placeBack ? 15277 : combatringStrings, 3, 3, height);
			p.getPacketSender().sendObject_cons(actualX, actualY + 5, placeBack ? 15277 : combatringStrings, 2, 3, height);
			p.getPacketSender().sendObject_cons(actualX, actualY, placeBack ? 15277 : combatringStrings, 1, 3, height);
			p.getPacketSender().sendObject_cons(actualX, actualY + 4, placeBack ? 15277 : combatringStrings, 2, 0, height);
			p.getPacketSender().sendObject_cons(actualX, actualY + 3, placeBack ? 15277 : combatringStrings, 2, 0, height);
			p.getPacketSender().sendObject_cons(actualX, actualY + 2, placeBack ? 15277 : combatringStrings, 2, 0, height);
			p.getPacketSender().sendObject_cons(actualX, actualY + 1, placeBack ? 15277 : combatringStrings, 2, 0, height);
			p.getPacketSender().sendObject_cons(actualX + 5, actualY + 4, placeBack ? 15277 : combatringStrings, 0, 0, height);
			p.getPacketSender().sendObject_cons(actualX + 5, actualY + 3, placeBack ? 15277 : combatringStrings, 0, 0, height);
			p.getPacketSender().sendObject_cons(actualX + 5, actualY + 2, placeBack ? 15277 : combatringStrings, 0, 0, height);
			p.getPacketSender().sendObject_cons(actualX + 5, actualY + 1, placeBack ? 15277 : combatringStrings, 0, 0, height);
			...
			for (int x = 0; x < s.getCarpetDim().getWidth() + 1; x++) {
				for (int y = 0; y < s.getCarpetDim().getHeight() + 1; y++) {
					boolean isEdge = (x == 0 && y == 0 || x == 0 && y == s.getCarpetDim().getHeight() || y == 0 && x == s.getCarpetDim().getWidth() || x == s.getCarpetDim().getWidth()
							&& y == s.getCarpetDim().getHeight());
					boolean isWall = ((x == 0 || x == s.getCarpetDim().getWidth()) && (y != 0 && y != s.getCarpetDim().getHeight()) || (y == 0 || y == s.getCarpetDim().getHeight())
					...
					int offsetX = ConstructionData.BASE_X + (myTiles[0] * 8);
					int offsetY = ConstructionData.BASE_Y + (myTiles[1] * 8);
					offsetX += ConstructionData.getXOffsetForObjectId(f.getFurnitureId(), s.getXOffset() + x - 1, s.getYOffset() + y - 1, roomRot, s.getRotation(roomRot));
					offsetY += ConstructionData.getYOffsetForObjectId(f.getFurnitureId(), s.getXOffset() + x - 1, s.getYOffset() + y - 1, roomRot, s.getRotation(roomRot));
					if (isEdge)
						p.getPacketSender().sendObject_cons(offsetX, offsetY, placeBack ? s.getObjectId() + 2 : f.getFurnitureId(), HotSpots.getRotation_2(rot, roomRot), 22, height);
					else if (isWall)
						p.getPacketSender()
								.sendObject_cons(offsetX, offsetY, placeBack ? s.getObjectId() + 1 : f.getFurnitureId() + 1, HotSpots.getRotation_2(rot, roomRot), s.getObjectType(), height);
					else
						p.getPacketSender().sendObject_cons(offsetX, offsetY, placeBack ? s.getObjectId() : f.getFurnitureId() + 2, HotSpots.getRotation_2(rot, roomRot), s.getObjectType(), height);
				}
			}
		} else if (s.isMutiple()) {
			...
			for (HotSpots find : hsses) {
				...
				p.getPacketSender().sendObject_cons(actualX1, actualY1, placeBack ? s.getObjectId() : f.getFurnitureId(), find.getRotation(roomRot), find.getObjectType(), height);
			}
		} else {
			p.getPacketSender().sendObject_cons(actualX, actualY, (portalId != -1 ? portalId : placeBack ? s.getObjectId() : f.getFurnitureId()), s.getRotation(roomRot), s.getObjectType(), height);
		}
	}
	...
	public static String hasReqs(Player p, Furniture f, HotSpots hs) {
		...
		if (f.getAdditionalSkillRequirements() != null) {
			for (int ii = 0; ii < f.getAdditionalSkillRequirements().length; ii++) {
				if (p.getSkillManager().getCurrentLevel(Skill.forId(f.getAdditionalSkillRequirements()[ii][0])) < f.getAdditionalSkillRequirements()[ii][1]) {
		...
		if (f.getFurnitureRequired() != -1) {
			...
			for (HouseFurniture pf : p.getHouseFurniture()) {
				if (pf.getRoomX() == myTiles[0] - 1 && pf.getRoomY() == myTiles[1] - 1) {
					if (pf.getHotSpot(p.getHouseRooms()[p.inConstructionDungeon() ? 4 : p.getPosition().getZ()][myTiles[0] - 1][myTiles[1] - 1].getRotation()) == hs) {
	...
	public static boolean buildActions(Player p, Furniture f, HotSpots hs) {
		...
		/*
		 * Fetch our value for the last time we constructed something
		 */
		long lastClick = p.getAttribute("last_const", 0L);
		
		/*
		 * Check to make sure 5 seconds have passed before we can build something else to prevent spamming
		 */
		...
		switch (buttonId) {
			...
			case 2471:
				...
				if (p.getDialogueActionId() == 645) {
					...
					p.setDialogueActionId(646);
					DialogueManager.start(p, 646);
					return true;
				}
				break;
			case 2472:
			...
			case 2494:
				...
				if (p.getDialogueActionId() == 425 || p.getDialogueActionId() == 426 || p.getDialogueActionId() == 427 || p.getDialogueActionId() == 429 || p.getDialogueActionId() == 431) {
					p.getInventory().add(7688, 1);
					return true;
				}
			...
			case 2498:
				...
				if (p.getDialogueActionId() == 646) {
					p.setDialogueActionId(647);
					DialogueManager.start(p, 647);
					return true;
				}
				if (p.getDialogueActionId() == 647) {
					p.setDialogueActionId(646);
			...
			case 2461:
				...
				if (p.getDialogueActionId() == 416) {
					...
					if (room.getType() != ConstructionData.EMPTY) {
						...
						createRoom(room_1.getType() == ConstructionData.SKILL_HALL_DOWN ? ConstructionData.SKILL_ROOM : ConstructionData.QUEST_ROOM, p, 101);
						p.getPacketSender().sendInterfaceRemoval();
					}
					return true;
				}
				...
				if (p.getDialogueActionId() == 414) {
					...
					if (room != null) {
						...
						createRoom(room_1.getType() == ConstructionData.SKILL_ROOM ? ConstructionData.SKILL_HALL_DOWN : ConstructionData.QUEST_HALL_DOWN, p, 100);
					}
					return true;
				}
				break;
			...
			case 2462:// no option
				...
				if (p.getDialogueActionId() == 414 || p.getDialogueActionId() == 416 || p.getDialogueActionId() == 419 || p.getDialogueActionId() == 457) {
					p.getPacketSender().sendInterfaceRemoval();
					return true;
				}
				break;
	...
	public static boolean roomExists(Player p) {
		...
		Room room = p.getRegionInstance().getOwner().getHouseRooms()[p.inConstructionDungeon() ? 4 : p.getPosition().getZ()][myTiles[0] - 1 + xOff][myTiles[1] - 1 + yOff];
		if (room == null)
			return false;
		if (room.getType() == ConstructionData.BUILDABLE || room.getType() == ConstructionData.EMPTY || room.getType() == ConstructionData.DUNGEON_EMPTY)
			return false;
		return true;
	}
	...
	public static boolean handleRemoveClick(int obX, int obY, int objectId, Player p) {
		...
		int[] myTiles = getMyChunk(p);
		int roomRot = p.getRegionInstance().getOwner().getHouseRooms()[p.inConstructionDungeon() ? 4 : p.getPosition().getZ()][myTiles[0] - 1][myTiles[1] - 1].getRotation();
		Room room = p.getRegionInstance().getOwner().getHouseRooms()[p.inConstructionDungeon() ? 4 : p.getPosition().getZ()][myTiles[0] - 1][myTiles[1] - 1];
		...
		if (objectId == 13126 || objectId == 13127 || objectId == 13128 || objectId == 13132 || objectId == 13133 || objectId == 13134 || objectId == 13135 || objectId == 13136 || objectId == 13137
				|| objectId == 13138 || objectId == 13139 || objectId == 13140 || objectId == 13145 || objectId == 13147 || objectId == 13142 || objectId == 13143 || objectId == 13144) {
		...
		if (objectId == 15270 || objectId == 15273 || objectId == 15274 || objectId >= 13588 && objectId <= 13597) {
			...
			if (room.getType() == ConstructionData.SKILL_ROOM || room.getType() == ConstructionData.SKILL_HALL_DOWN || room.getType() == ConstructionData.QUEST_ROOM
					|| room.getType() == ConstructionData.QUEST_HALL_DOWN || room.getType() == ConstructionData.DUNGEON_STAIR_ROOM || room.getType() == ConstructionData.SKILL_HALL_DOWN)
				hs = HotSpots.SKILL_HALL_RUG_3;
			if (room.getType() == ConstructionData.BEDROOM)
				hs = HotSpots.BEDROOM_RUG_3;
		}
		doFurniturePlace(hs, f, hsses, myTiles, obX, obY, roomRot, p, true, p.getPosition().getZ(), false);
		...
		while (iterator.hasNext()) {
			...
			if (pf.getRoomX() != myTiles[0] - 1 || pf.getRoomY() != myTiles[1] - 1 || pf.getRoomZ() != (p.inConstructionDungeon() ? 4 : p.getPosition().getZ()))
				continue;
	...
	public static boolean handleSpaceClick(int obX, int obY, int objectId, Player p) {
		...
		ArrayList<HotSpots> hsses = ConstructionData.HotSpots.forObjectId_2(objectId);
		if (hsses.isEmpty())
			return false;
		...
		if (hsses.size() == 1)
			...
			for (HotSpots find : hsses) {
				...
				int actualY = ConstructionData.BASE_Y + (myTiles[1] * 8);
				actualY += ConstructionData.getYOffsetForObjectId(find.getObjectId(), find, roomRot);
				if (p.getBuildFurnitureX() == actualX && p.getBuildFurnitureY() == actualY && myRoom == find.getRoomType() || find.getCarpetDim() != null && myRoom == find.getRoomType()) {
					hs = find;
					break;
				}
			}
		...
		if (hs == null)
			return true;
		ArrayList<Furniture> f = ConstructionData.Furniture.getForHotSpotId(hs.getHotSpotId());
		if (f == null)
			return false;
	...
	public static void createRoom(int roomType, Player p, int toHeight) {
		...
		if (rd == null)
			return;
		if (rd == RoomData.MENAGERY || rd == RoomData.QUEST_HALL_DOWN || rd == RoomData.QUEST_ROOM || rd == RoomData.SKILL_ROOM) {
			p.getPacketSender().sendMessage("This room has been disabled.");
			return;
		}
		if (rd.getLevelToBuild() > p.getSkillManager().getCurrentLevel(Skill.CONSTRUCTION)) {
			p.getPacketSender().sendMessage("You need a Construction level of at least " + rd.getLevelToBuild() + " to build this room.");
			return;
		}
		if (p.getInventory().getAmount(995) < rd.getCost() && p.getMoneyInPouch() < rd.getCost()) {
		...
		if (yOnTile == 7)
			direction = UP;
		int rotation = ConstructionData.RoomData.getFirstElegibleRotation(rd, direction);
		if (toHeight == 100) {
			...
			doFurniturePlace(HotSpots.SKILL_HALL_STAIRS_1, Furniture.forFurnitureId(stairId), null, myTiles, ConstructionData.BASE_X + (myTiles[0] * 8) + 3, ConstructionData.BASE_Y + (myTiles[1] * 8)
					+ 3, rotation, p, false, 1, false);
		...
		if (toHeight == 101) {
			...
			doFurniturePlace(HotSpots.SKILL_HALL_STAIRS, Furniture.forFurnitureId(stairId), null, myTiles, ConstructionData.BASE_X + (myTiles[0] * 8) + 3, ConstructionData.BASE_Y + (myTiles[1] * 8)
					+ 3, rotation, p, false, 1, false);
		...
		Room room = new Room(rotation, roomType, toHeight);
		...
		if (toHeight == 1) {
			Room r = p.getRegionInstance().getOwner().getHouseRooms()[0][(myTiles[0] - 1) + xOff][(myTiles[1] - 1) + yOff];
			if (r.getType() == ConstructionData.EMPTY || r.getType() == ConstructionData.BUILDABLE || r.getType() == ConstructionData.GARDEN || r.getType() == ConstructionData.FORMAL_GARDEN) {
		...
		house.getOwner().getHouseRooms()[p.inConstructionDungeon() ? 4 : toHeight][(myTiles[0] - 1) + xOff][(myTiles[1] - 1) + yOff] = new Room(rotation, roomType, toHeight);
		p.setConstructionCoords(new int[] { p.getPosition().getX(), p.getPosition().getY(), p.getPosition().getZ() });
		p.moveTo(new Position(ConstructionData.MIDDLE_X, ConstructionData.MIDDLE_Y, p.getPosition().getZ()));
		...
		if(p.getMoneyInPouch() >= rd.getCost())
		{
			p.setMoneyInPouch(p.getMoneyInPouch() - rd.getCost());
			p.getPacketSender().sendString(8135,
					"" + p.getMoneyInPouch() + "");
		}
		else
			p.getInventory().delete(995, rd.getCost());
		p.moveTo(new Position(p.getConstructionCoords()[0], p.getConstructionCoords()[1], p.getConstructionCoords()[2]));
		p.setConstructionCoords(null);
		p.getPacketSender().sendInterfaceRemoval();
		// enterHouse(p, p, true, false);
		// p.getPacketSender().sendMessage("Sent!" +
		// p.getPacketSender().getRegionOffset(p.getPosition())).sendMessage("xOff: "
		// + xOff + " yOff: " + yOff);
	...
	public static void rotateRoom(int wise, Player p) {
		...
		placeAllFurniture(p, chunkX, chunkY, p.inConstructionDungeon() ? 4 : p.getPosition().getZ());
	...
	public static void deleteRoom(Player p, int toHeight) {
		...
		Room room = new Room(0, roomType, toHeight);
		PaletteTile tile = new PaletteTile(room.getX(), room.getY(), room.getZ(), room.getRotation());
		...
		if (p.getPosition().getZ() == 0) {
			...
			p.getRegionInstance().getOwner().getHouseRooms()[p.inConstructionDungeon() ? 4 : toHeight][chunkX][chunkY] = new Room(0, roomType, toHeight);
		} else {
			if (p.inConstructionDungeon()) {
	...
	public static void handleInterfaceCrosses(ArrayList<Furniture> items, Player c, HotSpots hs) {
		...
		for (Furniture f : items) {
			c.getPacketSender().sendString(38275 + (i - 1000) * 6, Misc.formatPlayerName(f.toString().toLowerCase().replaceAll("_", " ")));
			c.getPacketSender().sendString(38280 + (i - 1000) * 6, "Level: " + f.getLevel());
			int i2 = 0;
			boolean canMake = true;
			for (int i1 = 0; i1 < f.getRequiredItems().length; i1++) {
				c.getPacketSender().sendString((38276 + i2) + (i - 1000) * 6, f.getRequiredItems()[i1][1] + " x " + ItemDefinition.forId(f.getRequiredItems()[i1][0]).getName());
			...
			if (f.getAdditionalSkillRequirements() != null) {
				for (int ii = 0; ii < f.getAdditionalSkillRequirements().length; ii++) {
					c.getPacketSender().sendString((38276 + (i2++)) + (i - 1000) * 6,
							Skill.forId(f.getAdditionalSkillRequirements()[ii][0]).getFormatName() + " " + f.getAdditionalSkillRequirements()[ii][1]);
					if (c.getSkillManager().getCurrentLevel(Skill.forId(f.getAdditionalSkillRequirements()[ii][0])) < f.getAdditionalSkillRequirements()[ii][1]) {
						canMake = false;
					}
				}
			}
			if (f.getFurnitureRequired() != -1) {
				Furniture fur = Furniture.forFurnitureId(f.getFurnitureRequired());
				c.getPacketSender().sendString((38276 + (i2++)) + (i - 1000) * 6, Misc.formatPlayerName(fur.toString().toLowerCase().replaceAll("_", " ")));
				if (canMake) {
					canMake = false;
					int[] myTiles = getMyChunk(c);
					for (HouseFurniture pf : c.getHouseFurniture()) {
						if (pf.getRoomX() == myTiles[0] - 1 && pf.getRoomY() == myTiles[1] - 1) {
							if (pf.getHotSpot(c.getHouseRooms()[c.inConstructionDungeon() ? 4 : c.getPosition().getZ()][myTiles[0] - 1][myTiles[1] - 1].getRotation()) == hs) {
								if (pf.getFurnitureId() != fur.getFurnitureId()) {
									canMake = false;
								} else {
									canMake = true;
	...
	public static void handleItemClick(int itemID, Player p) {
		...
		ArrayList<HotSpots> hsses = ConstructionData.HotSpots.forObjectId_2(p.getBuildFurnitureId());
		if (hsses.isEmpty())
			return;
		int[] myTiles = getMyChunk(p);
		...
		if (hsses.size() == 1) {
			...
			for (HotSpots find : hsses) {
				...
				int actualY = ConstructionData.BASE_Y + (myTiles[1] * 8);
				actualY += ConstructionData.getYOffsetForObjectId(find.getObjectId(), find, roomRot);
				if (p.getBuildFurnitureX() == actualX && p.getBuildFurnitureY() == actualY && myRoomType == find.getRoomType() || find.getCarpetDim() != null && myRoomType == find.getRoomType()) {
					s = find;
					break;
				}
			}
		...
		if (!buildActions(p, f, s))
			return;
		int actualX = ConstructionData.BASE_X + (myTiles[0] * 8);
		actualX += ConstructionData.getXOffsetForObjectId(f.getFurnitureId(), s, p.getRegionInstance().getOwner().getHouseRooms()[toHeight][myTiles[0] - 1][myTiles[1] - 1].getRotation());
		int actualY = ConstructionData.BASE_Y + (myTiles[1] * 8);
		...
		doFurniturePlace(s, f, hsses, myTiles, actualX, actualY, roomRot, p, false, p.getPosition().getZ(), false);
		HouseFurniture pf = new HouseFurniture(myTiles[0] - 1, myTiles[1] - 1, toHeight, s.getHotSpotId(), f.getFurnitureId(), s.getXOffset(), s.getYOffset());
		p.getHouseFurniture().add(pf);
		p.getPacketSender().sendInterfaceRemoval();
