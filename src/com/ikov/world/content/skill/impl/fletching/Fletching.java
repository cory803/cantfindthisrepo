	...
	public static void fletchBow(final Player player, final int product, final int amountToMake) {
		...
		player.setCurrentTask(new Task(2, player, true) {
			...
			public void execute() {
				...
				player.getSkillManager().addExperience(Skill.FLETCHING, shafts ? 80 : (int) (bow.getXp() * 15));
	...
	public static void makeBolts(final Player player, int item1, int item2) {
		...
		if (bolt != null) {
			if (player.getSkillManager().getCurrentLevel(Skill.FLETCHING) >= bolt.getLevelReq()) {
				if (player.getInventory().getAmount(bolt.getItem1()) >= 15 && player.getInventory().getAmount(bolt.getItem2()) >= 15) {
					player.getInventory().delete(new Item(bolt.getItem1()).setAmount(15), player.getInventory().getSlot(bolt.getItem1()), true);
					player.getInventory().delete(new Item(bolt.getItem2()).setAmount(15), player.getInventory().getSlot(bolt.getItem2()), true);
				...
				player.getPacketSender().sendMessage("You need a Fletching level of at least "+bolt.getLevelReq()+" to fletch this.");
			}
		}
	}
