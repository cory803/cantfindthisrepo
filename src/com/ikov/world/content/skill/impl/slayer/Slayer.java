...
public class Slayer {
  ...
  public void assignTask() {
    boolean hasTask = getSlayerTask() != SlayerTasks.NO_TASK
        && player.getSlayer().getLastTask() != getSlayerTask();
    ...
    DialogueManager.start(player,
        SlayerDialogues.receivedTask(player, getSlayerMaster(), getSlayerTask()));
  ...
  public void resetSlayerTask() {
    ...
    this.amountToSlay = 0;
    this.taskStreak = 0;
    player.getPointsHandler().setSlayerPoints(player.getPointsHandler().getSlayerPoints() - 5,
        false);
    PlayerPanel.refreshPanel(player);
  ...
  public void handleSlayerTaskDeath(boolean giveXp) {
    ...
    if (amountToSlay > 1) {
      ...
      player.getPacketSender().sendMessage("").sendMessage(
          "@red@You've completed your Slayer task! Return to a Slayer master for another one.");
      taskStreak++;
  ...
  public void givePoints(SlayerMaster master) {
    ...
    if (wearingHelm) {
      ...
      player.getPacketSender()
          .sendMessage("You received a bonus in slayer points for wearing a slayer helm.");
    }
    if (player.getSlayer().getTaskStreak() == 5) {
      ...
      player.getPacketSender().sendMessage(
          "You received " + per10 + " Slayer points and your Task Streak has been reset.");
  ...
  public void handleSlayerRingTP(int itemId) {
    ...
    if (task == SlayerTasks.NO_TASK)
      return;
    Position slayerTaskPos = new Position(task.getTaskPosition().getX(),
        task.getTaskPosition().getY(), task.getTaskPosition().getZ());
