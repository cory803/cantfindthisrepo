...
public class DesoSpan {
  ...
  enum Energy {
    ...
    Energy(int npcId, int levelReq, int experience, int playerGraphic, int projectileGraphic,
        int npcGraphic) {
      this.npcId = npcId;
    ...
    if (energyType != null) {
      ...
      if (player.getSkillManager().getCurrentLevel(Skill.RUNECRAFTING) < energyType.levelReq) {
        player.getPacketSender().sendMessage("You need a Runecrafting level of at least "
            + energyType.levelReq + " to siphon this energy source.");
        return;
      }
      if (!player.getInventory().contains(ENERGY_FRAGMENT)
      ...
      player.setCurrentTask(new Task(cycle, player, false) {
        ...
        public void execute() {
          ...
          if (Misc.getRandom(Skillcape_Data.RUNECRAFTING.isWearingCape(player) || Skillcape_Data.MASTER_RUNECRAFTING.isWearingCape(player) ? 50 : 30) <= 10) {
            player.dealDamage(new Hit(1 + Misc.getRandom(48),
                    Hitmask.RED, CombatIcon.DEFLECT));
            player.getPacketSender()
                    .sendMessage(
                            "You accidently attempt to siphon too much energy, and get hurt.");
          } else {
