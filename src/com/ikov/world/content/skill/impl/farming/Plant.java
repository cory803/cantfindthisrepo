        ...
        public void execute() {
          ...
          if (wearingMagicSecateurs) {
            ...
            player.getPacketSender()
                .sendMessage("You received 20% more harvest because of the magic secateurs.");
          } else {
          ...
          player.getPacketSender()
              .sendMessage("You harvest " + Misc.anOrA(name) + " " + name + ".");
          ...
          if (harvested == 3 && player.getInventory().contains(18336) || Skillcape_Data.MASTER_FARMING.isWearingCape(player)
                  && Misc.getRandom(Skillcape_Data.FARMING.isWearingCape(player) ? 6 : 4) == 0) {
            player.getPacketSender()
                    .sendMessage(
                            "You receive a seed back from your Scroll of life.");
            player.getInventory().add(Plants.values()[plant].seed,
                    1);
          }
          if (id == 219) {
  ...
  public void process(Player player) {
    ...
    if (elapsed >= grow) {
      for (int i = 0; i < elapsed / grow; i++) {
        /*
         * if (isDiseased()) { /*} else if (!isWatered()) { player.getPacketSender().sendMessage(
        ...
        if (isWatered()) {
          ...
          if (stage >= Plants.values()[plant].stages) {
            player.getPacketSender()
                .sendMessage("<img=4> <shad=996699>A seed which you planted has finished growing!");
            return;
          }
        }
