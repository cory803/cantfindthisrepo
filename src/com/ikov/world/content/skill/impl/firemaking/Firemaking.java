...
public class Firemaking {
  ...
  public static void lightFire(final Player player, int log, final boolean addingToFire,
      final int amount) {
    ...
    if (player.getPosition().getX() == 2352 && player.getPosition().getY() == 3703) {
      player.getPacketSender()
          .sendMessage("You can not light a fire on this spot try going south a few paces.");
      return;
    }
    ...
    boolean objectExists = CustomObjects.objectExists(
        new Position(player.getPosition().getLocalX() + 1, (player.getPosition().getLocalY())));
    if (!Dungeoneering.doingDungeoneering(player)) {
      if (objectExists || objectExists2 && !addingToFire || player.getPosition().getZ() > 0
          || !player.getMovementQueue().canWalk(1, 0) && !player.getMovementQueue().canWalk(-1, 0)
              && !player.getMovementQueue().canWalk(0, 1)
              && !player.getMovementQueue().canWalk(0, -1)) {
    ...
    if (player.getSkillManager().getMaxLevel(Skill.FIREMAKING) < logData.getLevel()) {
      player.getPacketSender().sendMessage(
          "You need a Firemaking level of atleast " + logData.getLevel() + " to light this.");
      return;
    }
    if (!addingToFire) {
