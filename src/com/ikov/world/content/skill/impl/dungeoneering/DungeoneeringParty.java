...
public class DungeoneeringParty {
  ...
  public void invite(Player p) {
    ...
    if (System.currentTimeMillis() - getOwner().getMinigameAttributes().getDungeoneeringAttributes()
        .getLastInvitation() < 2000) {
      getOwner().getPacketSender()
          .sendMessage("You must wait 2 seconds between each party invitation.");
      return;
    }
    if (p.busy()) {
    ...
    getOwner().getMinigameAttributes().getDungeoneeringAttributes()
        .setLastInvitation(System.currentTimeMillis());
    DialogueManager.start(p, new DungPartyInvitation(getOwner(), p));
    getOwner().getPacketSender()
        .sendMessage("An invitation has been sent to " + p.getUsername() + ".");
  }
  ...
  public void remove(Player p, boolean resetTab, boolean fromParty) {
    if (fromParty) {
      ...
      if (resetTab) {
        ...
        p.getPacketSender().sendTabInterface(GameSettings.QUESTS_TAB,
            Dungeoneering.FORM_PARTY_INTERFACE);
        p.getPacketSender().sendDungeoneeringTabIcon(true);
        p.getPacketSender().sendTab(GameSettings.QUESTS_TAB);
      }
    }
    ...
    if (p == owner) {
      for (Player member : player_members) {
        if (member != null
            && member.getMinigameAttributes().getDungeoneeringAttributes().getParty() != null
            && member.getMinigameAttributes().getDungeoneeringAttributes().getParty() == this) {
          if (member == owner)
            continue;
          if (fromParty) {
            member.getPacketSender()
                .sendMessage("Your party has been deleted by the party's leader.");
      ...
      if (hasEnteredDungeon) {
        for (NPC npc : p.getMinigameAttributes().getDungeoneeringAttributes().getParty()
            .getNpcs()) {
          if (npc != null && npc.getPosition().getZ() == p.getPosition().getZ())
            World.deregister(npc);
        }
        for (GroundItem groundItem : p.getMinigameAttributes().getDungeoneeringAttributes()
            .getParty().getGroundItems()) {
          if (groundItem != null)
    ...
    if (hasEnteredDungeon) {
      ...
      int exp = (int) (damage > 0
          ? (p.getMinigameAttributes().getDungeoneeringAttributes().getDamageDealt()
              * p.getSkillManager().getCurrentLevel(Skill.DUNGEONEERING) / 6.5)
              - (deaths * 3800 + Misc.getRandom(400))
          : 0);
      ...
      if (exp > 0 && tokens > 0) {
        ...
        p.getPacketSender()
            .sendMessage("<img=4> <col=660000>You've received some Dungeoneering experience and "
  ...
  public void refreshInterface() {
    for (Player member : getPlayers()) {
      if (member != null) {
        ...
        member.getPacketSender().sendString(26240,
            floor == null ? "-" : "" + (floor.ordinal() + 1));
