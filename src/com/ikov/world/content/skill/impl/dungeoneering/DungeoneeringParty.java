...
public class DungeoneeringParty {
	...
	public void invite(Player p) {
		...
		if(System.currentTimeMillis() - getOwner().getMinigameAttributes().getDungeoneeringAttributes().getLastInvitation() < 2000) {
	...
	public void remove(Player p, boolean resetTab, boolean fromParty) {
		...
		if(hasEnteredDungeon) {
			...
			int exp = (int) (damage > 0 ? (p.getMinigameAttributes().getDungeoneeringAttributes().getDamageDealt() * p.getSkillManager().getCurrentLevel(Skill.DUNGEONEERING) / 6.5) - (deaths * 3800 + Misc.getRandom(400)) : 0);
