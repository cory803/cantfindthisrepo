...
public class Dungeoneering {
  ...
  public static void start(final Player p) {
    ...
    final DungeoneeringParty party =
    ...
    if (GameServer.isUpdating()) {
      p.getPacketSender()
          .sendMessage("You can't start a game of Dungeoneering while a update is happening!");
      return;
    }
    if (party.getOwner() != p) {
      ...
      if (member != null) {
        ...
        if (member.getSummoning().getFamiliar() != null) {
          member.getPacketSender().sendMessage(
              "You must dismiss your familiar before being allowed to enter a dungeon.");
          p.getPacketSender().sendMessage("" + p.getUsername()
              + " has to dismiss their familiar before you can enter the dungeon.");
          return;
        }
        ...
        if (plrCannotEnter != null) {
          p.getPacketSender()
              .sendMessage(
                  "Your team cannot enter the dungeon because " + plrCannotEnter + " hasn't banked")
    ...
    party.setKills(0);
    party.sendMessage(
        "Welcome to Dungeoneering floor " + (party.getDungeoneeringFloor().ordinal() + 1)
            + ", complexity level " + party.getComplexity() + ".");
  ...
  public static void leave(Player p, boolean resetTab, boolean leaveParty) {
    if (p.getMinigameAttributes().getDungeoneeringAttributes().getParty() != null) {
      p.getMinigameAttributes().getDungeoneeringAttributes().getParty().remove(p, resetTab,
          leaveParty);
    } else if (resetTab) {
      p.getPacketSender().sendTabInterface(GameSettings.QUESTS_TAB,
          p.isKillsTrackerOpen() ? 55000 : 55065);
  ...
  public static void setupFloor(DungeoneeringParty party, int height) {
     ...
     */
    NPC smuggler =
        new NPC(11226, new Position(party.getDungeoneeringFloor().getSmugglerPosition().getX(),
  ...
  private static final Item[] misc = {new Item(555, 121), new Item(14484, 1), new Item(557, 87),
      new Item(554, 81), new Item(565, 63), new Item(5678), new Item(560, 97), new Item(861, 1),
      new Item(892, 127), new Item(1305, 1), new Item(15486, 1), new Item(9185, 1),
      new Item(18161, 2), new Item(18159, 2), new Item(139, 1)};
  ...
  public static void handleNpcDeath(Player p, NPC n) {
    if (n.getPosition().getZ() == p.getPosition().getZ()) {
      ...
      boolean boss =
          n.getId() == 2060 || n.getId() == 8549 || n.getId() == 1382 || n.getId() == 9939;
      if (boss) {
      ...
      if (random >= 20 && random <= 45 || boss) {
        GroundItemManager.spawnGroundItem(p,
            new GroundItem(new Item(ItemBinding.getRandomBindableItem()), n.getPosition().copy(),
                "Dungeoneering", false, -1, false, -1));
        if (boss) {
          party.sendMessage(
              "@red@The boss has been slain! Feel free to exit the dungeon via tha northern ladder.");
        }
      } else if (random >= 100 && random <= 150) {
        int amt = 3000 + Misc.getRandom(10000);
        GroundItemManager.spawnGroundItem(p, new GroundItem(new Item(18201, amt),
            n.getPosition().copy(), "Dungeoneering", false, -1, false, -1));
      } else if (random > 150 && random < 250)
        GroundItemManager.spawnGroundItem(p, new GroundItem(misc[Misc.getRandom(misc.length - 1)],
            n.getPosition().copy(), "Dungeoneering", false, -1, false, -1));
    }
  }
