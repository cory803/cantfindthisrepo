...
import com.ikov.world.content.skill.impl.mining.MiningData.Ores;
...
public class Mining {
  ...
  public static void startMining(final Player player, final GameObject oreObject) {
    ...
    final int reqCycle =
        o == Ores.Runite ? 6 + Misc.getRandom(2) : Misc.getRandom(o.getTicks() - 1);
    if (o != null) {
                ...
                if (cycle == reqCycle) {
                  ...
                  if (o.getRespawn() > 0) {
                    ...
                    if (oreObject.getId() == 38660) {
                      if (ShootingStar.CRASHED_STAR == null || ShootingStar.CRASHED_STAR
                          .getStarObject().getPickAmount() >= ShootingStar.MAXIMUM_MINING_AMOUNT) {
  ...
  public static void oreRespawn(final Player player, final GameObject oldOre, Ores o) {
    ...
    player.getPacketSender().sendClientRightClickRemoval();
    player.getSkillManager().stopSkilling();
    CustomObjects.globalObjectRespawnTask(new GameObject(452, oldOre.getPosition().copy(), 10, 0),
        oldOre, o.getRespawn());
  }
}
