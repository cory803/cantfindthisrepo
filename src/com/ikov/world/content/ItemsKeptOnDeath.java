...
public class ItemsKeptOnDeath {
  ...
  public static void sendInterfaceData(Player player) {
    ...
    switch (size) {
      case 0: // Player must be skulled WITHOUT the protect item prayer/curse active, player will
              // keep 0 items.
        infoToFill[0] = "You are skulled and will";
        infoToFill[1] = "not keep any items on";
        infoToFill[2] = "death.";
        break;
      case 1: // Player must be skulled but have the protect item prayer/curse active, player will
              // keep 1 item.
      case 4: // Player is not skulled & has protect item prayer/curse active, player will keep 4
    ...
    player.getPacketSender()
        .sendMessage("@red@Note: All untradeable items are automatically kept on death!");
    ArrayList<Item> toKeep = getItemsToKeep(player);
    for (int i = 0; i < toKeep.size(); i++) {
      player.getPacketSender().sendItemOnInterface(17108 + i, toKeep.get(i).getId(), 1);
    }
    int toSend = 17112;
    for (Item item : Misc.concat(player.getInventory().getItems(),
