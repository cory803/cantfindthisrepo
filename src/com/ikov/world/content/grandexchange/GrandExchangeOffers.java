    ...
    try {
      ...
      if (count > 0) {
        for (int i = 0; i < count; i++) {
          ...
          OFFERS[arrayIndex] = new GrandExchangeOffer(id, quantity, listedBy, arrayIndex,
              pricePerItem, box, amountFinished, coinsCollect, itemCollect, failedAttempts, type,
              updateStateOrdinal);
        }
      }
      ...
      for (GrandExchangeOffer o : OFFERS) {
        ...
        if (item1 == item2) {
          if (o.getType() == OfferType.SELLING && offer.getType() == OfferType.BUYING) {
            if (o.getPricePerItem() > offer.getPricePerItem()
                && o.getAmountFinished() < o.getAmount()) {
              ...
              if (o.getFailAttempts() >= 3) {
                ...
                if (p != null) {
                  p.getPacketSender().sendMessage("")
                      .sendMessage(
                          "<img=4> <col=996633>Perhaps you should try lowering the price on your "
                              + ItemDefinition.forId(item2).getName() + " offer")
                      .sendMessage(
            ...
            if (difference >= 0) {
              ...
              for (int i = 0; i <= remainingOfferAmount; i++) {
                if (o.getAmountFinished() >= o.getAmount()
                    || offer.getAmountFinished() >= offer.getAmount()) {
                  break;
                }
  ...
  public static void save() {
    try {
      DataOutputStream out =
          new DataOutputStream(new FileOutputStream("./data/saves/GE/offers.dat", false));
