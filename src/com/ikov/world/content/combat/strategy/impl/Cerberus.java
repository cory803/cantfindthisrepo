...
public class Cerberus implements CombatStrategy {
    ...
    public boolean customContainerAttack(Character entity, Character victim) {
        ...
        if (cerberus.getConstitution() <= 3000 && !cerberus.hasHealed()) {
            ...
            NPC melee = new NPC(750, new Position(player.getPosition().getX(), player.getPosition().getY(), 0)).setSpawnedFor(player);
            NPC magic = new NPC(750, new Position(player.getPosition().getX(), player.getPosition().getY(), 0)).setSpawnedFor(player);
            NPC ranged = new NPC(750, new Position(player.getPosition().getX(), player.getPosition().getY(), 0)).setSpawnedFor(player);
            //World.register(melee);
            //World.register(magic);
        ...
        if (random <= 1 && Locations.goodDistance(cerberus.getPosition().getX(), cerberus.getPosition().getY(),
            ...
            cerberus.getCombatBuilder()
                    .setContainer(new CombatContainer(cerberus, victim, 1, 6, CombatType.MAGIC, true));
