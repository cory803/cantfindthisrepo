...
public class Nex implements CombatStrategy {
    ...
    if (phase == 0) {
      ...
      if (rnd <= 2) {
        ...
        new CombatHit(NEX.getCombatBuilder(),
            new CombatContainer(NEX, p, 1, CombatType.MAGIC, true)).handleAttack();
        return true;
      }
    ...
    if (phase == 3) {
      if (NEX.getConstitution() <= 4000) {
        ...
        if (!magesAttackable[3]) {
          ...
          sendGlobalMsg(from,
              "@red@Glacies is now attackable! You need to defeat him to weaken Nex!");
    ...
    if (phase == 2) {
      if (NEX.getConstitution() <= 8000) {
        ...
        if (!magesAttackable[2]) {
          ...
          sendGlobalMsg(from,
              "@red@Cruor is now attackable! You need to defeat him to weaken Nex!");
    ...
    if (phase == 1) {
      if (NEX.getConstitution() <= 12000) {
        ...
        if (!magesAttackable[1]) {
          ...
          sendGlobalMsg(from,
              "@red@Umbra is now attackable! You need to defeat him to weaken Nex!");
    ...
    if (phase == 0) {
      if (NEX.getConstitution() <= 16000) {
        ...
        if (!magesAttackable[0]) {
          ...
          sendGlobalMsg(from,
              "@red@Fumus is now attackable! You need to defeat her to weaken Nex!");
  ...
  public static boolean checkAttack(Player p, int npc) {
    ...
    if (!magesAttackable[index] && !magesKilled[index]) {
      p.getPacketSender().sendMessage(
          "" + NpcDefinition.forId(npc).getName() + " is currently being protected by Nex!");
      return false;
    }
    return true;
  }
      ...
      public void execute() {
        ...
        for (Skill skill : Skill.values()) {
          if (skill != Skill.CONSTITUTION && skill != Skill.PRAYER) {
            p.getSkillManager().setCurrentLevel(skill,
                p.getSkillManager().getCurrentLevel(skill) - 1);
  ...
  public static void setShadow(Player p, int shadow) {
    ...
    p.getPacketSender().sendShadow()
        .sendMessage("@whi@Nex calls upon shadows to endarken your vision!");
  }
