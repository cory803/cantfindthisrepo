...
public class Revenant implements CombatStrategy {
  ...
  enum REVENANT_DATA {
    ...
    REVENANT_IMP(13465, new Animation(7500), new Animation(7501)), REVENANT_GOBLIN(13469,
        new Animation(7499), new Animation(7513)), REVENANT_WEREWOLF(13474, new Animation(7496),
            new Animation(7521)), REVENANT_ORK(13478, new Animation(7505),
                new Animation(7518)), REVENANT_DARK_BEAST(13479, new Animation(7502),
                    new Animation(7514));
  ...
  public boolean canAttack(Character entity, Character victim) {
    return victim.getLocation() == Location.WILDERNESS
        || victim.getLocation() == Location.EZONE_DONOR;
  }
  ...
  public boolean customContainerAttack(Character entity, Character victim) {
    ...
    final CombatType attkType = Misc.getRandom(5) <= 2
        && Locations.goodDistance(revenant.getPosition(), revenant.getPosition(), 2)
            ? CombatType.MELEE : Misc.getRandom(10) <= 5 ? CombatType.MAGIC : CombatType.RANGED;
    switch (attkType) {
      case MELEE:
      ...
      case RANGED:
        ...
        revenant.performAnimation(
            attkType == CombatType.MAGIC ? revData.magicAttack : revData.rangedAttack);
        revenant.getCombatBuilder()
            .setContainer(new CombatContainer(revenant, victim, 1, 2, attkType, true));
        TaskManager.submit(new Task(1, revenant, false) {
          ...
          public void execute() {
            switch (tick) {
              case 1:
                new Projectile(revenant, victim, (attkType == CombatType.RANGED ? 970 : 280), 44, 3,
                    43, 43, 0).sendProjectile();
                break;
              case 3:
