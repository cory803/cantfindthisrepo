...
public class Autocasting {
  ...
  private static int[] spellIds = {1152, 1154, 1156, 1158, 1160, 1163, 1166, 1169, 1171, 1172, 1175,
      1177, 1181, 1183, 1185, 1188, 1189, 1539, 12037, 1190, 1191, 1192, 12939, 12987, 12901, 12861,
      12963, 13011, 13023, 12919, 12881, 12951, 12999, 12911, 12871, 12975, 12929, 12891, 21744,
      22168, 21745, 21746, 50163, 50211, 50119, 50081, 50151, 50199, 50111, 50071, 50175, 50223,
      50129, 50091};
    ...
    for (int i = 0; i < spellIds.length; i++) {
      if (actionButtonId == spellIds[i]) {
        ...
        if (cbSpell.levelRequired() > p.getSkillManager().getCurrentLevel(Skill.MAGIC)) {
          p.getPacketSender().sendMessage("You need a Magic level of at least "
              + cbSpell.levelRequired() + " to cast this spell.");
          resetAutocast(p, true);
          return true;
        }
        p.setAutocast(true);
