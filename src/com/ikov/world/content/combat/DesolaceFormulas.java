...
public class DesolaceFormulas {
  ...
  public static int getMeleeAttack(Player plr) {
    ...
    if (PrayerHandler.isActivated(plr, PrayerHandler.CLARITY_OF_THOUGHT)) {
      ...
      attackLevel +=
          plr.getSkillManager().getMaxLevel(Skill.ATTACK) * 0.05 + plr.getLeechedBonuses()[2];
    } else if (CurseHandler.isActivated(plr, CurseHandler.TURMOIL)) {
      attackLevel +=
          plr.getSkillManager().getMaxLevel(Skill.ATTACK) * 0.3 + plr.getLeechedBonuses()[2];
    }
  ...
  public static int bestMeleeDef(Player p) {
    ...
    return p.getBonusManager().getDefenceBonus()[2] <= p.getBonusManager().getDefenceBonus()[0]
        || p.getBonusManager().getDefenceBonus()[2] <= p.getBonusManager().getDefenceBonus()[1] ? 0
            : 2;
  }
  ...
  public static int bestMeleeAtk(Player p) {
    ...
    return p.getBonusManager().getAttackBonus()[2] <= p.getBonusManager().getAttackBonus()[1]
        || p.getBonusManager().getAttackBonus()[2] <= p.getBonusManager().getAttackBonus()[0] ? 0
            : 2;
  }
  ...
  public static double getEffectiveStr(Player plr) {
    return ((plr.getSkillManager().getCurrentLevel(Skill.STRENGTH)) * getPrayerStr(plr))
        + getStyleBonus(plr);
  }
  ...
  public static int getRangedAttack(Player plr) {
    ...
    if (hasVoid) {
      rangeLevel +=
          SkillManager.getLevelForExperience(plr.getSkillManager().getExperience(Skill.RANGED))
              * 0.15;
    }
    if (plr.getCurseActive()[PrayerHandler.SHARP_EYE]
        || plr.getCurseActive()[CurseHandler.SAP_RANGER]) {
      rangeLevel *= 1.05;
    ...
    if (hasVoid && accuracy > 1.15)
      rangeLevel *= 1.68;
    /*
     * Slay helm if(plr.getAdvancedSkills().getSlayer().getSlayerTask() != null &&
     ...
     * plr.getAdvancedSkills().getSlayer().getSlayerTask().getNpcId()) rangeLevel *= 1.12; } }
     */
  ...
  public static int getRangedDefence(Player plr) {
    ...
    if (plr.getPrayerActive()[PrayerHandler.THICK_SKIN]) {
      ...
      defenceLevel +=
          plr.getSkillManager().getMaxLevel(Skill.DEFENCE) * 0.20 + plr.getLeechedBonuses()[0];
    }
  ...
  public static int getMagicAttack(Player plr) {
    ...
    if (plr.getPrayerActive()[PrayerHandler.MYSTIC_WILL]
        || plr.getCurseActive()[CurseHandler.SAP_MAGE]) {
      attackLevel *= 1.05;
  ...
  public static int getAttackDelay(Player plr) {
    ...
    if (rangedData != null) {
      ...
      if (plr.getFightType() == FightType.SHORTBOW_RAPID
          || plr.getFightType() == FightType.DART_RAPID
          || plr.getFightType() == FightType.KNIFE_RAPID
          || plr.getFightType() == FightType.THROWNAXE_RAPID
