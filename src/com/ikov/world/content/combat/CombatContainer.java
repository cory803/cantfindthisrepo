...
public class CombatContainer {
  ...
  public CombatContainer(Character attacker, Character victim, int hitAmount, int hitDelay,
  ...
  /**
   * Create a new {@link CombatContainer} that will deal no hit this turn. Used for things like
   * spells that have special effects but don't deal damage.
   * 
   * @param checkAccuracy if accuracy should be taken into account.
   */
  public CombatContainer(Character attacker, Character victim, CombatType hitType,
      boolean checkAccuracy) {
  ...
  private final ContainerHit[] prepareHits(int hitAmount) {
    ...
    for (int i = 0; i < array.length; i++) {
      boolean accuracy =
          checkAccuracy ? CombatFactory.rollAccuracy(attacker, victim, combatType) : true;
    ...
    if (attacker.isPlayer() && ((Player) attacker).isSpecialActivated()) {
      ...
      } else if (((Player) attacker).getCombatSpecial() == CombatSpecial.DARK_BOW
          && hitAmount == 2) {
  ...
  /**
   * Performs an action on every single hit in this container. the action to perform on every single
   * hit.
   */
  ...
  public final void dealDamage() {
    if (hits.length == 1) {
      ...
      victim.dealQuadrupleDamage(hits[0].getHit(), hits[1].getHit(), hits[2].getHit(),
          hits[3].getHit());
    }
  }
  ...
  /**
   * A dynamic method invoked when the victim is hit with an attack. An example of usage is using
   * this to do some sort of special effect when the victim is hit with a spell. <b>Do not reset
   * combat builder in this method!</b>
   * 
   * @param damage the damage inflicted with this attack, always 0 if the attack isn't accurate.
