...
/**
 * A graphic propelled through the air by some sort of spell, weapon, or other miscellaneous force.
 * 
 * @author lare96
 */
public final class Projectile {
   ...
   */
  public Projectile(Position start, Position end, int lockon, int projectileId, int speed,
      int delay, int startHeight, int endHeight, int curve) {
    this.start = start;
   ...
   * @param curve the curve angle of the projectile.
   */
  public Projectile(Entity source, Entity victim, int projectileId, int delay, int speed,
      int startHeight, int endHeight, int curve) {
    this(source.getPosition(), victim.getPosition(),
        (victim.isPlayer() ? -victim.getIndex() - 1 : victim.getIndex() + 1), projectileId, delay,
        speed, startHeight, endHeight, curve);
  }

  /**
   * Sends one projectiles using the values set when the {@link Projectile} was constructed.
   */
  public void sendProjectile() {
    for (Player player : World.getPlayers()) {
      ...
      if (start.isViewableFrom(player.getPosition())) {
        player.getPacketSender().sendProjectile(start, offset, 0, speed, projectileId, startHeight,
            endHeight, lockon, delay);
  ...
  /**
   * Sends <code>count</code> projectiles using the values set when the {@link Projectile} was
   * constructed.
   * 
