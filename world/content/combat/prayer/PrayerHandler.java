...
public class PrayerHandler {
	...
	private static void startDrain(final Player player) {
		...
		TaskManager.submit(new Task(1, player, true) {
			@Override
			public void execute() {
				if((player.getRights() == PlayerRights.EXTREME_DONATOR || player.getRights() == PlayerRights.DIAMOND_DONATOR || player.getRights() == PlayerRights.ONYX_DONATOR) && player.getLocation() != Location.WILDERNESS)
					return;
				if (player.getSkillManager().getCurrentLevel(Skill.PRAYER) <= 0) {
					for (int i = 0; i < player.getPrayerActive().length; i++) {
						if (player.getPrayerActive()[i])
				...
				if(player.getLocation() != Location.WILDERNESS) {
					if(player.getRights() == PlayerRights.EXTREME_DONATOR || player.getRights() == PlayerRights.DIAMOND_DONATOR || player.getRights() == PlayerRights.ONYX_DONATOR || player.getEquipment().get(Equipment.CAPE_SLOT).getId() == 19748) {
						drainAmount = 0;
					}
				}
